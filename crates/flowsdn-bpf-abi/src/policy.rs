...
impl PolicyKey {
    ...
    pub fn new(
        ...
        let prefixlen = if protocol == 0 {
            40
        } else {
            48u32.saturating_add(u32::from(port_prefix))
        };
        let mask = u16::MAX
            .checked_shl(u32::from(16u8.saturating_sub(port_prefix)))
            .unwrap_or(0);
        Ok(Self {
            prefixlen,
            sec_label: identity,
            egress: u8::from(egress),
            protocol,
            dport: Be16::new(port & mask),
        })
    }
}
impl MapBytes<12> for PolicyKey {
    ...
    fn from_bytes(b: [u8; 12]) -> Self {
        Self {
            ...
            sec_label: u32::from_le_bytes(take(&b, 4)),
            egress: take::<1, 12>(&b, 8)[0],
            protocol: take::<1, 12>(&b, 9)[0],
            dport: Be16(take(&b, 10)),
        }
    }
}
...
impl PolicyEntry {
    ...
    pub fn new(
        proxy_port: u16,
        deny: bool,
        lpm_prefix_length: u8,
        auth_type: u8,
        explicit_auth: bool,
        precedence: u32,
        cookie: u32,
    ) -> Result<Self, InvalidPolicyFields> {
        if !(lpm_prefix_length == 0 || (8..=24).contains(&lpm_prefix_length)) || auth_type > 127 {
            return Err(InvalidPolicyFields);
        }
        Ok(Self {
            proxy_port: Be16::new(proxy_port),
            flags: u8::from(deny) | (lpm_prefix_length << 3),
            auth: auth_type | (u8::from(explicit_auth) << 7),
            precedence,
            cookie,
        })
    }
    ...
    pub const fn lpm_prefix_length(self) -> u8 {
        self.flags >> 3
    }
    pub const fn auth_type(self) -> u8 {
        self.auth & 0x7f
    }
    pub const fn has_explicit_auth_type(self) -> bool {
        self.auth & 0x80 != 0
    }
}
impl MapBytes<12> for PolicyEntry {
    ...
    fn from_bytes(b: [u8; 12]) -> Self {
        Self {
            proxy_port: Be16(take(&b, 0)),
            flags: take::<1, 12>(&b, 2)[0],
            auth: take::<1, 12>(&b, 3)[0],
            precedence: u32::from_le_bytes(take(&b, 4)),
            cookie: u32::from_le_bytes(take(&b, 8)),
        }
    }
}
...
impl MapBytes<12> for PolicyStatsKey {
    ...
    fn from_bytes(b: [u8; 12]) -> Self {
        Self {
            endpoint_id: u16::from_le_bytes(take(&b, 0)),
            pad1: take::<1, 12>(&b, 2)[0],
            prefix_len: take::<1, 12>(&b, 3)[0],
            sec_label: u32::from_le_bytes(take(&b, 4)),
            egress: take::<1, 12>(&b, 8)[0],
            protocol: take::<1, 12>(&b, 9)[0],
            dport: Be16(take(&b, 10)),
...
impl MapBytes<16> for PolicyStatsValue {
    ...
    fn from_bytes(b: [u8; 16]) -> Self {
        Self {
            packets: u64::from_le_bytes(take(&b, 0)),
            bytes: u64::from_le_bytes(take(&b, 8)),
        }
    }
}
