...
impl<'de> serde::Deserialize<'de> for Fields {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        impl<'de> serde::de::Visitor<'de> for FieldsVisitor {
            ...
            fn visit_map<M: serde::de::MapAccess<'de>>(self, mut input: M) -> Result<Fields, M::Error> {
...
pub struct Step { pub nodes: u64, pub limit: f64, pub burst: u32 }
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum Error { Json, Empty, Object, UnknownField(String), DuplicateField(String), MissingField(&'static str), Nodes, Limit, Burst, DuplicateThreshold }
#[derive(Clone, Debug)]
pub struct RateTable { first: Step, rest: Vec<Step> }
impl Default for RateTable {
    fn default() -> Self { Self { first: Step { nodes: 0, limit: 10.0, burst: 20 }, rest: Vec::new() } }
}
impl RateTable {
    pub fn parse(input: &str) -> Result<Self, Error> {
        ...
        for row in rows {
            ...
            for (name, value) in fields {
                let slot = match name.to_ascii_lowercase().as_str() {
                    "nodes" => &mut nodes, "limit" => &mut limit, "burst" => &mut burst,
                    _ => return Err(Error::UnknownField(name.clone())),
                };
                if slot.replace(value).is_some() { return Err(Error::DuplicateField(name.clone())); }
            }
            let nodes = nodes.ok_or(Error::MissingField("nodes"))?.as_u64().ok_or(Error::Nodes)?;
            let limit = limit.ok_or(Error::MissingField("limit"))?.as_f64().filter(|v| v.is_finite() && *v > 0.0).ok_or(Error::Limit)?;
            let burst = burst.ok_or(Error::MissingField("burst"))?.as_u64().and_then(|v| u32::try_from(v).ok()).filter(|v| *v > 0).ok_or(Error::Burst)?;
            steps.push(Step { nodes, limit, burst });
        }
        ...
        for step in steps {
            if step.nodes == previous { return Err(Error::DuplicateThreshold); }
            previous = step.nodes;
            rest.push(step);
        }
        Ok(Self { first, rest })
    ...
    pub fn select(&self, nodes: u64) -> Step {
        self.rest.iter().take_while(|step| step.nodes <= nodes).last().copied().unwrap_or(self.first)
    }
}
#[derive(Debug)]
pub struct Selection { table: RateTable, current: Step }
impl Selection {
    pub fn new(table: RateTable) -> Self { let current = table.select(0); Self { table, current } }
    pub fn current(&self) -> Step { self.current }
