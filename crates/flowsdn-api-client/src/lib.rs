...
use nix::{
    ...
    sys::socket::{AddressFamily, SockFlag, SockType, UnixAddr, connect, getsockopt, socket, sockopt},
};
use serde_json::Value;
use std::{
    fmt, io::{self, Write}, net::IpAddr,
...
impl Error {
    ...
    pub fn is_transport(&self) -> bool { matches!(self, Self::Transport(_) | Self::Timeout) }
}
...
impl From<io::Error> for Error {
    fn from(e: io::Error) -> Self {
        if matches!(e.kind(), io::ErrorKind::TimedOut | io::ErrorKind::WouldBlock) { Self::Timeout }
        else { Self::Transport(e) }
    }
}
fn errno(e: Errno) -> Error { io::Error::from_raw_os_error(e as i32).into() }
...
impl Default for Limits {
    fn default() -> Self { Self { header_bytes: 16_384, body_bytes: 4_194_304, wire_bytes: 8_388_608 } }
}
...
#[derive(Clone, Debug)]
pub struct Client { socket: PathBuf, timeout: Duration, limits: Limits }
impl Client {
    pub fn new(path: impl AsRef<Path>, timeout: Duration) -> Self {
        Self { socket: path.as_ref().to_owned(), timeout, limits: Limits::default() }
    }
    pub fn with_limits(mut self, limits: Limits) -> Self { self.limits = limits; self }
    ...
    fn request_inner(&self, method: Method, path: &str, body: Option<&Value>, expiration: Option<bool>, unbounded_response: bool) -> Result<Response> {
        ...
        if !path.starts_with('/') || !path.is_ascii()
            ...
            return Err(Error::InvalidRequest("expected an escaped absolute HTTP path".into()));
        }
        let body = body.map(serde_json::to_vec).transpose()
            .map_err(|e| Error::InvalidRequest(e.to_string()))?.unwrap_or_default();
        if body.len() > self.limits.body_bytes { return Err(Error::Limit("request body")); }
        let mut head = format!("{} {path} HTTP/1.1\r\nHost: localhost\r\nAccept: application/json\r\nConnection: close\r\nContent-Length: {}\r\n", method.text(), body.len());
        if !body.is_empty() { head.push_str("Content-Type: application/json\r\n"); }
        if let Some(expiration) = expiration { head.push_str(if expiration { "expiration: true\r\n" } else { "expiration: false\r\n" }); }
        head.push_str("\r\n");
        if head.len() > self.limits.header_bytes { return Err(Error::Limit("request header")); }
        ...
        wire::response(stream, if unbounded_response { None } else { Some(deadline) }, self.limits)
    }

    pub fn config(&self) -> Result<Response> { self.request(Method::Get, "/v1/config", None) }
    pub fn allocate(&self, owner: &str, family: &str, pool: &str, expiration: bool) -> Result<Response> {
        let mut path = format!("/v1/ipam?owner={}", encode_component(owner));
        if !family.is_empty() { path.push_str(&format!("&family={}", encode_component(family))); }
        if !pool.is_empty() { path.push_str(&format!("&pool={}", encode_component(pool))); }
        self.request_inner(Method::Post, &path, None, Some(expiration), false)
    }
    pub fn release(&self, ip: IpAddr, pool: &str) -> Result<Response> {
        self.request(Method::Delete, &format!("/v1/ipam/{}?pool={}", encode_component(&ip.to_string()), encode_component(pool)), None)
    }
    ...
    pub fn delete_container(&self, container_id: &str) -> Result<Response> {
        self.request(Method::Delete, "/v1/endpoint", Some(&serde_json::json!({"container-id": container_id})))
    }
...
pub fn endpoint_path(id: &str) -> String { format!("/v1/endpoint/{}", encode_component(id)) }

/// RFC3986 unreserved bytes survive; every other UTF-8 byte is percent escaped.
pub fn encode_component(value: &str) -> String {
    let mut encoded = String::new();
    for byte in value.bytes() {
        if byte.is_ascii_alphanumeric() || matches!(byte, b'-' | b'.' | b'_' | b'~') { encoded.push(char::from(byte)); }
        else { use fmt::Write; let _ = write!(encoded, "%{byte:02X}"); }
    }
    encoded
}
...
pub(crate) fn remaining(deadline: Instant) -> Result<Duration> {
    ...
    if remaining.is_zero() { Err(Error::Timeout) } else { Ok(remaining) }
}

fn connect_deadline(path: &Path, deadline: Instant) -> Result<UnixStream> {
    let address = UnixAddr::new(path).map_err(errno)?;
    let fd = socket(AddressFamily::Unix, SockType::Stream, SockFlag::SOCK_CLOEXEC | SockFlag::SOCK_NONBLOCK, None).map_err(errno)?;
    loop {
        ...
        match connect(fd.as_raw_fd(), &address) {
            ...
            Err(Errno::EINPROGRESS | Errno::EALREADY) => {
                ...
                match poll(&mut fds, timeout) {
                    Ok(0) => { remaining(deadline)?; continue; }
                    Ok(_) => {
                        let code = getsockopt(&fd, sockopt::SocketError).map_err(errno)?;
                        if code != 0 { return Err(io::Error::from_raw_os_error(code).into()); }
                        break;
                    }
                    Err(Errno::EINTR) => continue,
...
fn write_deadline(stream: &mut UnixStream, mut bytes: &[u8], deadline: Instant) -> Result<()> {
    while !bytes.is_empty() {
        ...
        match stream.write(bytes) {
            ...
            Ok(count) => bytes = bytes.get(count..).ok_or_else(|| Error::Protocol("invalid write count".into()))?,
