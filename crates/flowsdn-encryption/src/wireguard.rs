...
use netlink_packet_wireguard::{
    WireguardAddressFamily as Family, WireguardAllowedIp, WireguardAllowedIpAttr as IpAttr,
    WireguardAttribute as Attr, WireguardCmd, WireguardDeviceFlags, WireguardMessage,
    WireguardPeer, WireguardPeerAttribute as PeerAttr,
};
use std::{
    collections::HashSet,
...
fn message(ifindex: u32, replace: bool, attributes: Vec<PeerAttr>) -> WireguardMessage {
    ...
    if replace {
        attrs.push(Attr::Flags(WireguardDeviceFlags::ReplacePeers));
    }
...
fn prefix(address: IpAddr, cidr: u8) -> WireguardAllowedIp {
    WireguardAllowedIp(vec![
        IpAttr::Family(if address.is_ipv4() {
            Family::Ipv4
        } else {
            Family::Ipv6
        }),
        IpAttr::IpAddr(address),
        IpAttr::Cidr(cidr),
    ])
}
fn attrs(peer: &Peer, first: bool, ips: Vec<WireguardAllowedIp>) -> Vec<PeerAttr> {
    ...
    if !first {
        flags.remove(PeerFlags::ReplaceAllowedIps);
    }
    let mut result = vec![PeerAttr::PublicKey(peer.public_key), PeerAttr::Flags(flags)];
    if first {
        if let Some(endpoint) = peer.endpoint {
            result.push(PeerAttr::Endpoint(endpoint));
        }
        if let Some(keepalive) = peer.keepalive {
            result.push(PeerAttr::PersistentKeepalive(keepalive));
        }
    }
    if !peer.flags.contains(PeerFlags::RemoveMe) {
        result.push(PeerAttr::AllowedIps(ips));
    }
    result
}
...
pub fn plan(
    ...
    peers: &[Peer],
    budget: usize,
) -> Result<Vec<WireguardMessage>, Error> {
    ...
    for peer in peers {
        total = total
            .checked_add(peer.allowed_ips.len())
            .ok_or(Error("prefix count overflow"))?;
        if total > 1_000_000
            || !keys.insert(peer.public_key)
            || peer.public_key == [0; 32]
            || peer.flags.bits() & !7 != 0
        {
        ...
        if peer.flags.contains(PeerFlags::RemoveMe)
            && (!peer.allowed_ips.is_empty()
                ...
                || peer.keepalive.is_some()
                || peer.flags.contains(PeerFlags::ReplaceAllowedIps))
        {
        ...
        if peer
            .allowed_ips
            .iter()
            .any(|(ip, mask)| *mask > if ip.is_ipv4() { 32 } else { 128 })
        {
    ...
    if peers.is_empty() {
        ...
        if replace_peers {
            attributes.push(Attr::Flags(WireguardDeviceFlags::ReplacePeers));
        }
        attributes.push(Attr::Peers(Vec::new()));
        return Ok(vec![WireguardMessage {
            cmd: WireguardCmd::SetDevice,
            attributes,
        }]);
    }
    ...
    for peer in peers {
        ...
        loop {
            ...
            let base = message(ifindex, replace, attrs(peer, first, Vec::new()))
                .buffer_len()
                .checked_add(20)
                .ok_or(Error("message length overflow"))?;
            if base > budget {
                return Err(Error("budget cannot fit peer metadata"));
            }
            let mut length = base;
            let mut ips = Vec::new();
            ...
            if ips.is_empty() && pending.peek().is_some() {
                return Err(Error("budget cannot fit an AllowedIP with peer metadata"));
            }
