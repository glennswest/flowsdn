...
pub struct LocalRib {
    ...
    handles: std::collections::BTreeMap<Prefix, Arc<()>>,
}
impl LocalRib {
    pub fn advertise(&mut self, prefix: Prefix) -> PathHandle {
        self.prefixes.insert(prefix);
        let incarnation = self.handles.entry(prefix).or_default().clone();
        PathHandle {
    ...
    pub fn withdraw(&mut self, handle: &PathHandle) -> bool {
        if !Arc::ptr_eq(&self.issuer, &handle.issuer)
            || !self
                .handles
                .get(&handle.prefix)
                .is_some_and(|active| Arc::ptr_eq(active, &handle.incarnation))
        {
            return false;
        }
        self.handles.remove(&handle.prefix);
        self.prefixes.remove(&handle.prefix)
    }
...
impl ObservedRib {
    ...
    pub fn prefixes(&self, ipv6: bool) -> &BTreeSet<Prefix> {
        if ipv6 { &self.v6 } else { &self.v4 }
    }
    ...
    pub fn observe(&mut self, update: &Summary) -> bool {
        ...
        for prefix in update.announced.iter().chain(&update.mp_announced) {
            ...
            let store = if prefix.address().is_ipv6() {
                &mut self.v6
            } else {
                &mut self.v4
            };
        ...
        let report = overflow && !self.overflow_reported;
        self.overflow_reported |= overflow;
        report
    }
}
