...
fn barrier() -> Barrier {
    Barrier::new(
        ...
        9,
        100,
        BTreeMap::from([("NPDS".into(), 4), ("LDS".into(), 6)]),
    )
    .expect("barrier")
}
fn event(kind: &str, version: u64) -> Observation {
    Observation {
        node: node(),
        epoch: 0,
        kind: kind.into(),
...
fn both_acknowledgements_gate_the_commit_and_commit_is_one_shot() {
    ...
    gate.sent("NPDS", 5, 1).expect("newer response");
    gate.sent("LDS", 6, 1).expect("listener");
...
fn stale_nonce_wrong_node_and_pre_first_ack_resume_cannot_open_barrier() {
    ...
    assert_eq!(epoch, 1);
    gate.sent("NPDS", 4, 4).expect("resend");
    gate.sent("LDS", 6, 4).expect("send");
...
fn nack_timeout_and_cancellation_never_grant_publication() {
    for explicit in [false, true] {
        let mut gate = barrier();
        gate.sent("NPDS", 4, 0).expect("send");
        let nack = Observation {
            version: if explicit { 4 } else { 3 },
            nack: explicit,
            ..event("NPDS", 4)
        };
        assert_eq!(gate.observe(&nack, 1), Ok(true));
    ...
    let mut gate = barrier();
    gate.sent("NPDS", 4, 0).expect("send");
    assert_eq!(gate.observe(&event("NPDS", 4), 100), Err(Error::Terminal));
    assert_eq!(gate.state(), State::TimedOut);
    let mut gate = barrier();
    gate.cancel();
    assert_eq!(gate.commit(1), Err(Error::Terminal));
}
#[test]
fn a_superseding_response_revokes_readiness_until_its_ack() {
    let mut gate = barrier();
    for (kind, version) in [("NPDS", 4), ("LDS", 6)] {
        gate.sent(kind, version, 1).expect("send");
        gate.observe(&event(kind, version), 2).expect("ack");
    }
    assert_eq!(gate.state(), State::Ready);
    gate.sent("NPDS", 5, 3).expect("supersede");
    assert_eq!(gate.observe(&event("NPDS", 4), 4), Ok(false));
...
fn fqdn_restore_uses_shared_full_local_scope_without_partition() {
    for valid in [0x01000001, 0x0100ffff, 0x01010000, 0x01ffffff] {
        assert_eq!(local_identity(valid).expect("local").get(), valid);
    }
    for invalid in [
        0,
        1,
        ...
        assert!(local_identity(invalid).is_err());
    }
}
#[test]
fn explicit_dns_configuration_overrides_topology_default() {
    assert!(!transparent_dns(false, None));
    assert!(transparent_dns(true, None));
    assert!(!transparent_dns(true, Some(false)));
    assert!(transparent_dns(false, Some(true)));
}
