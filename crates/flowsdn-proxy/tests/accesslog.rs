use flowsdn_proxy::accesslog::{Reader, Read, DEFAULT_BUFFER_SIZE, MAX_BUFFER_SIZE};
use std::os::{unix::net::{UnixDatagram,UnixStream},fd::OwnedFd};
#[test]
fn header_heavy_binary_records_and_exact_boundary_survive() {
    ...
    for size in [0, 8192, DEFAULT_BUFFER_SIZE] {
        ...
        assert_eq!(reader.receive(|_|panic!("unexpected truncation")).expect("read"),Read::Record(data));
    }
...
fn oversize_is_dropped_once_then_next_datagram_remains_intact() {
    ...
    let result=reader.receive(|warning|warnings.push(warning)).expect("receive");
    ...
    assert_eq!(reader.receive(|_|panic!("unexpected truncation")).expect("next"),Read::Record(b"\0\xffnext".to_vec()));
}
#[test]
fn legacy_buffer_truncates_header_heavy_record_and_bounds_reject() {
    ...
    assert!(matches!(reader.receive(|_|{}).expect("receive"),Read::Truncated(_)));
...
fn seqpacket_truncation_and_peer_close() {
    use nix::sys::socket::{socketpair,AddressFamily,SockType,SockFlag,send,MsgFlags};
    use std::os::fd::AsRawFd;
    let (sender,receiver)=socketpair(AddressFamily::Unix,SockType::SeqPacket,None,SockFlag::SOCK_CLOEXEC).expect("seqpacket");
    let mut reader=Reader::new(receiver).expect("reader");
    send(sender.as_raw_fd(),&vec![42;20_000],MsgFlags::empty()).expect("send");
    assert!(matches!(reader.receive(|_|{}).expect("read"),Read::Truncated(_)));
    send(sender.as_raw_fd(),b"record",MsgFlags::empty()).expect("send");
    assert_eq!(reader.receive(|_|{}).expect("read"),Read::Record(b"record".to_vec()));
    drop(sender);
