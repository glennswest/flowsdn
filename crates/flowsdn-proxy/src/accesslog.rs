...
use nix::sys::socket::{MsgFlags, SockType, getsockname, getsockopt, recvmsg, sockopt};
use std::{
    io::{self, IoSliceMut},
...
impl Reader {
    ...
    pub fn with_capacity(socket: OwnedFd, capacity: usize) -> io::Result<Self> {
        if !(1..=MAX_BUFFER_SIZE).contains(&capacity) {
            return Err(io::Error::new(
                ...
                "envoy-access-log-buffer-size must be in 1..=1048576",
            ));
        }
        ...
        if !matches!(kind, SockType::Datagram | SockType::SeqPacket) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "access logs require a message socket",
            ));
        }
        Ok(Self {
            socket,
            ...
            dropped: 0,
            seqpacket: kind == SockType::SeqPacket,
        })
    }
    ...
    pub fn receive(&mut self, mut warn: impl FnMut(Truncation)) -> io::Result<Read> {
        ...
        if length == 0 && self.seqpacket {
            return Ok(Read::EmptyOrClosed);
        }
        let bytes = self
            .buffer
            .get(..length)
            .ok_or_else(|| io::Error::other("invalid access-log receive length"))?;
