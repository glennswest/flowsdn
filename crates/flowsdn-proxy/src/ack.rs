...
impl Barrier {
    pub fn new(
        ...
        if required.is_empty()
            || required
                .iter()
                .any(|(kind, version)| kind.is_empty() || *version == 0)
        {
    ...
    fn live(&mut self, now: u64) -> Result<(), Error> {
        if !matches!(self.state, State::Pending | State::Ready) {
    ...
    pub fn sent(&mut self, kind: &str, version: u64, now: u64) -> Result<(), Error> {
        ...
        if version < *minimum || self.sent.get(kind).is_some_and(|old| version <= *old) {
            return Err(Error::InvalidResponse);
        }
        self.sent.insert(kind.into(), version);
        self.acked.remove(kind);
    ...
    pub fn observe(&mut self, event: &Observation, now: u64) -> Result<bool, Error> {
        ...
        if node != self.node || epoch != self.epoch || self.sent.get(kind) != Some(&nonce) {
            return Ok(false);
        }
        if version > nonce {
            return Err(Error::InvalidResponse);
        ...
        if self.acked.len() == self.required.len() {
