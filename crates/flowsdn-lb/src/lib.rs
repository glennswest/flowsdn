...
pub fn ordered_pools(mut pools: Vec<PoolOrder>) -> Result<Vec<PoolOrder>, &'static str> {
    if pools
        .iter()
        .any(|p| p.name.is_empty() || p.created_nanos >= 1_000_000_000)
    {
        return Err("invalid pool metadata");
    }
    let mut names = std::collections::BTreeSet::new();
    if pools.iter().any(|p| !names.insert(p.name.clone())) {
        return Err("duplicate pool name");
    }
    pools.sort();
    Ok(pools)
}
...
fn yaml_value(v: &yaml_rust2::Yaml) -> Result<Value, String> {
    ...
    Ok(match v {
        ...
        Yaml::Real(v) => {
            serde_json::from_str(v).map_err(|e| format!("invalid YAML number: {e}"))?
        }
        ...
        Yaml::Hash(m) => {
            ...
            for (k, v) in m {
                out.insert(
                    k.as_str().ok_or("non-string YAML key")?.to_owned(),
                    yaml_value(v)?,
                );
            }
            Value::Object(out)
...
fn objects(root: &Value, field: &str) -> Result<BTreeMap<(String, String), Value>, String> {
    ...
    let Some(array) = root.get(field).filter(|v| !v.is_null()) else {
        return Ok(result);
    };
    for object in array.as_array().ok_or("expected object array")? {
        ...
        let namespace = match metadata.get("namespace") {
            ...
            Some(value) => value.as_str().ok_or("invalid namespace")?,
        };
        if result
            .insert((namespace.to_owned(), name.to_owned()), object.clone())
            .is_some()
        {
            return Err("duplicate object".into());
        }
    }
    Ok(result)
}
impl LocalSnapshot {
    ...
    pub fn parse(text: &str) -> Result<Self, String> {
        ...
        if docs.len() != 1 {
            return Err("expected one state document".into());
        }
        let root = yaml_value(docs.first().ok_or("missing document")?)?;
        if !root.is_object() {
            return Err("expected state object".into());
        }
        Ok(Self {
            services: objects(&root, "services")?,
            endpoints: objects(&root, "endpoints")?,
        })
    }
...
mod tests {
    ...
    fn pool_order_is_total_and_rejects_ambiguous_input() -> Result<(), &'static str> {
        let pool = |name: &str, created_seconds| PoolOrder {
            name: name.into(),
            created_seconds,
            created_nanos: 0,
        };
        ...
        assert_eq!(
            ordered_pools(vec![pool("z", 2), pool("old", 1), pool("a", 2)])?,
            expected
        );
    ...
    fn termination_never_destroys_unowned_or_live_sockets() {
        ...
        for (owned, alive) in [(false, false), (false, true), (true, true)] {
            assert!(!t.eligible(true, Protocol::Udp, owned, alive));
        }
        ...
        assert!(
            ...
            .eligible(false, Protocol::Udp, true, false)
        );
        assert!(
            Termination {
                all_protocols: true,
            ...
            .eligible(true, Protocol::Tcp, true, false)
        );
        assert!(
            !Termination {
            ...
            .eligible(true, Protocol::Udp, true, false)
        );
    }
    #[test]
    fn reflector_replacement_is_atomic_and_roundtrips() -> Result<(), String> {
        let mut state = LocalSnapshot::parse(
    ...
    fn malformed_identity_and_duplicate_objects_rejected() {
        for s in [
            "services: [null]",
            "services: [{metadata: {name: a, namespace: true}}]",
            "services: [{metadata: {name: a}}, {metadata: {name: a}}]",
            "---\n{}\n---\n{}",
            "services: true",
        ] {
