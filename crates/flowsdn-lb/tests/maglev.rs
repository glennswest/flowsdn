...
fn weighted_removal() {
    ...
    assert!(before.iter().zip(&after).filter(|(a,b)|**a!=1&&a!=b).count()<11);
    ...
    for count in [0,1,2,5,111,222,333,1001] {
        ...
        for size in [251usize,509,1021] {
            ...
            for backend in &backends {
                ...
                let skip=h2.checked_rem(m.saturating_sub(1)).expect("nonzero").saturating_add(1);
                for _ in 0..size {
                    expected.push(usize::try_from(position).expect("slot"));
                    position=position.saturating_add(skip).checked_rem(m).expect("nonzero");
                }
            }
            ...
            for chunks in [1,2,3,4,8,100] {
                ...
                let actual:Vec<_>=backends.chunks(chunk_size).flat_map(|chunk|chunk.iter().flat_map(|backend|permutation(backend,size,0x24b7ef82).expect("permutation"))).collect();
                assert_eq!(actual,expected,"count={count},size={size},chunks={chunks}");
            }
        }
    }
}
