...
fn aggregate_auth_explicit_disable_and_wildcard_fallback() {
    ...
    let mut records = vec![
        ...
        record(
            flowsdn_identity::numeric::ReservedIdentity::AggregateCluster as u32,
            6,
            80,
            16,
            0x101,
...
fn canonical_records_reject_unsupported_identity_scopes_and_redirect_rank_mismatch() {
    ...
    for id in [
        0, 1, 11, 12, 13, 14, 1000, 0x01000001, 0x02000001, 0x00ffffff,
    ] {
    ...
    for rank in [2, 129, 254] {
        ...
        assert_eq!(
            index.lookup(context(), packet(1000, 80)),
            Ok(Outcome::Allow {
                proxy_port: 15000,
                authentication: 0,
                cookie: 0
