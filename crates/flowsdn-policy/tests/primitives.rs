use flowsdn_policy::{Error, OverflowAction, pressure, ports::PortRange, cidr::{CidrRule, PrefixUpdate},
    named_ports::{NamedPortState, Contribution, Key}, oracle::{Rule, Tier, Verdict, Peer, Authentication},
    repository::SubjectRepository};
use std::collections::BTreeSet;

fn allow() -> Rule {
    Rule { tier: Tier::Normal, priority: 0.0, verdict: Verdict::Allow, egress: true,
        peer: Peer::Identity(123), protocol: 6, ports: PortRange::from_api(443, None).expect("port"),
        authentication: None, proxy_port: 0, listener_priority: 0 }
}
#[test]
fn import_rejects_cross_resource_pass_auth_without_replacing_old_policy() {
    let mut repository = SubjectRepository::default();
    let authenticated = Rule { authentication: Some(Authentication::Required), ..allow() };
    repository.replace("cnp/a", vec![authenticated]).expect("auth policy");
    repository.replace("kcnp/b", vec![allow()]).expect("old rule");
    let before = repository.clone();
    let pass = Rule { tier: Tier::Admin, verdict: Verdict::Pass, ..allow() };
    assert_eq!(repository.replace("kcnp/b", vec![pass.clone()]), Err(Error::PassWithAuthentication));
    assert_eq!(repository, before);
    repository.replace("cnp/a", vec![]).expect("remove auth");
    repository.replace("kcnp/b", vec![pass]).expect("pass accepted after auth gone");
    let before = repository.clone();
    assert_eq!(repository.replace("cnp/c", vec![Rule { authentication: Some(Authentication::Disabled), ..allow() }]),
...
fn rejected_numeric_and_protocol_inputs_preserve_repository_revision() {
    ...
    for priority in [f64::NAN, f64::INFINITY, -1.0] {
        assert_eq!(repository.replace("cnp/a", vec![Rule { priority, ..allow() }]), Err(Error::InvalidPriority));
        assert_eq!(repository, original);
    }
    assert_eq!(repository.replace("cnp/a", vec![Rule { protocol: 0, ..allow() }]), Err(Error::InvalidProtocol));
    assert_eq!(repository, original);
}
#[test]
...
fn named_port_deletion_preserves_other_selector_and_restores_shadowed_precedence() {
    ...
    let key = Key { identity: 123, protocol: 6, port: 443 };
    state.replace("a", vec![Contribution { key, precedence: 10 }]).expect("a");
    state.replace("b", vec![Contribution { key, precedence: 20 }]).expect("b");
...
fn named_port_reresolution_removes_old_key_and_keeps_unrelated_identity() {
    let mut state = NamedPortState::default();
    let old = Key { identity: 123, protocol: 6, port: 80 };
    let new = Key { port: 8080, ..old };
    let other = Key { identity: 456, ..old };
    state.replace("a", vec![Contribution { key: old, precedence: 5 }]).expect("old");
    state.replace("b", vec![Contribution { key: other, precedence: 7 }]).expect("other");
    let change = state.replace("a", vec![Contribution { key: new, precedence: 5 }]).expect("new");
    assert_eq!(change.deletes, vec![old]);
    assert_eq!(change.upserts.get(&new), Some(&5));
    assert_eq!(state.desired().get(&other), Some(&7));
    let original = state.clone();
    assert!(state.replace("a", vec![Contribution { key: Key { protocol: 0, ..new }, precedence: 2 }]).is_err());
...
fn cidr_exception_validation_preserves_family_and_containment() {
    ...
    assert_eq!(CidrRule::new(v6, [inner]).expect("valid").import_prefixes().len(), 2);
    for invalid in ["2001:db9::/32", "0.0.0.0/0", "2001::/16"] {
        assert_eq!(CidrRule::new(v6, [invalid.parse().expect("prefix")]), Err(Error::InvalidException));
    }
}
#[test]
fn api_range_regression_seeds_reject_zero_start_without_wildcarding() {
    for line in include_str!("corpus/port-ranges.tsv").lines().filter(|s| !s.starts_with('#')) {
        ...
        assert_eq!(PortRange::from_api(start, Some(end)).is_ok(), accepted, "{line}");
    }
    assert!(PortRange::from_api(0, None).expect("wildcard without endPort").is_any());
}
#[test]
fn port_blocks_cover_exactly_the_inclusive_input_without_overlap() {
    for (start, end) in [(1,65535),(80,80),(1024,2047),(1001,9876),(65535,65535)] {
...
fn pressure_alarm_is_exact_at_ninety_percent_and_lockdown_is_explicit() {
    ...
    assert_eq!(pressure(1001,1000,false).expect("compatibility").action, OverflowAction::Reconcile);
    assert_eq!(pressure(1001,1000,true).expect("lockdown").action, OverflowAction::Lockdown);
    assert!(pressure(u64::MAX,65536,true).expect("large desired").alarm);
    for capacity in [0,255,65537,u32::MAX] { assert_eq!(pressure(0,capacity,true), Err(Error::InvalidCapacity)); }
}
