...
fn compare_all_ports(rules: &[Rule], identity: u32, egress: bool, protocol: u8) {
    ...
    for port in 0..=u16::MAX {
        ...
        assert_eq!(
            ...
            evaluate(rules, packet).expect("oracle"),
...
fn exhaustive_port_gate_covers_pass_deny_redirect_and_unknown_identity() {
    let rules = vec![
        Rule {
            ...
            protocol: 6,
            ports: PortRange::from_api(80, Some(95)).expect("range"),
            ..base()
        },
        Rule {
            tier: Tier::Admin,
        ...
        Rule {
            ...
            protocol: 6,
            ports: PortRange::from_api(85, Some(90)).expect("range"),
            proxy_port: 15000,
            listener_priority: 1,
            ..base()
        },
    ...
    for (identity, egress, protocol) in [
        (123, true, 6),
        (124, true, 6),
        (123, true, 17),
        (123, false, 6),
    ] {
        compare_all_ports(&rules, identity, egress, protocol);
    }
}
...
fn generated(seed: u64) -> Vec<Rule> {
    ...
    (0..10)
        .map(|_| {
            let mut rule = base();
            rule.tier = match random(&mut state) % 4 {
                0 => Tier::Admin,
                1 => Tier::Normal,
                2 => Tier::Baseline,
                _ => Tier::Default,
            ...
            rule.verdict = match random(&mut state) % 3 {
                0 => Verdict::Deny,
                1 => Verdict::Pass,
                _ => Verdict::Allow,
            };
            rule.egress = random(&mut state) & 1 != 0;
            rule.peer = match random(&mut state) % 3 {
                0 => Peer::Any,
                1 => Peer::Identity(123),
                _ => Peer::Identity(456),
            };
            rule.protocol = match random(&mut state) % 3 {
                0 => 0,
                1 => 6,
                _ => 17,
            };
            if rule.protocol != 0 {
                rule.ports = match random(&mut state) % 5 {
                    0 => PortRange::any(),
                    1 => PortRange::from_api(80, Some(95)).expect("range"),
                    2 => PortRange::from_api(90, Some(443)).expect("range"),
                    3 => PortRange::from_api(1024, Some(2047)).expect("range"),
                    _ => PortRange::from_api(65535, None).expect("port"),
                };
            }
            if rule.verdict == Verdict::Allow && random(&mut state) & 1 != 0 {
                rule.proxy_port = if random(&mut state) & 1 != 0 {
                    15000
                } else {
                    15001
                };
                rule.listener_priority = u8::try_from(random(&mut state) % 3).expect("small");
            }
            rule
        })
        .collect()
...
fn deterministic_generated_policies_match_independent_oracle_and_update_order() {
    ...
    let ports = [
        0, 1, 79, 80, 81, 89, 90, 91, 94, 95, 96, 442, 443, 444, 1023, 1024, 1025, 2046, 2047,
        2048, 65534, 65535,
    ];
    for seed in 0..192 {
...
fn coalescing_is_semantic_and_invalid_replacement_preserves_published_index() {
    ...
    assert_eq!(
        compiled.replace(&[auth]),
        Err(Error::OracleAuthenticationUnsupported)
    );
    assert_eq!(
        compiled.lookup(packet),
        evaluate(&[plain], packet).expect("oracle")
    );
    compiled.replace(&[]).expect("delete");
    assert_eq!(compiled.lookup(packet), Decision::Deny);
}
#[test]
