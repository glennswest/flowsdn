...
#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Changes {
    pub upserts: BTreeMap<Key, u32>,
    pub deletes: Vec<Key>,
}
impl NamedPortState {
    pub fn desired(&self) -> &BTreeMap<Key, u32> {
        &self.desired
    }
    ...
    pub fn replace(
        ...
        if contributions
            .iter()
            .any(|c| c.key.protocol == 0 || c.key.port == 0 || c.key.identity == 0)
        {
        ...
        if contributions.is_empty() {
            ...
            self.selectors.insert(selector.into(), contributions);
        }
        ...
        let upserts = desired
            ...
            .filter_map(|(key, value)| {
                (self.desired.get(key) != Some(value)).then_some((*key, *value))
            })
            .collect();
        let deletes = self
            .desired
            .keys()
            .filter_map(|key| (!desired.contains_key(key)).then_some(*key))
            .collect();
