...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            Self::PassWithAuthentication => "Pass and authentication cannot coexist for one subject",
            ...
            Self::InvalidRedirect => "listener priority requires a redirect port and must be at most 126",
            ...
            Self::OracleAuthenticationUnsupported => "oracle does not yet evaluate authentication inheritance",
        })
    }
}
...
pub fn pressure(desired: u64, capacity: u32, lockdown: bool) -> Result<Pressure, Error> {
    if !(256..=65536).contains(&capacity) { return Err(Error::InvalidCapacity); }
    ...
    Ok(Pressure { alarm: desired >= threshold, overflow,
        action: if overflow && lockdown { OverflowAction::Lockdown } else { OverflowAction::Reconcile } })
}
