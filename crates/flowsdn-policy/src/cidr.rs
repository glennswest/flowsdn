...
pub struct CidrRule {
    allow: CidrPrefix,
    except: BTreeSet<CidrPrefix>,
}
impl CidrRule {
    pub fn new(
        allow: CidrPrefix,
        except: impl IntoIterator<Item = CidrPrefix>,
    ) -> Result<Self, Error> {
        let except: BTreeSet<_> = except.into_iter().collect();
        if except.iter().any(|prefix| !allow.contains_prefix(*prefix)) {
    ...
    pub fn selects(&self, peer: CidrPrefix) -> bool {
        self.allow.contains_prefix(peer)
            && !self
                .except
                .iter()
                .any(|prefix| prefix.contains_prefix(peer))
    }
}
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl PrefixUpdate {
    pub fn between(old: &[CidrRule], new: &[CidrRule]) -> Self {
        ...
        Self {
            allocate_before_publish: new.difference(&old).copied().collect(),
