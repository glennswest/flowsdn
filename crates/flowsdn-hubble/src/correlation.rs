...
pub fn correlate(
    snapshot: &impl PolicySnapshot,
    endpoint_id: u16,
    key: Key,
    verdict: Verdict,
) -> Option<Correlation> {
    if endpoint_id == 0 || key.protocol == 0 || key.destination_port == 0 {
        return None;
    }
    let allowed = match verdict {
        ...
        Verdict::Audit
        | Verdict::Dropped {
            reason: DROP_POLICY_DENIED | DROP_POLICY_DENY,
        } => false,
        _ => return None,
    };
    let policy = snapshot.correlation_info(endpoint_id, key)?;
    let policy_log = policy
        .rules
        .iter()
        .filter_map(|r| r.log.clone())
        .collect::<BTreeSet<_>>()
        .into_iter()
        .collect();
