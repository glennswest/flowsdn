...
impl fmt::Display for DecodeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "invalid monitor drop sample: {self:?}")
    }
}
...
fn read<const N: usize>(sample: &[u8], offset: usize) -> Result<[u8; N], DecodeError> {
    sample
        .get(offset..offset.checked_add(N).ok_or(DecodeError::Truncated)?)
...
impl<'a> DropNotification<'a> {
    pub fn decode(sample: &'a [u8]) -> Result<Self, DecodeError> {
        ...
        if *sample.get(15).ok_or(DecodeError::Truncated)? != 0 {
        ...
        if u32::from(length) > original_length {
            return Err(DecodeError::InvalidCaptureLength);
        }
        let end = offset
        ...
        Ok(Self {
            ...
            flags: if version >= 2 {
                *sample.get(36).ok_or(DecodeError::Truncated)?
            } else {
                0
            },
            ip_trace_id: if version >= 3 {
                u64::from_le_bytes(read(sample, 40)?)
            } else {
                0
            },
    ...
    pub const fn flow_interface(&self) -> Option<Interface> {
        None
    }
}
