...
#[derive(Clone, Debug, Eq, PartialEq)]
enum IpMatch { Address(IpAddr), Prefix { network: IpAddr, bits: u8 } }
    ...
    pub fn compile(entries: &[impl AsRef<str>]) -> Result<Self, InvalidArgument> {
        ...
        for entry in entries {
            ...
            let rule = if let Some((address, prefix)) = raw.split_once('/') {
                ...
                if bits > if address.is_ipv4() { 32 } else { 128 } { return Err(invalid()); }
                IpMatch::Prefix { network: address, bits }
            } else { IpMatch::Address(raw.parse().map_err(|_| invalid())?) };
            output.push(rule);
        }
        Ok(Self(output))
    }
    pub fn matches(&self, rendered: &str) -> bool {
        let Ok(address) = rendered.parse::<IpAddr>() else { return false; };
        self.0.iter().any(|rule| match rule {
            ...
            IpMatch::Prefix { network, bits } => match (*network, address) {
                (IpAddr::V4(a), IpAddr::V4(b)) => {
                    ...
                    u32::from(a).checked_shr(shift).unwrap_or(0) == u32::from(b).checked_shr(shift).unwrap_or(0)
                }
                (IpAddr::V6(a), IpAddr::V6(b)) => {
                    let shift = 128u32.saturating_sub(u32::from(*bits));
                    u128::from(a).checked_shr(shift).unwrap_or(0) == u128::from(b).checked_shr(shift).unwrap_or(0)
                }
                _ => false,
            },
        })
...
impl FlowFilter {
    pub fn matches(&self, addresses: Addresses<'_>) -> bool {
        self.source.as_ref().is_none_or(|f| f.matches(addresses.source))
            && self.destination.as_ref().is_none_or(|f| f.matches(addresses.destination))
            && self.translated_source.as_ref().is_none_or(|f| f.matches(addresses.translated_source))
    }
}
#[derive(Clone, Debug, Default)]
...
impl FilterSet {
    ...
    pub fn matches(&self, event: Option<Addresses<'_>>) -> bool {
        let Some(addresses) = event else { return true; };
...
pub fn validate_cel(expressions: &[impl AsRef<str>]) -> Result<(), InvalidArgument> {
    if expressions.is_empty() { Ok(()) } else {
        Err(InvalidArgument("unsupported filter: experimental.cel_expression".into()))
    }
}
