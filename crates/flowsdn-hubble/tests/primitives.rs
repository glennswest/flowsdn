...
use flowsdn_hubble::{EMITTER_NAME, EMITTER_VERSION, address_preference};
use std::sync::Arc;
...
fn address_spelling_prefix_boundaries_and_families() {
    ...
    for ip in ["192.0.2.128", "192.0.2.255", "2001:0db8:0007:ffff::1"] {
        assert!(prefix.matches(ip), "{ip}");
    }
    for ip in ["192.0.2.127", "192.0.3.0", "2001:db8:8::1"] {
        assert!(!prefix.matches(ip), "{ip}");
    }
    ...
    assert!(
        IpSet::compile(&["192.0.2.1/32"])
            .expect("host")
            .matches("192.0.2.1")
    );
    assert!(
        IpSet::compile(&["2001:db8::1/128"])
            .expect("host")
            .matches("2001:0db8::1")
    );
    assert!(
        !IpSet::compile(&["::ffff:192.0.2.1"])
            .expect("mapped")
            .matches("192.0.2.1")
    );
    for bad in [
...
fn filtering_has_and_or_deny_and_unfilterable_loss() {
    ...
    assert!(!set.matches(Some(Addresses {
...
fn cel_rejected_before_stream_and_preference_preserves_explicit_false() {
    ...
    for expression in ["", "true", "flow.IP.source == '192.0.2.1'"] {
        assert!(
            validate_cel(&[expression])
                .expect_err("unsupported")
                .to_string()
                .contains("experimental.cel_expression")
        );
    }
...
fn drop_sample(version: u8) -> Vec<u8> {
    ...
    put(&mut data, 0, [1, 133]);
    put(&mut data, 2, 257u16.to_le_bytes());
    put(&mut data, 8, 100u32.to_le_bytes());
    put(&mut data, 12, 3u16.to_le_bytes());
    put(&mut data, 14, [version]);
    put(&mut data, 32, 0x01020304u32.to_le_bytes());
    if version >= 2 {
        put(&mut data, 36, [0x03]);
    }
    if version >= 3 {
        put(&mut data, 40, 0x0102030405060708u64.to_le_bytes());
    }
...
fn versioned_drop_decodes_raw_interface_but_preserves_reference_projection() {
    for version in 0..=3 {
        ...
        assert_eq!(
            drop.ip_trace_id,
            if version >= 3 { 0x0102030405060708 } else { 0 }
        );
    ...
    put(&mut sample, 14, [4]);
    assert_eq!(
        DropNotification::decode(&sample),
        Err(DecodeError::UnknownVersion)
    );
    put(&mut sample, 14, [3, 1]);
    assert_eq!(
        DropNotification::decode(&sample),
        Err(DecodeError::UnknownExtension)
    );
    put(&mut sample, 15, [0]);
    put(&mut sample, 12, 101u16.to_le_bytes());
    assert_eq!(
        DropNotification::decode(&sample),
        Err(DecodeError::InvalidCaptureLength)
    );
    put(&mut sample, 0, [4]);
    assert_eq!(
        DropNotification::decode(&sample),
...
fn ring_is_bounded_reserves_newest_and_reports_each_overwritten_position() {
    for invalid in [0, 2, 4, 65536, u32::MAX] {
        assert!(MemoryRing::<u8>::new(invalid).is_err());
    }
    for valid in [1, 3, 4095, 65535] {
        assert!(MemoryRing::<u8>::new(valid).is_ok());
    }
    let mut ring = MemoryRing::new(3).expect("capacity");
    let mut cursor = 0;
    assert_eq!(ring.read_next(&mut cursor), Read::End);
    assert_eq!(cursor, 0);
    ring.push(10);
    assert_eq!(ring.read(0), Read::End);
    ring.push(20);
    assert_eq!(ring.read_next(&mut cursor), Read::Event(Arc::new(10)));
    assert_eq!(cursor, 1);
    assert_eq!(ring.read_next(&mut cursor), Read::End);
    for value in 30..=35 {
        ring.push(value);
    ...
    for expected_cursor in 2..=4 {
        assert_eq!(ring.read_next(&mut cursor), Read::Lost { count: 1 });
    ...
    assert_eq!(restarted.next_sequence(), 0);
    assert_eq!(restarted.read(0), Read::End);
}
