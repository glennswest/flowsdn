...
impl<T> Index<T> {
    pub fn new(name: &'static str, unique: bool, keys: impl Fn(&T) -> Vec<Key> + Send + Sync + 'static) -> Self {
        Self { name, unique, keys: Arc::new(keys) }
    }
}
...
impl fmt::Display for TableError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UniqueViolation { index, key } => write!(f, "unique index {index} collision on {key:?}"),
            Self::RevisionMismatch { current } => write!(f, "revision mismatch (current: {current:?})"),
...
impl<T: Keyed> Table<T> {
    ...
    pub fn new(indexes: Vec<Index<T>>) -> Result<Self, TableError> {
        ...
        let version = Version {
            revision: 0, rows: OrdMap::new(), revisions: OrdMap::new(),
            indexes: indexes.iter().map(|_| OrdMap::new()).collect(),
        };
        Ok(Self { indexes: Arc::new(indexes), published: ArcSwap::from_pointee(version), writer: Mutex::new(()) })
    }

    /// O(1); no writer lock is taken. A retained snapshot never changes.
    pub fn snapshot(&self) -> Snapshot<T> {
        Snapshot { version: self.published.load_full(), indexes: self.indexes.clone() }
    }
    ...
    pub async fn batch<R>(&self, apply: impl FnOnce(&mut Write<'_, T>) -> R) -> R {
        ...
        let result = apply(&mut Write { version: &mut version, indexes: &self.indexes });
    ...
    pub async fn insert_if_revision(&self, row: T, expected: Revision) -> Result<WriteResult<T>, TableError> {
        self.batch(|write| write.insert_if_revision(row, expected)).await
    }
    pub async fn delete(&self, key: &[u8]) -> Result<WriteResult<T>, TableError> {
        self.batch(|write| write.delete(key)).await
    }
    pub async fn delete_if_revision(&self, key: &[u8], expected: Revision) -> Result<WriteResult<T>, TableError> {
        self.batch(|write| write.delete_if_revision(key, expected)).await
    }
    pub async fn modify(&self, key: &[u8], modify: impl FnOnce(Option<&T>) -> Option<T>) -> Result<WriteResult<T>, TableError> {
...
impl<T: Keyed> Write<'_, T> {
    fn require_revision(&self, key: &[u8], expected: Revision) -> Result<(), TableError> {
        ...
        if current != Some(expected) { return Err(TableError::RevisionMismatch { current }); }
        Ok(())
    }

    fn next_revision(&self) -> Result<Revision, TableError> {
        self.version.revision.checked_add(1).ok_or(TableError::RevisionExhausted)
    }
    ...
    fn remove_indexes(&mut self, key: &Key, row: &T, revision: Revision) {
        ...
        for (index, map) in self.indexes.iter().zip(&mut self.version.indexes) {
            for secondary in (index.keys)(row) { map.remove(&(secondary, key.clone())); }
        }
    }
    ...
    pub fn insert(&mut self, row: T) -> Result<WriteResult<T>, TableError> {
        ...
        let keys: Vec<BTreeSet<Key>> = self.indexes.iter().map(|index| (index.keys)(&row).into_iter().collect()).collect();
        ...
        for ((index, map), keys) in self.indexes.iter().zip(&self.version.indexes).zip(&keys) {
            if index.unique {
                for secondary in keys {
                    if let Some(((found, primary), _)) = map.range((secondary.clone(), Key::new())..).next() {
                        if found == secondary && primary != &key {
                            return Err(TableError::UniqueViolation { index: index.name, key: secondary.clone() });
                        }
                    }
                }
            }
        ...
        if let Some((old, old_revision)) = &previous { self.remove_indexes(&key, old, *old_revision); }
        for (map, keys) in self.version.indexes.iter_mut().zip(keys) {
            for secondary in keys { map.insert((secondary, key.clone()), ()); }
        }
        self.version.rows.insert(key.clone(), (Arc::new(row), revision));
        self.version.revisions.insert(revision, key);
        self.version.revision = revision;
        Ok(WriteResult { previous: previous.map(|(row, _)| row), revision })
    }

    pub fn insert_if_revision(&mut self, row: T, expected: Revision) -> Result<WriteResult<T>, TableError> {
    ...
    pub fn delete(&mut self, key: &[u8]) -> Result<WriteResult<T>, TableError> {
        ...
        if let Some((old, old_revision)) = &previous { self.remove_indexes(&key.to_vec(), old, *old_revision); }
        self.version.revision = revision;
        Ok(WriteResult { previous: previous.map(|(row, _)| row), revision })
    }

    pub fn delete_if_revision(&mut self, key: &[u8], expected: Revision) -> Result<WriteResult<T>, TableError> {
        self.require_revision(key, expected)?;
        self.delete(key)
    }

    pub fn modify(&mut self, key: &[u8], modify: impl FnOnce(Option<&T>) -> Option<T>) -> Result<WriteResult<T>, TableError> {
        let current = self.version.rows.get(key);
        match modify(current.map(|(row, _)| row.as_ref())) {
            Some(row) => {
                if row.primary_key() != key { return Err(TableError::PrimaryKeyChanged); }
                self.insert(row)
            }
            None if current.is_some() => self.delete(key),
            None => Ok(WriteResult { previous: None, revision: self.version.revision }),
        }
    }
    ...
    pub fn delete_all(&mut self) -> Result<usize, TableError> {
        ...
        self.version.revision.checked_add(count).ok_or(TableError::RevisionExhausted)?;
...
impl<T: Keyed> Snapshot<T> {
    pub fn len(&self) -> usize { self.version.rows.len() }
    pub fn is_empty(&self) -> bool { self.version.rows.is_empty() }
    pub fn revision(&self) -> Revision { self.version.revision }
    pub fn all(&self) -> impl Iterator<Item = Row<T>> + '_ { self.version.rows.values().cloned() }
    pub fn by_revision(&self, from: Revision) -> impl Iterator<Item = Row<T>> + '_ {
        self.version.revisions.range(from..).filter_map(|(_, key)| self.version.rows.get(key).cloned())
    }
    fn index(&self, name: &str) -> Result<(&Index<T>, &IndexMap), TableError> {
        self.indexes.iter().zip(&self.version.indexes).find(|(index, _)| index.name == name)
            .ok_or_else(|| TableError::UnknownIndex(name.to_owned()))
    }
    pub fn get(&self, index: &str, key: &[u8]) -> Result<Option<Row<T>>, TableError> {
        if index == "primary" { return Ok(self.version.rows.get(key).cloned()); }
        let (definition, _) = self.index(index)?;
        if !definition.unique { return Err(TableError::NonUniqueIndex(index.to_owned())); }
        Ok(self.list(index, key)?.into_iter().next())
    }
    pub fn list(&self, index: &str, key: &[u8]) -> Result<Vec<Row<T>>, TableError> {
        if index == "primary" { return Ok(self.version.rows.get(key).cloned().into_iter().collect()); }
        let (_, map) = self.index(index)?;
        Ok(map.range((key.to_vec(), Key::new())..).take_while(|((secondary, _), _)| secondary.as_slice() == key)
            .filter_map(|((_, primary), _)| self.version.rows.get(primary).cloned()).collect())
    }
    pub fn prefix(&self, index: &str, prefix: &[u8]) -> Result<Vec<Row<T>>, TableError> {
        if index == "primary" {
            return Ok(self.version.rows.range(prefix.to_vec()..).take_while(|(key, _)| key.starts_with(prefix)).map(|(_, row)| row.clone()).collect());
        }
        let (_, map) = self.index(index)?;
        Ok(map.range((prefix.to_vec(), Key::new())..).take_while(|((key, _), _)| key.starts_with(prefix))
            .filter_map(|((_, primary), _)| self.version.rows.get(primary).cloned()).collect())
    }
    pub fn lower_bound(&self, index: &str, key: &[u8]) -> Result<Vec<Row<T>>, TableError> {
        if index == "primary" { return Ok(self.version.rows.range(key.to_vec()..).map(|(_, row)| row.clone()).collect()); }
        let (_, map) = self.index(index)?;
        Ok(map.range((key.to_vec(), Key::new())..).filter_map(|((_, primary), _)| self.version.rows.get(primary).cloned()).collect())
    }
}

/// Order-preserving encodings used by the primary and secondary indexes.
pub mod key {
    pub fn u32be(value: u32) -> Vec<u8> { value.to_be_bytes().to_vec() }
    pub fn u64be(value: u64) -> Vec<u8> { value.to_be_bytes().to_vec() }
