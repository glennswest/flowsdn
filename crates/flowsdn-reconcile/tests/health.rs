...
use flowsdn_reconcile::{BatchResult, BatchUpdate, DriverState, Options, ReconcileError, Reconciler, Target};
...
#[derive(Default)]
struct State { fail: Option<u8>, block: Option<u8>, fail_prune: bool, batch: bool, malformed: bool }
...
impl Fake {
    async fn apply(&mut self, id: u8) -> Result<(), &'static str> {
        ...
        if fail { Err("target rejected row") } else { Ok(()) }
    }
}
impl Target<Item> for Fake {
    ...
    async fn prune(&mut self, _: Snapshot<Item>) -> Result<(), Self::Error> {
        if std::mem::take(&mut self.0.lock().unwrap().fail_prune) { Err("prune failed") } else { Ok(()) }
    }
...
fn status(registry: &Registry) -> HealthStatus { registry.snapshot().get("primary", b"reconcile").unwrap().unwrap().0.as_ref().clone() }
struct NoopWake;
...
async fn optional_reporter_leaves_registry_untouched_and_success_counts_desired_objects() {
    ...
    assert_eq!((health.level, health.message.as_str()), (Level::Ok, "2 objects"));
...
async fn retry_recovery_and_cancelled_replay_preserve_failure_diagnostics() {
    for batch in [false, true] {
        ...
        { let mut state = target.0.lock().unwrap(); state.fail = Some(1); state.batch = batch; }
        let control = target.clone();
        let mut reconciler = Reconciler::new(&table, target, options()).unwrap().with_reporter(registry.reporter("reconcile").unwrap());
        reconciler.run_round(now()).await.unwrap();
        let health = status(&registry);
        assert_eq!((health.level, health.message.as_str()), (Level::Degraded, "1 errors"));
...
async fn prune_and_row_failures_are_joined_and_clear_independently() {
    ...
    let registry = Registry::new();
    let target = Fake::default();
    { let mut state = target.0.lock().unwrap(); state.fail = Some(1); state.fail_prune = true; }
    let mut reconciler = Reconciler::new(&table, target, options()).unwrap().with_reporter(registry.reporter("reconcile").unwrap());
    reconciler.run_round(now()).await.unwrap();
    let health = status(&registry);
...
async fn resync_degrades_until_initialization_allows_prune() {
    let table = Table::with_stream_options(vec![], flowsdn_table::StreamOptions { tombstone_max_count: 1, ..flowsdn_table::StreamOptions::default() }).unwrap();
    let registry = Registry::new();
    let mut reconciler = Reconciler::new(&table, Fake::default(), options()).unwrap().with_reporter(registry.reporter("reconcile").unwrap());
...
async fn joined_diagnostics_are_bounded_without_losing_error_count() {
    ...
    impl Target<Item> for LargeError {
        type Error = String;
        async fn update(&mut self, _: Arc<Item>) -> Result<(), Self::Error> { Err("é".repeat(40_000)) }
    ...
    let registry = Registry::new();
    let mut reconciler = Reconciler::new(&table, LargeError, options()).unwrap().with_reporter(registry.reporter("reconcile").unwrap());
    reconciler.run_round(now()).await.unwrap();
    let health = status(&registry);
...
async fn healthy_round_does_not_publish_health_per_row() {
    let table = Table::new(vec![]).unwrap();
    for id in 1..=100 { table.insert(Item(id)).await.unwrap(); }
    let registry = Registry::new();
    let mut reconciler = Reconciler::new(&table, Fake::default(), options()).unwrap().with_reporter(registry.reporter("reconcile").unwrap());
    reconciler.run_round(now()).await.unwrap();
    let health = status(&registry);
