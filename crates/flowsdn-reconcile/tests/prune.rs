...
use std::{collections::BTreeMap, future::Future, sync::{Arc, Mutex}, task::{Context, Wake, Waker}, time::{Duration, Instant}};
...
impl Target<Item> for Fake {
    ...
    async fn prune(&mut self, desired: Snapshot<Item>) -> Result<(), Self::Error> {
        ...
        self.snapshots.push(desired.all().map(|(row, _)| (row.0, row.1)).collect());
        ...
        if let Some(table) = self.mutate_during.take() {
            ...
            assert_eq!(desired.all().map(|(row, _)| (row.0, row.1)).collect::<Vec<_>>(), [(1, 10)]);
        }
        if self.failures > 0 {
            self.failures -= 1;
...
async fn first_prune_waits_for_seal_and_every_initializer() {
    ...
    let target = Fake { rows: BTreeMap::from([(99, 99)]), ..Fake::default() };
    ...
    assert_eq!(reconciler.next_prune(), Some(now + Duration::from_secs(3600)));
    assert!(reconciler.run_round(now).await.unwrap().prune.is_none());
}
...
async fn failures_wait_for_interval_but_coalesced_requests_can_retry_earlier() {
    ...
    table.seal_initializers();
    let target = Fake { failures: 2, ..Fake::default() };
    let mut reconciler = Reconciler::new(&table, target, Options { prune_interval: Duration::from_secs(10), ..Options::default() }).unwrap();
    let now = Instant::now();
    assert_eq!(reconciler.run_round(now).await.unwrap().prune.unwrap().error.as_deref(), Some("prune fault"));
    assert!(reconciler.next_retry().is_none());
    assert!(reconciler.run_round(now + Duration::from_secs(1)).await.unwrap().prune.is_none());
    let handle = reconciler.prune_handle();
    let mut requests = Vec::new();
    for _ in 0..16 {
        let handle = handle.clone();
        requests.push(tokio::spawn(async move { handle.prune_now(); }));
    }
    for request in requests { request.await.unwrap(); }
    assert!(reconciler.run_round(now + Duration::from_secs(2)).await.unwrap().prune.unwrap().error.is_some());
    assert_eq!(reconciler.target().snapshots.len(), 2);
    assert!(reconciler.run_round(now + Duration::from_secs(11)).await.unwrap().prune.is_none());
    assert!(reconciler.run_round(now + Duration::from_secs(12)).await.unwrap().prune.unwrap().error.is_none());
...
async fn resync_remains_required_after_failed_prune_until_success() {
    let table = Table::<Item>::with_stream_options(vec![], StreamOptions { tombstone_max_count: 1, ..StreamOptions::default() }).unwrap();
    let target = Fake { rows: BTreeMap::from([(99, 1)]), failures: 1, ..Fake::default() };
    let mut reconciler = Reconciler::new(&table, target, Options { prune_interval: Duration::from_secs(10), ..Options::default() }).unwrap();
    table.delete(&[1]).await.unwrap();
    ...
    assert!(waiting.prune.is_none(), "a retained resync must not busy-retry failures");
    assert!(waiting.resync_required);
    let success = reconciler.run_round(now + Duration::from_secs(10)).await.unwrap();
...
async fn cancelled_prune_replays_side_effect_and_retains_resync_obligation() {
    let table = Table::<Item>::with_stream_options(vec![], StreamOptions { tombstone_max_count: 0, ..StreamOptions::default() }).unwrap();
    let target = Fake { rows: BTreeMap::from([(99, 1)]), block_once: true, ..Fake::default() };
    let mut reconciler = Reconciler::new(&table, target, Options::default()).unwrap();
    table.delete(&[1]).await.unwrap();
    table.seal_initializers();
    let now = Instant::now();
    ...
    assert!(reconciler.target().rows.is_empty(), "side effect occurred before cancellation");
...
async fn desired_changes_during_prune_use_stable_snapshot_then_converge() {
    ...
    table.insert(Item(1, 10)).await.unwrap();
    table.seal_initializers();
    let target = Fake { rows: BTreeMap::from([(99, 99)]), mutate_during: Some(table.clone()), ..Fake::default() };
    let mut reconciler = Reconciler::new(&table, target, Options::default()).unwrap();
    let now = Instant::now();
...
async fn readiness_transition_advertises_first_prune_without_an_armed_timer() {
    ...
    assert!(reconciler.has_pending_work(), "readiness must schedule the first prune");
    assert!(reconciler.next_prune().is_none());
