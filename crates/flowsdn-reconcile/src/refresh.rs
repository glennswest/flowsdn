...
impl<T: Keyed, U: Target<T>> Reconciler<'_, T, U> {
    ...
    pub(crate) fn arm_refresh(&mut self, now: Instant) -> Result<(), ReconcileError> {
        if self.options.refresh_interval.is_zero() || self.refresh.next_due.is_some() {
            return Ok(());
        }
    ...
    fn finish_refresh_pass(&mut self, now: Instant) -> Result<(), ReconcileError> {
        let next = now
            .checked_add(self.options.refresh_interval)
            .ok_or(ReconcileError::RefreshDeadlineOverflow)?;
        let next = self
            .refresh
            .pass
            .as_ref()
            .and_then(|pass| pass.next_eligible)
            .map_or(next, |eligible| next.min(eligible.max(now)));
        self.refresh.next_due = Some(
            self.refresh
                .not_before
                .map_or(next, |limit| next.max(limit)),
        );
    ...
    pub(crate) fn complete_refresh_work(&mut self, now: Instant) -> Result<(), ReconcileError> {
        if self
            ...
            .pass
            .as_ref()
            .is_some_and(|pass| pass.exhausted && pass.candidates.is_empty())
        {
            self.finish_refresh_pass(now)?;
        }
        Ok(())
    ...
    fn refresh_eligible(&self, work: &Work<T>, started: Instant) -> bool {
        self.current(work)
            && self.statuses.get(&work.key).is_some_and(|status| {
                status.id == work.revision
                    && status.kind == Kind::Done
                    && status.updated_at.is_some_and(|updated| {
                        started.saturating_duration_since(updated) >= self.options.refresh_interval
                    })
            })
    }
    ...
    fn remember_recent(&mut self, work: &Work<T>) -> Result<(), ReconcileError> {
        ...
        if let Some(status) = self
            .statuses
            .get(&work.key)
            .filter(|status| status.id == work.revision && status.kind == Kind::Done)
            && let Some(updated) = status.updated_at
        {
            ...
            if eligible > pass.started {
                pass.next_eligible =
                    Some(pass.next_eligible.map_or(eligible, |old| old.min(eligible)));
            }
        }
        Ok(())
    }

    pub(crate) fn next_refresh_work(
        &mut self,
        now: Instant,
    ) -> Result<Option<Work<T>>, ReconcileError> {
        if self.options.refresh_interval.is_zero() {
            return Ok(None);
        }
        self.arm_refresh(now)?;
        if self.refresh.next_due.is_some_and(|due| due > now) {
            return Ok(None);
        }
        if self.refresh.pass.is_none() {
            self.refresh.pass = Some(Pass {
        ...
        let pass = self
            ...
            .pass
            .as_ref()
            .expect("refresh pass was created");
        if pass.candidates.is_empty() && !pass.exhausted {
            ...
            for _ in 0..self.options.round_size {
                let Some((row, revision)) = rows.next() else {
                    exhausted = true;
                    break;
                };
                let work = Work {
                ...
                if self.refresh_eligible(&work, started) {
            ...
            match candidate {
                ...
                None => {
                        ...
                        .exhausted
                    {
                        ...
                        self.refresh.next_due = Some(now);
                    }
                    return Ok(None);
                }
            }
