...
impl Write for Diagnostics {
    fn write_str(&mut self, value: &str) -> fmt::Result {
        let available = MAX_DIAGNOSTIC_BYTES
...
impl<T: Keyed, U: Target<T>> Reconciler<'_, T, U> {
    ...
    pub fn with_reporter(mut self, reporter: Reporter) -> Self {
        self.health_failures = self
            .statuses
            .iter()
            .filter(|(_, status)| status.kind == Kind::Error || status.retries > 0)
            .map(|(key, status)| {
                (
                    key.clone(),
                    (
                        status.id,
                        retain_error(status.error.as_deref().unwrap_or("previous attempt failed")),
                    ),
                )
            })
            .collect();
    ...
    pub fn last_health_error(&self) -> Option<&str> {
    ...
    pub(crate) async fn finish_health_round(
        &mut self,
        ...
        for (key, (revision, error)) in &self.health_failures {
            let Some(status) = self.statuses.get(key).filter(|status| {
                status.id == *revision && (status.kind == Kind::Error || status.retries > 0)
            }) else {
                continue;
            };
            let current = desired.get("primary", key).expect("primary index exists");
            let relevant = match current {
                Some((_, desired_revision)) => {
                    status.operation == Operation::Update && *revision == desired_revision
                }
                None => status.operation == Operation::Delete,
            };
            if relevant {
                if count > 0 {
                    let _ = errors.write_str("; ");
                }
        ...
        let result = if count == 0 {
            ...
            reporter
                .degraded(format!("{count} errors"), errors.finish())
                .await
        };
    ...
    pub(crate) async fn stop_health(
        &mut self,
        failure: Option<ReconcileError>,
    ) -> Result<(), ReconcileError> {
        let Some(reporter) = self.reporter.clone() else {
        ...
        let reason = failure.map_or_else(
            || "reconciler loop stopped".to_owned(),
            |error| format!("reconciler loop stopped: {error}"),
        );
...
mod tests {
    ...
    async fn publication_error_boundary_preserves_queued_work_and_can_recover() {
        ...
        let mut reconciler = Reconciler::new(&table, TargetOk, Options::default())
            .unwrap()
            .with_reporter(registry.reporter("test").unwrap());
        ...
        assert_eq!(
            reconciler.health_result(Err(TableError::RevisionExhausted)),
            Err(ReconcileError::HealthPublicationFailed)
        );
        ...
        assert_eq!(
            reconciler.run_round(Instant::now()).await.unwrap().updated,
            1
        );
