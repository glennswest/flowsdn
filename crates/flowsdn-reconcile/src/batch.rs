...
impl<T: Keyed, U: Target<T>> Reconciler<'_, T, U> {
    pub(crate) async fn run_batch_round(
        ...
        while self.pending.len().saturating_add(round.processed) < self.options.round_size {
            if let Some(retry) = self.retries.due(clock()) {
                let row = if retry.operation == crate::Operation::Update {
                    self.table.snapshot().get("primary", &retry.key)
                        .expect("primary index exists").map(|(row, _)| row)
                } else { None };
                ...
                self.pending.push_back(Work { key: retry.key, revision: retry.revision, row, failures: retry.failures, refresh: retry.refresh });
        ...
        self.pending.make_contiguous().sort_by_key(|work| (work.row.is_some(), work.revision));
        ...
        for deleting in [true, false] {
            ...
            let group: Vec<_> = self.pending.iter().take(capacity)
                .take_while(|work| work.row.is_none() == deleting).cloned().collect();
            let active: Vec<_> = group.iter().filter(|work| self.current(work)).cloned().collect();
            for work in &active {
                let mut status = Status::pending(work.revision, work.operation());
                status.retries = work.failures;
                if work.refresh { status.kind = Kind::Refreshing; }
                self.statuses.insert(work.key.clone(), status);
            }
            let results = if active.is_empty() { Vec::new() }
                else if deleting {
                    self.target.delete_batch(active.iter().map(|work| BatchDelete { key: work.key.clone(), revision: work.revision }).collect()).await
                } else {
                    self.target.update_batch(active.iter().map(|work| BatchUpdate {
                        key: work.key.clone(), revision: work.revision,
                        row: work.row.clone().expect("update group has rows"),
                        hint: if work.refresh { UpdateHint::Refresh } else { UpdateHint::Changed },
                    }).collect()).await
                };
            ...
            if results.len() != active.len() { return Err(ReconcileError::InvalidBatchResults); }
            let mut indexed = BTreeMap::new();
            for result in results {
                if indexed.insert((result.key, result.revision), result.result).is_some() {
                    return Err(ReconcileError::InvalidBatchResults);
                }
            }
            if active.iter().any(|work| !indexed.contains_key(&(work.key.clone(), work.revision))) {
                return Err(ReconcileError::InvalidBatchResults);
            }
            for work in group {
                let result = indexed.remove(&(work.key.clone(), work.revision));
                match result {
                    Some(result) if self.current(&work) => self.record_result(&work, result, clock(), round)?,
                    _ => {
