...
use aya_obj::generated::{bpf_attr, bpf_link_info, bpf_cmd, bpf_link_type, bpf_attach_type};
use std::{ffi::CString, mem::{size_of, zeroed}, os::{fd::{OwnedFd, FromRawFd, AsRawFd}, unix::ffi::OsStrExt}, path::Path};
pub struct Identity { pub id: u32, ifindex: u32, attach_type: u32 }
impl Identity {
    pub fn matches(&self, interface: &str, allow_detached: bool) -> KernelResult<bool> {
        ...
        Ok(self.attach_type == bpf_attach_type::BPF_TCX_INGRESS as u32
            && ((index != 0 && index == self.ifindex) || (allow_detached && index == 0 && self.ifindex == 0)))
    }
}
pub fn read(path: &Path) -> KernelResult<Identity> {
    ...
    unsafe {
        ...
        let fd = libc::syscall(libc::SYS_bpf, bpf_cmd::BPF_OBJ_GET as u32, &attr, size_of::<bpf_attr>());
        ...
        if libc::syscall(libc::SYS_bpf, bpf_cmd::BPF_OBJ_GET_INFO_BY_FD as u32, &mut attr, size_of::<bpf_attr>()) < 0 {
            return Err(std::io::Error::last_os_error().into());
        }
        if info.type_ != bpf_link_type::BPF_LINK_TYPE_TCX as u32 { return Err("pinned link is not TCX".into()); }
        let tcx = info.__bindgen_anon_1.tcx;
        Ok(Identity { id: info.id, ifindex: tcx.ifindex, attach_type: tcx.attach_type })
    }
}
