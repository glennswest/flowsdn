use flowsdn_bpf_loader::nested::{
    self, ARRAY_OF_MAPS, Error, Features, HASH_OF_MAPS, NestedSpec, Plan, Purpose,
};
use flowsdn_bpf_loader::{LRU_HASH, MapSpec, Owner, RDONLY_PROG, Replacement};
...
fn each_outer_family_requires_its_own_feature() {
    for (purpose, spec, features) in [
        ...
        assert_eq!(
            nested::plan(spec, None, Owner::Agent, purpose, Features::default()),
            Ok(Plan::Disabled)
        );
        assert_eq!(
            nested::plan(spec, Some(spec), Owner::Agent, purpose, Features::default()),
            Ok(Plan::Disabled)
        );
        assert_eq!(
            nested::plan(spec, None, Owner::Agent, purpose, features),
            Ok(Plan::Create { spec })
        );
        for other in [
            Purpose::Maglev,
            ...
            if other != purpose
                && !(purpose == Purpose::PerClusterNat && other == Purpose::PerClusterConntrack)
            {
...
fn identical_outer_and_template_reuse_with_outer_read_only_upgrade() {
    ...
    assert_eq!(
        nested::plan(MAGLEV, Some(pinned), Owner::Loader, Purpose::Maglev, ALL),
        Ok(Plan::Reuse {
...
fn lru_inner_capacity_is_exact_not_the_standalone_reuse_exception() {
    ...
    assert!(matches!(
        ...
        Ok(Plan::Replace {
            inner_changes: flowsdn_bpf_loader::Differences {
                max_entries: true,
                ..
            },
...
fn invalid_and_wrong_purpose_layouts_fail_before_creation() {
    for (spec, error) in [
        ...
        assert_eq!(
            nested::plan(spec, None, Owner::Agent, Purpose::Maglev, ALL),
            Err(error)
        );
    }
    let invalid_array = NestedSpec {
        outer: MapSpec {
            key_size: 2,
            ..CT.outer
        },
    ...
    assert_eq!(
        invalid_array.validate(Purpose::PerClusterConntrack),
        Err(Error::InvalidOuterLayout)
    );
}
...
fn cluster_outer_array_includes_the_highest_id_and_cluster_zero() {
    ...
    for invalid in [0, 254, 256, 512, u32::MAX] {
        assert_eq!(
            nested::cluster_outer_entries(invalid),
            Err(Error::InvalidClusterMaximum)
        );
    }
}
