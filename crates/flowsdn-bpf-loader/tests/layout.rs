...
fn target_cache_lines_drive_exact_stride_and_storage() {
    for (arch, section, stride, size, last) in [
        ...
        assert_eq!((layout.stride(), layout.value_size(), layout.max_offset()), (stride, size, last));
    }
}
#[test]
fn addressing_clamps_unexpected_cpu_without_wrapping() {
    ...
    for cpu in [2, 3, 1000, u32::MAX] { assert_eq!(layout.offset_for_cpu(cpu), 256); }
...
fn empty_input_and_both_overflow_stages_fail_before_map_allocation() {
    for arch in [Arch::X86_64, Arch::Aarch64] {
        assert_eq!(ScratchLayout::new(0, 1, arch), Err(LayoutError::EmptySection));
        assert_eq!(ScratchLayout::new(1, 0, arch), Err(LayoutError::NoPossibleCpus));
        assert_eq!(ScratchLayout::new(u32::MAX, 1, arch), Err(LayoutError::ValueSizeOverflow));
        assert_eq!(ScratchLayout::new(1, u32::MAX, arch), Err(LayoutError::ValueSizeOverflow));
    }
    ...
    assert_eq!(ScratchLayout::new(0xffff_ff80, 2, Arch::Aarch64), Err(LayoutError::ValueSizeOverflow));
}
