...
fn lru_capacity_changes_preserve_contents_and_report_effective_capacity() {
    ...
    for map_type in [LRU_HASH, LRU_PERCPU_HASH] {
        for (requested, pinned) in [(1024, 4096), (4096, 1024)] {
            let spec = MapSpec { map_type, max_entries: requested, ..SPEC };
            let existing = MapSpec { max_entries: pinned, ..spec };
            for owner in [Owner::Agent, Owner::Loader] {
                assert_eq!(plan_map(spec, Some(existing), owner), MapPlan::Reuse {
...
fn lru_capacity_tolerance_cannot_hide_schema_flags_or_zero_capacity() {
    let spec = MapSpec { map_type: flowsdn_bpf_loader::LRU_HASH, ..SPEC };
    for existing in [
        MapSpec { max_entries: 2048, key_size: 4, ..spec },
        MapSpec { max_entries: 2048, value_size: 40, ..spec },
        MapSpec { max_entries: 2048, flags: 2, ..spec },
        MapSpec { max_entries: 2048, map_type: 1, ..spec },
        MapSpec { max_entries: 0, ..spec },
    ] {
        assert!(matches!(plan_map(spec, Some(existing), Owner::Agent), MapPlan::Replace { .. }));
    }
    assert!(matches!(plan_map(MapSpec { max_entries: 0, ..spec }, Some(spec), Owner::Agent), MapPlan::Replace { .. }));
}

#[test]
fn node_ids_cannot_be_silently_discarded_by_any_replacement() {
    use flowsdn_bpf_loader::plan_node_id_map;
    let spec = MapSpec { map_type: 1, key_size: 20, value_size: 4, max_entries: 16384, flags: RDONLY_PROG | 1 };
    for owner in [Owner::Agent, Owner::Loader] {
        assert_eq!(plan_node_id_map(spec, None, owner), Ok(MapPlan::Create { spec }));
        assert!(matches!(plan_node_id_map(spec, Some(spec), owner), Ok(MapPlan::Reuse { .. })));
        // The read-write upgrade exception preserves the original contents.
        assert!(matches!(plan_node_id_map(spec, Some(MapSpec { flags: 1, ..spec }), owner), Ok(MapPlan::Reuse { relaxed_program_read_only: true, .. })));
        for existing in [
            MapSpec { key_size: 24, ..spec }, MapSpec { value_size: 8, ..spec },
            MapSpec { max_entries: 32768, ..spec }, MapSpec { flags: spec.flags | 2, ..spec },
        ] {
            let refused = plan_node_id_map(spec, Some(existing), owner).expect_err("migration required");
