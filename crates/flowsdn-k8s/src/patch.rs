...
pub fn node_status_patch(
    ...
    let metadata = node
        ...
        .ok_or_else(|| Error("missing node metadata".into()))?;
    let mut ops = Vec::new();
    for field in ["uid", "resourceVersion"] {
        let value = metadata
            ...
            .filter(|v| !v.is_empty())
            .ok_or_else(|| Error(format!("missing node {field}")))?;
        ops.push(json!({"op":"test", "path":format!("/metadata/{field}"), "value":value}));
    }
    let updates: serde_json::Map<String, Value> = annotations
        .iter()
        .filter_map(|(key, value)| value.as_ref().map(|value| (key.clone(), json!(value))))
        .collect();
    if !updates.is_empty() {
        match metadata.get("annotations") {
            None | Some(Value::Null) => {
                ops.push(json!({"op":"add", "path":"/metadata/annotations", "value":updates}))
            }
            Some(Value::Object(_)) => {
                for (key, value) in updates {
                    ops.push(json!({"op":"add", "path":format!("/metadata/annotations/{}", escaped(&key)), "value":value}));
                }
            }
    ...
    if let Some(condition) = condition {
        let kind = condition
            ...
            .and_then(Value::as_str)
            .filter(|v| !v.is_empty())
            .ok_or_else(|| Error("condition requires a type".into()))?;
        match node.get("status") {
            None | Some(Value::Null) => {
                ops.push(json!({"op":"add", "path":"/status", "value":{"conditions":[condition]}}))
            }
            Some(Value::Object(status)) => {
                match status.get("conditions") {
                    None | Some(Value::Null) => ops.push(
                        json!({"op":"add", "path":"/status/conditions", "value":[condition]}),
                    ),
                    Some(Value::Array(conditions)) => {
                        ...
                        for (position, existing) in conditions.iter().enumerate() {
                            let existing_kind = existing
                                ...
                                .filter(|v| !v.is_empty())
                                .ok_or_else(|| {
                                    Error("existing condition requires a type".into())
                                })?;
                            if !seen.insert(existing_kind) {
                                return Err(Error("duplicate condition type".into()));
                            }
