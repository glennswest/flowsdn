...
fn random_scan_exhausts_each_usable_address_and_reuses_a_hole() {
    ...
    for _ in 0..254 {
        ...
        assert_eq!(pool.dump().get(&address).map(String::as_str), Some("ns/pod"));
    }
    ...
    for (network, prefix) in [("192.0.2.0", 30), ("2001:db8::", 126)] {
        for allow_first_ip in [false, true] {
            for allow_last_ip in [false, true] {
                let mut pool = HostScope::new(ip(network), prefix, RangeOptions {
                    allow_first_ip, allow_last_ip,
                }).expect("range");
                let expected = 2u128.saturating_add(u128::from(allow_first_ip))
                    .saturating_add(u128::from(allow_last_ip));
                assert_eq!(pool.capacity(), expected);
                for _ in 0..expected { pool.allocate_next("pod").expect("free"); }
                assert_eq!(pool.allocate_next("pod"), Err(Error::Full));
                assert_eq!(pool.dump().contains_key(&ip(network)), allow_first_ip);
                let last = if prefix == 30 { ip("192.0.2.3") } else { ip("2001:db8::3") };
                assert_eq!(pool.dump().contains_key(&last), allow_last_ip);
            }
        }
    }
    for (network, prefix, count) in [
        ("255.255.255.254", 31, 2), ("255.255.255.255", 32, 1),
        ...
        for _ in 0..count { pool.allocate_next("pod").expect("free"); }
        assert_eq!(pool.allocate_next("pod"), Err(Error::Full));
...
fn specific_restore_preserves_owners_and_rejects_collisions_and_boundaries() {
    ...
    pool.allocate_without_sync(address, "restored/pod").expect("restore");
    pool.restore_finished();
    assert_eq!(pool.allocate(address, "new/pod"), Err(Error::Allocated));
    assert_eq!(pool.dump().get(&address).map(String::as_str), Some("restored/pod"));
...
fn excluded_addresses_are_reserved_with_owner_on_scan_and_survive_release() {
    ...
    assert_eq!(pool.allocate(excluded, "pod"), Err(Error::Excluded { owner: "router".into() }));
    assert_eq!(pool.allocate_next("pod"), Ok(ip("192.0.2.2")));
    assert_eq!(pool.allocate_next("pod"), Err(Error::Full));
    assert_eq!(pool.dump().get(&excluded).map(String::as_str), Some("router (excluded)"));
    pool.release(excluded);
    assert_eq!(pool.allocate_without_sync(excluded, "pod"), Err(Error::Excluded { owner: "router".into() }));
    assert_eq!(pool.allocate_next("pod"), Err(Error::Full));
...
fn excluding_an_existing_allocation_preserves_its_owner_until_release() {
    ...
    assert_eq!(pool.dump().get(&address).map(String::as_str), Some("health"));
    pool.release(address);
    assert_eq!(pool.allocate_next("pod"), Err(Error::Full));
    assert_eq!(pool.dump().get(&address).map(String::as_str), Some("router (excluded)"));
}

#[test]
fn ipv6_large_prefixes_have_full_capacity_without_dense_storage() {
    for (prefix, capacity) in [(96, (1u128 << 32).saturating_sub(2)),
        (64, (1u128 << 64).saturating_sub(2)), (0, u128::MAX.saturating_sub(1))] {
        let mut pool = pool("2001:db8::", prefix);
        assert_eq!(pool.capacity(), capacity);
        for _ in 0..16 { pool.allocate_next("pod").expect("sparse allocation"); }
        assert_eq!(pool.allocated(), 16);
    }
    let all = RangeOptions { allow_first_ip: true, allow_last_ip: true };
    assert!(matches!(HostScope::new(ip("::"), 0, all), Err(Error::CapacityOverflow)));
    assert!(matches!(HostScope::new(ip("::"), 129, all), Err(Error::InvalidPrefix)));
    assert!(matches!(HostScope::new(ip("0.0.0.0"), 33, all), Err(Error::InvalidPrefix)));
}
...
fn dual_stack_failure_rolls_back_only_the_new_ipv6_allocation() {
    ...
    v6.allocate(ip("2001:db8::1"), "existing6").expect("v6 existing");
    ...
    assert_eq!(v6.dump().get(&ip("2001:db8::1")).map(String::as_str), Some("existing6"));
...
fn ipv6_failure_does_not_allocate_ipv4_and_disabled_families_are_explicit() {
    ...
    assert!(matches!(Ipam::new(Some(pool("::", 128)), None), Err(Error::FamilyMismatch)));
    let mut ipam = Ipam::new(None, None).expect("disabled");
    assert_eq!(ipam.allocate_next("pod"), Err(Error::FamilyDisabled));
    assert_eq!(ipam.allocate(ip("192.0.2.1"), "pod"), Err(Error::FamilyDisabled));
