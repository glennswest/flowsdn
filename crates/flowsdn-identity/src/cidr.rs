...
impl CidrPrefix {
    pub fn new(address: IpAddr, length: u8) -> Result<Self, CidrError> {
        let network = match address {
            IpAddr::V4(address) => {
                if length > 32 {
                    return Err(CidrError::InvalidLength);
                }
                let mask = u32::MAX
            ...
            IpAddr::V6(address) => {
                ...
                let mask = u128::MAX
                    .checked_shl(128_u32.saturating_sub(u32::from(length)))
                    .unwrap_or(0);
    ...
    pub fn decode_key(key: &str) -> Result<Self, CidrError> {
        if key.len() > 64 {
            return Err(CidrError::EncodedKeyTooLong);
        }
        key.replace('-', ":").parse()
    }

    pub fn from_label(label: &Label) -> Result<Self, CidrError> {
        if label.source() != "cidr" || !label.value().is_empty() {
            return Err(CidrError::InvalidLabel);
        }
    ...
    pub fn to_label(self) -> Option<Label> {
        if self.is_world() {
            ...
            Some(
                Label::new("cidr", &self.encoded_key(), "")
                    .expect("canonical CIDR key is nonempty"),
            )
        }
    }
