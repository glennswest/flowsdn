...
pub enum Scope { Global, Local, RemoteNode }
...
impl fmt::Display for IdentityError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            ...
            Self::ScopedIdentityOnWire => "scoped identities cannot be encoded in a 24-bit wire field",
            ...
            Self::MarkCollision => "cluster ID bit 0x80 conflicts with the selected IPAM or chaining mode",
            Self::ScopedIdentityHasNoGlobalCluster => "scoped identity has no global cluster encoding",
...
impl NumericIdentity {
    ...
    pub const fn scope(self) -> Scope {
        match self.0 >> 24 { 0 => Scope::Global, 1 => Scope::Local, _ => Scope::RemoteNode }
    }
    pub const fn local(index: u32) -> Result<Self, IdentityError> { Self::scoped(index, 0x0100_0000) }
    pub const fn remote_node(index: u32) -> Result<Self, IdentityError> { Self::scoped(index, 0x0200_0000) }
    const fn scoped(index: u32, scope: u32) -> Result<Self, IdentityError> {
        if index == 0 { return Err(IdentityError::EmptyScopedIdentity); }
        if index > WIRE_MAX { return Err(IdentityError::InvalidScopeIndex); }
        Ok(Self(scope | index))
    }
    pub const fn from_le_bytes(bytes: [u8; 4]) -> Result<Self, IdentityError> { Self::new(u32::from_le_bytes(bytes)) }
    pub const fn to_le_bytes(self) -> [u8; 4] { self.0.to_le_bytes() }
    pub const fn from_be_bytes(bytes: [u8; 4]) -> Result<Self, IdentityError> { Self::new(u32::from_be_bytes(bytes)) }
    pub const fn to_be_bytes(self) -> [u8; 4] { self.0.to_be_bytes() }
    pub const fn is_cidr(self) -> bool { matches!(self.scope(), Scope::Local) }
    pub const fn is_world(self) -> bool { matches!(self.0, 2 | 9 | 10) || self.is_cidr() }
    pub fn class(self) -> IdentityClass {
        match self.scope() {
            Scope::Local => IdentityClass::Local,
            Scope::RemoteNode => IdentityClass::RemoteNode,
            Scope::Global => {
                if let Some(reserved) = ReservedIdentity::from_number(self.0) { return IdentityClass::Reserved(reserved); }
                match self.0 {
    ...
    pub const fn tunnel_id(self) -> Result<WireIdentity, IdentityError> {
        if self.0 > WIRE_MAX { return Err(IdentityError::ScopedIdentityOnWire); }
        Ok(WireIdentity(if matches!(self.0, 9 | 10) { 2 } else { self.0 }))
    }
}
impl TryFrom<u32> for NumericIdentity {
    type Error = IdentityError;
    fn try_from(value: u32) -> Result<Self, Self::Error> { Self::new(value) }
}
...
pub enum IdentityClass {
    Reserved(ReservedIdentity), UnallocatedReserved, DeprecatedReserved,
    WellKnown, UserReserved, Global, Local, RemoteNode,
}
...
pub const ALL_AGGREGATES: [ReservedIdentity; 5] = [ReservedIdentity::Unknown,
    ReservedIdentity::AggregateRemoteNode, ReservedIdentity::AggregateWorld,
    ReservedIdentity::AggregateCluster, ReservedIdentity::AggregateClusterMesh];
...
impl WireIdentity {
    pub const fn new(identity: NumericIdentity) -> Result<Self, IdentityError> {
        if identity.0 > WIRE_MAX { Err(IdentityError::ScopedIdentityOnWire) } else { Ok(Self(identity.0)) }
    }
    pub const fn get(self) -> u32 { self.0 }
    pub const fn to_be_bytes(self) -> [u8; 3] { let [_, a, b, c] = self.0.to_be_bytes(); [a, b, c] }
    pub const fn from_be_bytes([a, b, c]: [u8; 3]) -> Self { Self(u32::from_be_bytes([0, a, b, c])) }
    ...
    pub const fn received(self, family: IpFamily, dual_stack: bool) -> NumericIdentity {
        NumericIdentity(if self.0 == 2 && dual_stack { match family { IpFamily::V4 => 9, IpFamily::V6 => 10 } } else { self.0 })
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct AllocationRange { pub min: NumericIdentity, pub max: NumericIdentity }
impl AllocationRange {
    pub const fn contains(self, identity: NumericIdentity) -> bool { identity.0 >= self.min.0 && identity.0 <= self.max.0 }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ClusterEncoding { max_clusters: u16, shift: u32 }
impl ClusterEncoding {
    pub const fn new(max_clusters: u16) -> Result<Self, IdentityError> {
        match max_clusters {
            255 => Ok(Self { max_clusters, shift: 16 }),
            511 => Ok(Self { max_clusters, shift: 15 }),
            _ => Err(IdentityError::InvalidClusterLimit),
        }
    }
    pub const fn max_clusters(self) -> u16 { self.max_clusters }
    pub const fn shift(self) -> u32 { self.shift }
    pub const fn index_mask(self) -> u32 { match self.shift { 16 => 0xffff, _ => 0x7fff } }
    fn validate_cluster(self, cluster: u16) -> Result<(), IdentityError> {
        if cluster > self.max_clusters { Err(IdentityError::InvalidClusterId) } else { Ok(()) }
    }
    ...
    pub fn validate_meshing(self, cluster: u16, mark_sensitive: bool) -> Result<(), IdentityError> {
        ...
        if cluster == 0 { return Err(IdentityError::ClusterUnset); }
        if mark_sensitive && cluster & 0x80 != 0 { return Err(IdentityError::MarkCollision); }
        Ok(())
    }
    pub fn allocation_range(self, cluster: u16) -> Result<AllocationRange, IdentityError> {
        self.validate_cluster(cluster)?;
        let prefix = u32::from(cluster) << self.shift;
        Ok(AllocationRange { min: NumericIdentity(if cluster == 0 { FIRST_ALLOCATED_GLOBAL } else { prefix }), max: NumericIdentity(prefix | self.index_mask()) })
    }
    ...
    pub fn global(self, cluster: u16, index: u32) -> Result<NumericIdentity, IdentityError> {
        ...
        if index > self.index_mask() { return Err(IdentityError::OutsideAllocationRange); }
        let identity = NumericIdentity((u32::from(cluster) << self.shift) | index);
        if range.contains(identity) { Ok(identity) } else { Err(IdentityError::OutsideAllocationRange) }
    }
    pub fn global_cluster_id(self, identity: NumericIdentity) -> Result<u16, IdentityError> {
        if !matches!(identity.scope(), Scope::Global) { return Err(IdentityError::ScopedIdentityHasNoGlobalCluster); }
        // The mask is at most nine bits, so this conversion cannot truncate.
        Ok(u16::try_from((identity.0 >> self.shift) & u32::from(self.max_clusters)).expect("nine-bit cluster ID"))
    }
    ...
    pub fn validate_remote_allocated(self, cluster: u16, identity: NumericIdentity) -> Result<(), IdentityError> {
        self.validate_meshing(cluster, false)?;
        if self.allocation_range(cluster)?.contains(identity) { Ok(()) } else { Err(IdentityError::OutsideAllocationRange) }
    }
    ...
    pub fn validate_remote_ipcache(self, cluster: u16, identity: NumericIdentity) -> Result<(), IdentityError> {
        self.validate_meshing(cluster, false)?;
        if identity.0 < FIRST_ALLOCATED_GLOBAL || self.allocation_range(cluster)?.contains(identity) { Ok(()) } else { Err(IdentityError::OutsideAllocationRange) }
    }
    pub fn aggregate_for(self, identity: NumericIdentity, local_cluster: u16) -> Result<ReservedIdentity, IdentityError> {
        ...
        Ok(match identity.scope() {
            ...
            Scope::Global if self.global_cluster_id(identity)? == local_cluster => ReservedIdentity::AggregateCluster,
