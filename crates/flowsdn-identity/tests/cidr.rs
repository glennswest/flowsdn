...
fn prefix(text: &str) -> CidrPrefix {
    text.parse().expect("valid prefix fixture")
}

#[test]
fn ipv4_canonical_masking_and_label_roundtrip() {
    for (input, canonical) in [
        ("10.23.45.67/8", "10.0.0.0/8"),
...
fn ipv6_leading_trailing_colons_and_canonical_compression() {
    for (input, canonical, key) in [
        ...
        assert_eq!(
            CidrPrefix::from_label(&value.to_label().expect("non-world")),
            Ok(value)
        );
    }
}
...
fn zero_prefix_omits_cidr_identity_label_but_retains_family_for_selectors() {
    ...
    assert_eq!(
        CidrPrefix::from_label(&Label::parse("reserved:world").expect("label")),
        Err(CidrError::InvalidLabel)
    );
}

#[test]
fn prefix_boundaries_and_invalid_text_are_rejected() {
    for text in ["10.0.0.1/33", "::1/129", "::/256"] {
        assert_eq!(text.parse::<CidrPrefix>(), Err(CidrError::InvalidLength));
    }
    for text in [
        "10.0.0.1",
        "10.0.0.1/",
        ...
        assert_eq!(text.parse::<CidrPrefix>(), Err(CidrError::InvalidPrefix));
    }
    for text in [
        "300.0.0.1/24",
        "192.168.001.1/24",
        ...
        assert_eq!(text.parse::<CidrPrefix>(), Err(CidrError::InvalidAddress));
    }
    for key in ["0---1/128", "fffff--0/64", "192.0.2.1/33"] {
        assert!(CidrPrefix::decode_key(key).is_err());
    }
    assert_eq!(
        CidrPrefix::decode_key(&"a".repeat(65)),
        Err(CidrError::EncodedKeyTooLong)
    );
}
...
fn containment_observes_prefix_length_and_both_network_edges() {
    ...
    for address in ["192.0.2.128", "192.0.2.255"] {
        assert!(selector.contains_address(address.parse().expect("address")));
    }
    for address in ["192.0.2.127", "192.0.3.0"] {
        assert!(!selector.contains_address(address.parse().expect("address")));
    }
...
fn all_prefix_lengths_mask_and_roundtrip_without_shift_overflow() {
    for length in 0..=32 {
        ...
        let IpAddr::V4(network) = value.network() else {
    ...
    for length in 0..=128 {
        ...
        let IpAddr::V6(network) = value.network() else {
            panic!("family changed");
        };
...
fn cidr_labels_match_by_containment_without_general_source_rules() {
    ...
    assert_eq!(
        cidr_selector_matches(&any, &candidate),
        Err(CidrError::InvalidLabel)
    );
}
