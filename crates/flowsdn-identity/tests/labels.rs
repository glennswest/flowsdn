...
fn reserved_shorthand_and_empty_key_validation() {
    for input in ["$host", "reserved:host", "reserved:=host", "$=host"] {
        assert_eq!(
            Label::parse(input).unwrap(),
            Label::new("reserved", "host", "").unwrap()
        );
    }
...
fn selectors_default_unspecified_source_to_any() {
    ...
    assert_eq!(
        Label::parse_selector("k8s:app=web").unwrap().source(),
        "k8s"
    );
...
fn cidr_values_are_rejected_without_claiming_prefix_validation() {
    assert_eq!(
        Label::parse("cidr:10.0.0.0/8=x"),
        Err(LabelError::CidrValue)
    );
...
fn key_only_identity_replacement_and_removal() {
    ...
    assert_eq!(
        labels.insert(Label::parse("container:app=new").unwrap()),
        Some(old)
    );
...
fn unicode_keys_follow_utf8_byte_order_not_source_order() {
    let labels: Labels = ["a:é=x", "z:z=y", "q:Z="]
        .into_iter()
        .map(|s| Label::parse(s).unwrap())
        .collect();
