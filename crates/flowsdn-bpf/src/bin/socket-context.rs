...
pub fn socket_connect4(ctx:SockAddrContext)->i32 {
    ...
    let mut address=model::Address {family:raw.user_family,ipv4:raw.user_ip4,ipv6:[0;4],port:raw.user_port};
    let verdict=model::rewrite4(&mut address);
    raw.user_ip4=address.ipv4;raw.user_port=address.port;
...
pub fn socket_connect6(ctx:SockAddrContext)->i32 {
    ...
    let raw=unsafe {&mut *ctx.sock_addr};
    let mut address=model::Address {family:raw.user_family,ipv4:0,ipv6:raw.user_ip6,port:raw.user_port};
    let verdict=model::rewrite6(&mut address);
    raw.user_ip6=address.ipv6;raw.user_port=address.port;
    if verdict==0 {0}else{1}
}
