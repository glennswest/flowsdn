...
use crate::{
    CniError, Result,
    delete::{DeleteQueueGuard, DeleteRequest},
};
use nix::{
    errno::Errno,
...
fn error(error: impl std::fmt::Display) -> CniError {
    CniError::internal(error.to_string())
}
...
fn open_lockfile(path: &Path) -> Result<File> {
    ...
    if !file.metadata().map_err(error)?.is_file() {
        return Err(error("queue lockfile is not a regular file"));
    }
    Ok(file)
}
...
fn lock(mut file: File, kind: FlockArg, timeout: Duration) -> Result<Flock<File>> {
    ...
    loop {
        match Flock::lock(file, kind) {
            ...
            Err((returned, errno)) => {
                file = returned;
                if !matches!(errno, Errno::EWOULDBLOCK | Errno::EINTR) {
                    return Err(error(errno));
                }
                let remaining = timeout.saturating_sub(start.elapsed());
                if remaining.is_zero() {
                    return Err(error("deletion queue lock timed out"));
                }
...
impl Temporary {
    fn create(directory: &Path) -> Result<(Self, File)> {
        ...
        Err(error(
            "could not create a unique deletion queue temporary file",
        ))
    }
}
...
impl ExclusiveGuard {
    pub fn entries(&self) -> Result<Vec<Entry>> {
        Ok(entry_paths(&self.path)?
            .into_iter()
            .map(|path| {
                let request = read_regular(&path)
                    .map_err(error)
...
fn entry_paths(path: &Path) -> Result<Vec<PathBuf>> {
    ...
    for entry in fs::read_dir(path).map_err(error)? {
        ...
        if path.extension() == Some(OsString::from("delete").as_os_str()) {
            entries.push(path);
        }
    }
    entries.sort();
...
fn read_regular(path: &Path) -> std::io::Result<Vec<u8>> {
    ...
    if !file.metadata()?.is_file() {
        return Err(std::io::Error::other("queue entry is not a regular file"));
    }
...
fn decode(bytes: &[u8]) -> Result<ReplayRequest> {
    if let Ok(json) = serde_json::from_slice::<serde_json::Value>(bytes) {
        let container_id = json
            ...
            .and_then(|v| v.as_str())
            .filter(|v| !v.is_empty())
            .ok_or_else(|| error("invalid container deletion entry"))?;
        return Ok(ReplayRequest::Container {
            container_id: container_id.to_owned(),
        });
    }
    let text = std::str::from_utf8(bytes).map_err(error)?;
    let (container_id, ifname) = text
        .rsplit_once(':')
        .ok_or_else(|| error("invalid attachment deletion entry"))?;
