...
impl Temp {
    fn new() -> Self {
        let path = std::env::temp_dir().join(format!("flowsdn-state-{}-{}", std::process::id(), NEXT.fetch_add(1,Ordering::Relaxed)));
        fs::create_dir(&path).expect("temp"); Self(path)
    }
}
...
fn publication_survives_reopen_and_preserves_unknown_fields() {
    ...
    store.stage(&record).expect("stage").publish().expect("publish");
...
fn failed_creation_drops_only_its_staging_directory() {
    let temp = Temp::new(); let store = Store::open(&temp.0).expect("store");
...
fn concurrent_agent_is_excluded_until_owner_drops() {
    let temp=Temp::new(); let owner=Store::open(&temp.0).expect("owner");
    assert!(Store::open(&temp.0).is_err()); drop(owner);
    Store::open(&temp.0).expect("new owner");
}
#[test]
fn duplicate_attachment_and_directory_record_mismatch_fail_restore() {
    let temp=Temp::new(); let store=Store::open(&temp.0).expect("store");
    store.stage(&record(1,"same")).expect("stage").publish().expect("publish");
    store.stage(&record(2,"same")).expect("stage").publish().expect("publish");
    assert!(store.restore().is_err()); store.remove(2).expect("remove duplicate");
...
fn unexpected_resources_and_symlinks_are_preserved_without_partial_delete() {
    let temp=Temp::new(); let store=Store::open(&temp.0).expect("store");
    store.stage(&record(1,"one")).expect("stage").publish().expect("publish");
    ...
    assert!(store.restore().is_err()); assert!(store.remove(2).is_err());
    assert!(temp.0.join("1/ep_config.json").exists());
}
#[test]
fn lowest_free_ids_exhaust_and_restore_supports_historical_high_ids() {
    let mut pool=IdPool::default(); pool.reserve(65535).expect("historical");
    assert!(pool.reserve(65535).is_err()); assert!(pool.reserve(0).is_err());
    for id in 1..=4095 { assert_eq!(pool.allocate().expect("allocate"),id); }
    assert!(pool.allocate().is_err()); pool.release(17);
