...
impl Temp {
    fn new() -> Self {
        let path = std::env::temp_dir().join(format!("flowsdn-api-test-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
...
fn status(bytes: &[u8]) -> u16 {
    match parse(bytes) {
        ...
        Err(error) => error.downcast_ref::<Failure>().expect("HTTP failure").status,
    }
}
#[test]
fn framing_rejects_ambiguous_truncated_and_oversized_requests() {
    ...
    let request=parse(b"POST /v1/ipam?owner=a%2Fb HTTP/1.1\r\nExpiration: true\r\nContent-Length: 0\r\n\r\n").expect("valid request");
    assert!(request.expiration);
    assert_eq!(query_values("owner=a%2Fb").expect("query").get("owner").map(String::as_str),Some("a/b"));
...
fn stalled_request_has_a_total_deadline() {
    ...
    assert_eq!(error.downcast_ref::<Failure>().expect("HTTP failure").status,408);
    assert!(start.elapsed()<Duration::from_secs(5));
}
#[test]
fn socket_binding_preserves_live_and_non_socket_paths_and_recovers_stale_socket() {
    let temp=Temp::new();let path=temp.0.join("agent.sock");
    ...
    drop(listener);drop(guard);
    let stale=UnixListener::bind(&path).expect("stale bind");drop(stale);
    let (listener,guard)=bind(&path).expect("recover stale");
    drop(listener);drop(guard);assert!(!path.exists());
}
#[test]
fn failed_replay_preserves_durable_entry_for_next_startup() {
    let temp=Temp::new();let queue=Queue::open(temp.0.join("queue")).expect("queue");
    {
        let mut shared=queue.lock_shared(Duration::from_millis(100)).expect("shared");
        shared.enqueue(&DeleteRequest {container_id:"sandbox".into(),ifname:"eth0".into(),netns:None,delegated_ipam:false}).expect("enqueue");
        assert!(queue.lock_exclusive(Duration::from_millis(20)).is_err());
    }
    let exclusive=queue.lock_exclusive(Duration::from_millis(100)).expect("exclusive");
    assert!(replay_pending(&exclusive, |_| Err("injected teardown failure".into())).is_err());
    assert_eq!(exclusive.entries().expect("entries").len(),1);
    let mut called=false;
    replay_pending(&exclusive, |request| {assert!(matches!(request,ReplayRequest::Attachment {container_id,ifname} if container_id=="sandbox" && ifname=="eth0"));called=true;Ok(())}).expect("retry");
    assert!(called);assert!(exclusive.entries().expect("entries").is_empty());
}
#[test]
fn configuration_excludes_gateways_and_validates_families_and_mtu() {
    ...
    assert!(ipam.allocate("198.18.0.1".parse().expect("IP"),"pod").is_err());
    config["route-mtu"]=json!(1600);
    fs::write(&path,serde_json::to_vec(&config).expect("JSON")).expect("config");
    assert!(Config::read(&path).is_err());
    config["route-mtu"]=json!(1450);config["ipv4-pool"]=json!("2001:db8::/64");
    fs::write(&path,serde_json::to_vec(&config).expect("JSON")).expect("config");
    assert!(Config::read(&path).is_err());
}
