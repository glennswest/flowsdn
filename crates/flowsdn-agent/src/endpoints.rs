...
fn mac(value: &str) -> Result<u64> {
    let bytes = value.split(':').map(|v| u8::from_str_radix(v,16)).collect::<std::result::Result<Vec<_>,_>>()?;
...
fn addresses(document: &Value) -> Result<Vec<IpAddr>> {
    ...
    for key in ["IPv6", "IPv4"] {
        ...
        if !raw.is_empty() {
            ...
            if ip.is_ipv4() != (key=="IPv4") { return Err("endpoint address family mismatch".into()); }
            addresses.push(ip);
        }
    }
    if addresses.is_empty() { return Err("endpoint has no address".into()); }
    Ok(addresses)
}
fn info(record: &Record) -> Result<EndpointInfo> {
    Ok(EndpointInfo {
        ifindex: record.document.get("IfIndex").and_then(Value::as_u64).and_then(|n| u32::try_from(n).ok()).ok_or("invalid host ifindex")?,
        lxc_id:record.id, mac:mac(text(&record.document,"LXCMAC"))?,node_mac:mac(text(&record.document,"NodeMAC"))?,
...
impl Manager {
    ...
    pub fn restore(path: &Path, object: &Path, ipam: Ipam) -> Result<Self> {
        ...
        let mut manager = Self { store, ids:IdPool::default(), records:BTreeMap::new(),
            addresses:BTreeMap::new(), driver:kernel(LocalDelivery::load(object))?, ipam };
        ...
        for record in &records {
            ...
            let owner = format!("{}/{}",text(&record.document,"K8sNamespace"),text(&record.document,"K8sPodName"));
            for ip in addresses(&record.document)? {
                if manager.addresses.insert(ip,record.attachment.clone()).is_some() { return Err("duplicate restored endpoint address".into()); }
    ...
    pub fn ipam_mut(&mut self) -> &mut Ipam { &mut self.ipam }
    pub fn get(&self, attachment: &str) -> Option<&Record> { self.records.get(attachment) }
    ...
    pub fn create(&mut self, mut document: Value) -> Result<u16> {
        ...
        let result = (|| {
            document.as_object_mut().ok_or("endpoint document must be an object")?.insert("ID".into(),json!(id));
            let record = Record::parse(document)?;
            if self.records.contains_key(&record.attachment) { return Err("endpoint attachment already exists".into()); }
            let ips = addresses(&record.document)?;
            let owner = format!("{}/{}",text(&record.document,"K8sNamespace"),text(&record.document,"K8sPodName"));
            for ip in &ips {
                let scope = if ip.is_ipv4() { self.ipam.ipv4() } else { self.ipam.ipv6() }.ok_or("endpoint family is disabled")?;
                if self.addresses.contains_key(ip) || scope.dump().get(ip) != Some(&owner) {
                    return Err("endpoint address is not exclusively allocated to this owner".into());
                }
            }
            let endpoint = info(&record)?;
            let link = Connector::open()?.require_link(text(&record.document,"IfName"))?;
            if link.index != endpoint.ifindex || mac_value(&link.mac)? != endpoint.node_mac { return Err("endpoint host link identity mismatch".into()); }
            ...
            for ip in ips { self.addresses.insert(ip,record.attachment.clone()); }
            self.records.insert(record.attachment.clone(),record);
            Ok(id)
        })();
        if result.is_err() { self.ids.release(id); }
        result
    }
    pub fn delete(&mut self, attachment: &str) -> Result<bool> {
        let Some(record) = self.records.get(attachment).cloned() else { return Ok(false); };
        ...
        for ip in addresses(&record.document)? { self.ipam.release(ip)?; self.addresses.remove(&ip); }
        self.records.remove(attachment); self.ids.release(record.id); Ok(true)
    }
    fn install(&mut self, record: &Record) -> Result<()> { install(&mut self.driver,record) }
}
...
fn install(driver: &mut LocalDelivery, record: &Record) -> Result<()> {
    ...
    let outcome = (|| {
        for ip in addresses(&record.document)? { kernel(driver.upsert(ip,endpoint))?; installed.push(ip); }
        kernel(driver.attach(text(&record.document,"IfName")))
    })();
    if outcome.is_err() { for ip in installed { let _ = driver.remove(ip); } }
    outcome
}
fn uninstall(driver: &mut LocalDelivery, record: &Record) -> Result<()> {
    ...
    for ip in addresses(&record.document)? { kernel(driver.remove_if_present(ip))?; }
    Ok(())
}
