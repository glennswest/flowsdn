...
fn resource(service: Value) -> Value {
    json!({"spec":{"service":service}})
}
#[test]
fn lb_only_fields_are_rejected_even_when_boolean_false_or_ranges_empty() {
    for (name, value) in [
        ("loadBalancerClass", json!("example.com/lb")),
        ("loadBalancerSourceRanges", json!([])),
        ("allocateLoadBalancerNodePorts", json!(false)),
    ] {
...
fn ip_family_policy_validates_combinations_not_only_enum_values() {
    for (policy, families, valid) in [
        ("SingleStack", json!(["IPv4"]), true),
        ("SingleStack", json!(["IPv6"]), true),
        ("SingleStack", json!(["IPv4", "IPv6"]), false),
        ("PreferDualStack", json!(["IPv6"]), true),
        ("PreferDualStack", json!(["IPv6", "IPv4"]), true),
        ("RequireDualStack", json!(["IPv4"]), true),
        ("RequireDualStack", json!(["IPv4", "IPv6"]), true),
        ("RequireDualStack", json!([]), true),
        ("RequireDualStack", json!(["IPv4", "IPv4"]), false),
        ("Unknown", json!(["IPv4"]), false),
        ("SingleStack", json!(["IPv5"]), false),
        ("PreferDualStack", json!(["IPv4", "IPv6", "IPv4"]), false),
    ] {
        assert_eq!(
            config::validate(&resource(
                json!({"ipFamilyPolicy":policy,"ipFamilies":families})
            ))
            .is_empty(),
            valid,
            "{policy} {families}"
...
fn access_log_format_requires_corresponding_populated_payload() {
    for (entry, valid) in [
        ...
        (
            json!({"format":"JSON","json":{"path":"%REQ(:PATH)%"}}),
            true,
        ),
        ...
        (
            json!({"format":"Text","text":"x","json":{"unused":"default"}}),
            true,
        ),
...
fn malformed_shapes_and_ranges_cannot_be_accepted() {
    for input in [
        ...
        json!({"spec":[]}),
        resource(json!(42)),
        resource(json!({"type":5})),
        resource(json!({"ipFamilies":"IPv4"})),
        resource(json!({"allocateLoadBalancerNodePorts":"false"})),
        resource(json!({"loadBalancerSourceRanges":["10.0.0.0/33","::/129"]})),
        json!({"spec":{"telemetry":{"accessLogs":{}}}}),
        json!({"spec":{"telemetry":{"accessLogs":[]}}}),
        json!({"spec":{"telemetry":{"accessLogs":[null]}}}),
    ] {
        assert!(!config::validate(&input).is_empty(), "{input}");
        assert_eq!(
            config::accepted_condition(&input, 7).get("reason"),
            Some(&json!("InvalidParameters"))
        );
    }
    assert!(
        config::validate(&resource(
            json!({"loadBalancerSourceRanges":["10.0.0.0/8","2001:db8::/32"]})
        ))
        .is_empty()
    );
}
...
fn validation_condition_tracks_acceptance_and_generation_without_mutating_resource() {
    ...
    assert_eq!(
        config::accepted_condition(&input, 9),
        json!({"type":"Accepted","status":"False","reason":"InvalidParameters","message":"Invalid GatewayClassConfig","observedGeneration":9})
    );
    assert_eq!(input, before);
    assert_eq!(
        config::accepted_condition(&json!({"spec":{}}), 10),
        json!({"type":"Accepted","status":"True","reason":"Accepted","message":"Valid GatewayClassConfig","observedGeneration":10})
    );
}
#[test]
fn redirect_keeps_headers_and_queries_separate_including_equal_names() {
    ...
    let query_parameters = vec![QueryMatch {
        name: "token".into(),
...
fn reason_strings_preserve_reference_quirks_but_not_config_validation_bug() {
    for (kind, expected) in [
        (RouteKind::Http, "InvalidHTTPRoute"),
        (RouteKind::Grpc, "InvalidGRPCRoute"),
        (RouteKind::Tls, "InvalidTLSRoute"),
        (RouteKind::Tcp, "InvalidTCPRoute"),
        (RouteKind::Udp, "InvalidUDPRoute"),
    ] {
    ...
    for (kind, reason) in [
        ...
        for success in [true, false] {
            ...
            assert_eq!(
                ...
                Some(&json!(if success { "True" } else { "False" }))
            );
        }
    }
}
...
fn conformance_plan_detects_drift_and_cannot_advertise_unimplemented_targets() {
    ...
    let all = golden
        .iter()
        .cloned()
        .chain(conformance::EXEMPT_FEATURES.iter().map(|s| (*s).to_owned()))
        .collect::<BTreeSet<_>>();
    let target = conformance::checked_target(&all, &golden).expect("target");
    assert_eq!(
        conformance::supported_features(&target, &golden).expect("implemented"),
        vec!["Gateway", "HTTPRoute"]
    );
    ...
    drift.insert("NewFeature".into());
    assert_eq!(
        conformance::checked_target(&drift, &golden),
        Err(conformance::Error::GoldenMismatch)
    );
    drift.remove("ListenerSet");
    assert!(matches!(
        conformance::checked_target(&drift, &golden),
        Err(conformance::Error::ExemptionNotInCatalogue(_))
    ));
    for list in [
        conformance::EXEMPT_FEATURES,
        conformance::SKIPPED_TESTS,
