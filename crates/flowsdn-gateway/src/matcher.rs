...
impl HttpsRedirect {
    ...
    pub fn envoy_route(&self, path: &StringMatch) -> Result<serde_json::Value, &'static str> {
        ...
        if self.status_code != 301 { return Err("force HTTPS requires status301"); }
        if let StringMatch::Exact(value) | StringMatch::Prefix(value) = path {
            if !value.is_empty() && !value.starts_with('/') { return Err("route path must start with slash"); }
        }
        let mut matched = serde_json::Map::new();
        match path {
            StringMatch::Exact(value) if !value.is_empty() => { matched.insert("path".into(),json!(value)); }
            StringMatch::Prefix(value) if !value.trim_end_matches('/').is_empty() => { matched.insert("pathSeparatedPrefix".into(),json!(value.trim_end_matches('/'))); }
            StringMatch::Regex(value) if !value.is_empty() => { matched.insert("safeRegex".into(),json!({"regex":value})); }
            _ => { matched.insert("prefix".into(),json!("/")); }
        }
        let headers = self.matches.headers.iter().map(|m| {
            if m.name.is_empty() { return Err("empty header matcher name"); }
        ...
        let queries = self.matches.query_parameters.iter().map(|m| {
            if m.name.is_empty() { return Err("empty query matcher name"); }
            let mut entry = serde_json::Map::from_iter([("name".into(),json!(m.name))]);
            let (field,value)=project_string(&m.value);
            entry.insert(field.into(),value);
            Ok(Value::Object(entry))
        }).collect::<Result<Vec<_>,_>>()?;
        if !headers.is_empty() { matched.insert("headers".into(),Value::Array(headers)); }
        if !queries.is_empty() { matched.insert("queryParameters".into(),Value::Array(queries)); }
