...
fn main() -> Result<()> {
    ...
    if args.len() != 1 {
        return Err("usage: flowsdn-bpftest PATH_TO_SMOKE_OBJECT (requires network/BPF privileges)".into());
    }
    ...
    require(std::fs::read_link("/proc/self/ns/net")? != previous, "network namespace did not change")?;
    require(Command::new("ip").args(["link", "set", "lo", "up"]).status()?.success(), "could not enable isolated loopback")?;

    let mut bpf = Ebpf::load_file(path)?;
    let mut state: Array<MapData, u32> = Array::try_from(bpf.take_map("SMOKE_STATE").ok_or("missing smoke map")?)?;
    require(state.len() == 2, "unexpected smoke map layout")?;
    let program: &mut SchedClassifier = bpf.program_mut("smoke").ok_or("missing smoke program")?.try_into()?;
    program.load()?;

    for size in [64_usize, 128, 1500] {
        let mut input = vec![0_u8; size];
        input.get_mut(12..14).ok_or("short fixture")?.copy_from_slice(&[0x08, 0x00]);
        *input.get_mut(14).ok_or("short fixture")? = 0x45;
        for verdict in [0_u32, 2] {
            ...
            let result = program.test_run(TestRunOptions {
                data_in: Some(&input), data_out: Some(&mut output), repeat: 1,
                ..Default::default()
            })?;
            require(result.return_value == verdict, "incorrect test-run verdict")?;
            require(result.data_size_out == u32::try_from(size)?, "incorrect test-run output length")?;
            require(output.get(..size) == Some(input.as_slice()), "test-run changed packet bytes")?;
            require(state.get(&0, 0)? == u32::try_from(size)?, "BPF map did not observe input length")?;
        }
    }
    ...
    require(SchedClassifier::query_tcx("lo", TcAttachType::Ingress)?.1.is_empty(), "new namespace has unexpected attachments")?;
    let link = program.attach("lo", TcAttachType::Ingress)?;
    require(SchedClassifier::query_tcx("lo", TcAttachType::Ingress)?.1.len() == 1, "TCX attachment missing")?;
    ...
    for payload in [b"first-pass".as_slice(), b"resumed-pass".as_slice()] {
        ...
        require(received.get(..size) == Some(payload), "attached pass changed UDP payload")?;
        ...
        require(state.get(&0, 0)? > 0, "drop packet did not reach classifier")?;
    }
    println!("PASS: live UDP pass, drop and recovery after map updates");

    program.detach(link)?;
    require(SchedClassifier::query_tcx("lo", TcAttachType::Ingress)?.1.is_empty(), "TCX link leaked after detach")?;
    state.set(0, 0, 0)?; // Keep the drop rule: detached packets must still arrive.
    sender.send(b"detached")?;
    let size = receiver.recv(&mut received)?;
    require(received.get(..size) == Some(b"detached".as_slice()), "traffic failed after detach")?;
    ...
    drop(bpf);
    require(SchedClassifier::query_tcx("lo", TcAttachType::Ingress)?.1.is_empty(), "TCX link leaked after owner drop")?;
    drop(state);
