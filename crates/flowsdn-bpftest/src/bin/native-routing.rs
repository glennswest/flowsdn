...
fn ip(args: &[&str]) -> Result<String> {
    ...
    ensure(output.status.success(), &format!("ip {}: {}", args.join(" "),
...
fn address(id: u8, v6: bool, host: bool) -> String {
    ...
    if v6 { format!("2001:db8:{id}::{last}") } else { format!("198.18.{id}.{last}") }
}

fn underlay(id: u8, v6: bool) -> String {
    if v6 { format!("2001:db8:ff::{id}") } else { format!("192.0.2.{id}") }
}

fn mac(id: u8, kind: u8) -> String { format!("02:00:00:00:{kind:02x}:{id:02x}") }

fn mac_value(id: u8, kind: u8) -> u64 {
    u64::from_le_bytes([2, 0, 0, 0, kind, id, 0, 0])
}

fn peer(id: u8) -> u8 { if id == 1 { 2 } else { 1 } }

fn host_prefix(v6: bool) -> u8 { if v6 { 128 } else { 32 } }

fn family(v6: bool) -> &'static str { if v6 { "-6" } else { "-4" } }
...
impl Worker {
    fn spawn(mode: &str, id: u8, object: &str) -> Result<Self> {
        ...
        let worker = Self { child, input, replies };
...
fn load_driver(object: &str, id: u8) -> Result<LocalDelivery> {
    ...
    for v6 in [false, true] { driver.upsert(address(id, v6, false).parse()?, endpoint)?; }
    driver.attach("p")?;
    driver.attach("u")?;
    Ok(driver)
}
...
fn router(id: u8, object: &str) -> Result<()> {
    ...
    ensure(lines.next().transpose()?.as_deref() == Some("configure"), "missing router setup")?;
    ...
    ensure(output.status.success(), &format!("nft forward isolation: {}", String::from_utf8_lossy(&output.stderr)))?;
    for v6 in [false, true] {
        ...
        ip(&[family(v6), "route", "add",
            &format!("{}/{}", address(id, v6, false), host_prefix(v6)), "dev", "p"])?;
    ...
    for line in lines {
        match line?.as_str() {
            ...
            "detach" => { drop(driver.take().ok_or("already detached")?); },
            "reload" => {
...
fn endpoint(id: u8) -> Result<()> {
    ...
    say("READY")?;
    let mut lines = std::io::stdin().lock().lines();
    ensure(lines.next().transpose()?.as_deref() == Some("configure"), "missing endpoint setup")?;
    ...
    for v6 in [false, true] {
        ...
        ip(&[family(v6), "route", "add",
            &format!("{}/{}", address(id, v6, true), host_prefix(v6)), "dev", "eth0"])?;
        ip(&[family(v6), "route", "add", "default", "via", &address(id, v6, true), "dev", "eth0"])?;
    ...
    for line in lines {
        ...
        let socket = match args.next() {
            Some("4") => &v4, Some("6") => &v6, _ => return Err("bad family".into()),
        };
        match command {
            "send" => {
            ...
            "recv" => {
                ...
                match socket.recv_from(&mut bytes) {
                    Ok((len, _)) => say(&format!("DATA {}", std::str::from_utf8(bytes.get(..len).ok_or("receive overflow")?)?))?,
                    Err(error) if matches!(error.kind(), ErrorKind::TimedOut | ErrorKind::WouldBlock) => say("TIMEOUT")?,
...
fn ports(worker: &mut Worker) -> Result<[u16; 2]> {
    ...
    let ports = [args.next().ok_or("missing v4 port")?.parse()?, args.next().ok_or("missing v6 port")?.parse()?];
    ensure(args.next().is_none(), "extra endpoint response")?;
    Ok(ports)
}

fn exchange(from: &mut Worker, to: &mut Worker, destination: SocketAddr, payload: &str, allowed: bool) -> Result<()> {
    let family = if destination.is_ipv6() { 6 } else { 4 };
    ensure(from.command(&format!("send {family} {destination} {payload}"))? == "SENT", "send failed")?;
    let received = to.command(&format!("recv {family}"))?;
    let expected = if allowed { format!("DATA {payload}") } else { "TIMEOUT".into() };
    ensure(received == expected, &format!("IPv{family} {payload}: expected {expected}, got {received}"))
}
...
fn run(object: &str) -> Result<()> {
    ...
    ensure(r1.command("configure")? == "CONFIGURED", "router 1 configuration failed")?;
    ensure(r2.command("configure")? == "CONFIGURED", "router 2 configuration failed")?;
    ...
    for v6 in [false, true] {
        ...
        ensure(r1.command(&format!("remove{suffix}"))? == "OK", "route removal failed")?;
        exchange(&mut e1, &mut e2, s2, "route-removed", false)?;
        ensure(r1.command(&format!("restore{suffix}"))? == "OK", "route restoration failed")?;
        exchange(&mut e1, &mut e2, s2, "route-restored", true)?;
    }
    println!("PASS: bidirectional IPv4/IPv6 across two BPF hops with kernel FORWARD blocked; route removal drops and restoration recovers");
    ...
    for v6 in [false, true] {
        ...
        exchange(&mut e2, &mut e1, socket(1, ports1, v6)?, "reverse-detached", false)?;
    }
    ensure(r1.command("reload")? == "OK", "reload failed")?;
    for v6 in [false, true] {
        exchange(&mut e1, &mut e2, socket(2, ports2, v6)?, "reloaded", true)?;
        exchange(&mut e2, &mut e1, socket(1, ports1, v6)?, "reverse-reloaded", true)?;
    }
    println!("PASS: detached BPF cannot fall back to kernel forwarding; fresh object/map reload restores both directions");
    Ok(())
}
