...
fn packet(v6: bool, local: bool) -> Result<Vec<u8>> {
    ...
    if v6 {
        ...
        for bytes in frame.get(14..34).ok_or("header")?.chunks_exact(2) {
            sum = sum
                .checked_add(u32::from(u16::from_be_bytes(bytes.try_into()?)))
...
fn run(program: &SchedClassifier, input: &[u8], expected: u32) -> Result<Vec<u8>> {
    ...
    let result = program.test_run(TestRunOptions {
        ...
        &format!("verdict {}, expected {expected}", result.return_value),
    )?;
...
fn main() -> Result<()> {
    ...
    let endpoint = EndpointInfo {
        ifindex: 1,
        mac: 0x020100000002,
        node_mac: 0x020000000002,
        ..Default::default()
    };
    endpoints.insert(
        EndpointKey::v4([198, 18, 0, 2], 0, 0).to_bytes(),
        endpoint.to_bytes(),
        0,
    )?;
    endpoints.insert(
        EndpointKey::v6("2001:db8::2".parse::<std::net::Ipv6Addr>()?.octets(), 0, 0).to_bytes(),
        endpoint.to_bytes(),
        0,
    )?;
    let program: &mut SchedClassifier = bpf
        .program_mut("uplink_ingress")
        .ok_or("uplink program")?
        .try_into()?;
    program.load()?;
    for ethertype in [[8, 6], [0x88, 0xcc], [0x81, 0x00]] {
        let mut frame = vec![0; 80];
        put(&mut frame, 12, &ethertype)?;
        ensure(
            run(program, &frame, 0)? == frame,
            "non-IP pass mutated frame",
        )?;
    }
    for v6 in [false, true] {
        let remote = packet(v6, false)?;
        ensure(
            run(program, &remote, 0)? == remote,
            "unowned IP pass mutated frame",
        )?;
        let local = packet(v6, true)?;
        let forwarded = run(program, &local, 7)?;
        ensure(
            forwarded.get(if v6 { 21 } else { 22 }) == Some(&63),
            "hop reduction missing",
        )?;
        ensure(
            forwarded.get(..12) == Some([2, 0, 0, 0, 1, 2, 2, 0, 0, 0, 0, 2].as_slice()),
            "endpoint MAC rewrite",
        )?;
        for hop in [0, 1] {
            let mut bad = local.clone();
            put(&mut bad, if v6 { 21 } else { 22 }, &[hop])?;
            run(program, &bad, 2)?;
        }
        let mut bad = local.clone();
        put(&mut bad, 14, &[0x75])?;
        run(program, &bad, 2)?;
        let mut bad = local.clone();
        put(&mut bad, if v6 { 18 } else { 16 }, &[0xff, 0xff])?;
        run(program, &bad, 2)?;
        let mut bad = local.clone();
        put(&mut bad, if v6 { 18 } else { 16 }, &[0, 0])?;
        run(program, &bad, 2)?;
        let mut truncated = local.clone();
        truncated.truncate(if v6 { 54 } else { 34 });
        run(program, &truncated, 2)?;
        if !v6 {
            let mut bad = local.clone();
            put(&mut bad, 14, &[0x44])?;
            run(program, &bad, 2)?;
        ...
        let key = if v6 {
            EndpointKey::v6("2001:db8::2".parse::<std::net::Ipv6Addr>()?.octets(), 0, 0)
        } else {
            EndpointKey::v4([198, 18, 0, 2], 0, 0)
        };
        endpoints.insert(key.to_bytes(), EndpointInfo::default().to_bytes(), 0)?;
        run(program, &local, 2)?;
        endpoints.remove(&key.to_bytes())?;
        ensure(
            run(program, &local, 0)? == local,
            "deleted endpoint must return to stack",
        )?;
    }
    ...
    put(
        &mut ndp,
        38,
        &"ff02::1:ff00:2".parse::<std::net::Ipv6Addr>()?.octets(),
    )?;
    put(&mut ndp, 54, &[135, 0])?;
    ensure(run(program, &ndp, 0)? == ndp, "NDP pass mutated frame")?;
    println!(
        "PASS uplink ARP/non-IP/NDP/unowned-IP pass; known IPv4/IPv6 endpoint redirect, malformed/hop/invalid endpoint drop; deletion pass"
    );
    Ok(())
}
