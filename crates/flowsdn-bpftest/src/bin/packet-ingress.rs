...
mod raw {
    ...
    impl Socket {
        pub fn new(interface: &str) -> Result<Self> {
            ...
            address.sll_family = u16::try_from(libc::AF_PACKET)?;
            address.sll_protocol = 0x0800_u16.to_be();
            address.sll_ifindex = i32::try_from(index)?;
            address.sll_halen = 6;
            address
                .sll_addr
                .get_mut(..6)
                .ok_or("address size")?
                .copy_from_slice(&[2, 0, 0, 0, 0, 1]);
            // SAFETY: address and its full checked size remain valid for bind.
            let result = unsafe {
                libc::bind(
                    fd.as_raw_fd(),
                    (&raw const address).cast(),
                    u32::try_from(size_of::<libc::sockaddr_ll>())?,
                )
            };
            if result < 0 {
                return Err(std::io::Error::last_os_error().into());
        ...
        pub fn send(&self, frame: &[u8]) -> Result<()> {
            ...
            ensure(usize::try_from(sent)? == frame.len(), "short raw send")
        }
        pub fn capture(&self, expected: &[u8]) -> Result<()> {
            ...
            ensure(ready == 1, "packet capture timed out")?;
            ...
            if size < 0 {
                ...
                frame.get(..usize::try_from(size)?) == Some(expected),
                "captured frame differs or was truncated",
            )
        }
    }
}
fn packet(size: usize, port: u16, tag: u8) -> Result<Vec<u8>> {
    ...
    header
        .get_mut(10..12)
        .ok_or("checksum")?
        .copy_from_slice(&(!u16::try_from(checksum)?).to_be_bytes());
    frame
        .get_mut(34..36)
        .ok_or("source port")?
        .copy_from_slice(&40000_u16.to_be_bytes());
    frame
        .get_mut(36..38)
...
fn absent(receiver: &UdpSocket) -> Result<()> {
    let mut bytes = [0; 1600];
    match receiver.recv(&mut bytes) {
        Err(error) if matches!(error.kind(), ErrorKind::WouldBlock | ErrorKind::TimedOut) => Ok(()),
        Err(error) => Err(error.into()),
        Ok(_) => Err("packet unexpectedly reached UDP receiver".into()),
    }
}
fn main() -> Result<()> {
    let object = std::env::args_os()
        .nth(1)
        .ok_or("usage: packet-ingress PATH_TO_SMOKE_OBJECT")?;
    ...
    ip(&[
        "link", "add", "target0", "type", "veth", "peer", "name", "inject0",
    ])?;
    for (name, mac) in [
        ("target0", "02:00:00:00:00:01"),
        ("inject0", "02:00:00:00:00:02"),
    ] {
    ...
    let receiver = UdpSocket::bind("192.0.2.1:0")?;
    ...
    let mut state: Array<MapData, u32> =
        Array::try_from(bpf.take_map("SMOKE_STATE").ok_or("smoke map missing")?)?;
    let program: &mut SchedClassifier = bpf
        .program_mut("smoke")
        .ok_or("smoke program missing")?
        .try_into()?;
    program.load()?;
    ensure(
        SchedClassifier::query_tcx("target0", TcAttachType::Ingress)?
            .1
            .is_empty(),
        "unexpected initial attachment",
    )?;
    ...
    for size in [64, 128, 1500] {
        ...
        let direct = packet(size, port, 1)?;
        let target = raw::Socket::new("target0")?;
        let capture = raw::Socket::new("inject0")?;
        state.set(0, 0, 0)?;
        state.set(1, 0, 0)?;
        target.send(&direct)?;
        ...
        absent(&receiver)?;
            ...
            state.get(&0, 0)? == 0,
            "send on target unexpectedly ran its ingress classifier",
        )?;
        println!(
            "PASS size={size}: target send bypasses target TCX ingress, peer captured exact frame"
        );
        for (verdict, tag) in [(0, 2), (2, 3), (0, 4)] {
            let frame = packet(size, port, tag)?;
            let sender = raw::Socket::new("inject0")?;
            state.set(0, 0, 0)?;
            state.set(1, verdict, 0)?;
            sender.send(&frame)?;
            if verdict == 0 {
                let mut payload = [0; 1600];
                let got = receiver.recv(&mut payload)?;
                ensure(payload.get(..got) == frame.get(42..), "UDP payload changed")?;
            } else {
                absent(&receiver)?;
            }
            ensure(
                state.get(&0, 0)? == u32::try_from(size)?,
                "peer injection did not reach target classifier with exact frame length",
            )?;
    ...
    program.detach(link)?;
        ...
        SchedClassifier::query_tcx("target0", TcAttachType::Ingress)?
            .1
            .is_empty(),
        "TCX link did not detach",
    )?;
    println!(
        "PASS: AF_PACKET injection direction, UDP pass/drop/recovery, map observations, capture and detach"
    );
    Ok(())
}
