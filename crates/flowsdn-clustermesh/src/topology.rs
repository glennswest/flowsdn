...
pub struct PodCidrs {
    clusters: BTreeMap<String, Vec<CidrPrefix>>,
}
impl PodCidrs {
    pub fn replace(&mut self, cluster: &str, prefixes: Vec<CidrPrefix>) -> Result<(), Error> {
        ...
        for (peer, existing) in &self.clusters {
            ...
            for candidate in &prefixes {
                if existing.iter().any(|prefix| {
                    prefix.contains_prefix(*candidate) || candidate.contains_prefix(*prefix)
                }) {
                    return Err(Error(format!(
                        "PodCIDR overlap between {cluster} and {peer}"
                    )));
                }
            }
        }
    ...
    pub fn remove(&mut self, cluster: &str) -> Option<Vec<CidrPrefix>> {
        self.clusters.remove(cluster)
    }
    pub fn get(&self, cluster: &str) -> Option<&[CidrPrefix]> {
