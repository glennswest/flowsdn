...
fn identity_defaults_and_explicit_backend_restrictions() {
    ...
    assert_eq!(IdentityMode::parse("kvstore", "etcd"), Ok(IdentityMode::Kvstore));
    for mode in ["kvstore", "doublewrite-readkvstore", "doublewrite-readcrd", "unknown"] { assert!(IdentityMode::parse(mode, "").is_err()); }
...
fn service_resync_and_deferred_features_are_explicit() {
    assert_eq!(service_plan("prefer-legacy").expect("plan"), ServicePlan { export_legacy:true, export_slices:true, import_legacy:true, import_slices:false });
    for mode in ["prefer-endpointslice", "only-endpointslice", ""] { assert!(service_plan(mode).is_err()); }
    ...
    assert_eq!(resync_interval(Some(Duration::from_secs(42))), Duration::from_secs(42));
    assert_eq!(DeferredFeatures::default().warnings(),vec!["clustermesh-mcs-api-install-crds is not implemented"]);
    assert!(DeferredFeatures { mcs_api:false,mcs_install_crds:false,endpoint_sync:false }.warnings().is_empty());
    assert_eq!(DeferredFeatures { mcs_api:true,mcs_install_crds:true,endpoint_sync:true }.warnings().len(),3);
}
#[test]
fn cidr_replacement_is_atomic_and_family_aware() {
    let mut mesh = PodCidrs::default();
    mesh.replace("east", vec!["10.0.0.9/24".parse().expect("cidr"), "fd00::/64".parse().expect("cidr")]).expect("east");
    mesh.replace("west", vec!["10.1.0.0/16".parse().expect("cidr")]).expect("west");
    let before = mesh.clone();
    for cidr in ["10.0.0.128/25", "10.0.0.0/8", "fd00::1/128", "::/0"] {
        assert!(mesh.replace("west", vec![cidr.parse().expect("cidr")]).is_err());
        assert_eq!(mesh,before);
    }
    mesh.replace("west", vec!["::ffff:10.0.0.9/128".parse().expect("cidr")]).expect("mapped remains IPv6");
    mesh.replace("east", vec!["10.0.0.0/16".parse().expect("cidr"),"10.0.0.0/24".parse().expect("cidr")]).expect("same cluster nested");
    mesh.remove("east");
    mesh.replace("west", vec!["0.0.0.0/0".parse().expect("cidr")]).expect("removed ownership");
}
#[test]
fn remote_ranges_reject_escape_boundaries_and_mutations() {
    ...
    for rpc in [Rpc::Put,Rpc::Delete,Rpc::Txn,Rpc::Lease,Rpc::Auth,Rpc::Other] { assert!(!front.permits(rpc,b"cilium/state/x",None)); }
...
fn instance() -> InstanceUuid { InstanceUuid::parse("7beea67f-6435-4f2a-9d80-c6c9c18e0f01").expect("uuid") }
fn apply(store:&mut BTreeMap<String,Record>, plan:&ClaimPlan, revision:u64)->bool {
    if !plan.guards.iter().all(|guard| store.get(&guard.key)==guard.expected.as_ref()) { return false; }
    let mut next=store.clone();
    for put in &plan.puts { next.insert(put.key.clone(),Record { value:put.value.clone(),mod_revision:revision,lease_id:match put.lease { Lease::None => None, Lease::Config(id) => Some(id) } }); }
    *store=next;
    true
}
#[test]
fn ownership_claim_is_atomic_and_restart_stable() {
    ...
    let replay=plan_claim("east",&InstanceUuid::parse(uuid.as_str()).expect("restart"),owner,config,&desired,1).expect("same");
    ...
    assert_eq!(rebound.puts.first().map(|put| &put.lease),Some(&Lease::Config(2)));
    assert_eq!(rebound.puts.first().map(|put| put.value.as_slice()),config.map(|record| record.value.as_slice()));
    ...
    assert_eq!(rebound_store.get("cilium/cluster-config/east").and_then(|record|record.lease_id),Some(2));
    assert_eq!(rebound_store.get("flowsdn/cluster-owners/east"),owner);
    let value:serde_json::Value=serde_json::from_slice(&config.expect("config").value).expect("json");
    assert_eq!(value.pointer("/capabilities/unknownFutureField"),Some(&json!("preserved")));
    assert_eq!(value.pointer("/capabilities/flowsdnInstanceUUID"),Some(&json!(uuid.as_str())));
    ...
    assert!(plan_claim("east",&other,store.get("flowsdn/cluster-owners/east"),None,&desired,1).is_err());
    let repair=plan_claim("east",&uuid,store.get("flowsdn/cluster-owners/east"),None,&desired,1).expect("repair");
...
fn competing_claims_and_stale_revisions_cannot_overwrite() {
    ...
    let updated=plan_claim("east",&instance(),store.get("flowsdn/cluster-owners/east"),store.get("cilium/cluster-config/east"),&json!({"id":4}),1).expect("updated");
    store.get_mut("cilium/cluster-config/east").expect("config").mod_revision=3;
    let before=store.clone();
...
fn legacy_and_malformed_ownership_fail_closed() {
    let legacy=Record { value:br#"{"id":3}"#.to_vec(),mod_revision:1,lease_id:Some(1) };
    assert!(plan_claim("east",&instance(),None,Some(&legacy),&json!({"id":3}),1).is_err());
    let leased_owner=Record { value:instance().as_str().as_bytes().to_vec(),mod_revision:1,lease_id:Some(1) };
    assert!(plan_claim("east",&instance(),Some(&leased_owner),None,&json!({"id":3}),1).is_err());
    for value in ["", "00000000-0000-0000-0000-000000000000", "7BEEA67F-6435-4F2A-9D80-C6C9C18E0F01"] { assert!(InstanceUuid::parse(value).is_err()); }
    assert!(plan_claim("east",&instance(),None,None,&json!({"id":0}),1).is_err());
    assert!(plan_claim("east",&instance(),None,None,&json!({"id":3,"capabilities":[]}),1).is_err());
}
