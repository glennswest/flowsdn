...
use rtnetlink::{Handle, LinkUnspec, LinkVeth, RouteMessageBuilder, packet_route::{
...
impl Connector {
    ...
    fn run<T>(&self, work: impl Future<Output = Result<T>>) -> Result<T> {
        self.runtime.block_on(async { tokio::time::timeout(Duration::from_secs(5), work).await? })
    }
    ...
    pub fn require_link(&self, name: &str) -> Result<Link> {
        self.link(name)?.ok_or_else(|| format!("interface {name} not found").into())
    }
    ...
    pub fn create_veth(&self, host: &str, peer: &str, mtu: u32) -> Result<Link> {
        validate_name(host)?;
        validate_name(peer)?;
        if host == peer || mtu < 1280 { return Err("distinct names and dual-stack MTU >= 1280 required".into()); }
        self.run(async {
            self.handle.link().add(LinkVeth::new(host, peer).mtu(mtu).build()).execute().await?;
            Ok(())
        })?;
    ...
    pub fn configure(&self, index: u32, name: &str, mac: [u8; 6], mtu: u32) -> Result<()> {
        ...
        self.run(async {
            self.handle.link().set(LinkUnspec::new_with_index(index).name(name).address(mac.to_vec()).mtu(mtu).up().build()).execute().await?;
            Ok(())
        })
    }
    pub fn move_to_namespace(&self, index: u32, namespace: &File) -> Result<()> {
        self.run(async {
            self.handle.link().set(LinkUnspec::new_with_index(index).setns_by_fd(namespace.as_raw_fd()).build()).execute().await?;
            Ok(())
        })
    }
    pub fn delete(&self, name: &str) -> Result<()> {
        let Some(link) = self.link(name)? else { return Ok(()); };
        self.run(async { self.handle.link().del(link.index).execute().await?; Ok(()) })
    }
    pub fn add_address(&self, index: u32, address: IpAddr, prefix: u8) -> Result<()> {
        if prefix > if address.is_ipv4() {32} else {128} { return Err("invalid address prefix".into()); }
        self.run(async {
            let mut request = self.handle.address().add(index, address, prefix);
            if address.is_ipv6() { request.message_mut().header.flags |= AddressHeaderFlags::Nodad; }
            request.execute().await?;
            Ok(())
        })
    }
    pub fn add_route(&self, index: u32, destination: IpAddr, prefix: u8, gateway: Option<IpAddr>, mtu: Option<u32>) -> Result<()> {
        if gateway.is_some_and(|g| g.is_ipv4() != destination.is_ipv4()) || mtu == Some(0) {
            return Err("invalid route gateway family or MTU".into());
        }
        let mut route = RouteMessageBuilder::<IpAddr>::new().destination_prefix(destination, prefix)?.output_interface(index);
        route = if let Some(gateway) = gateway { route.gateway(gateway)? } else { route.scope(RouteScope::Link) };
        let mut message = route.build();
        if let Some(mtu) = mtu { message.attributes.push(RouteAttribute::Metrics(vec![RouteMetric::Mtu(mtu)])); }
        self.run(async { self.handle.route().add(message).execute().await?; Ok(()) })
    }
    pub fn neighbour(&self, index: u32, address: IpAddr, mac: [u8; 6]) -> Result<()> {
        self.run(async {
            self.handle.neighbours().add(index, address).link_layer_address(&mac).state(NeighbourState::Permanent).replace().execute().await?;
            Ok(())
        })
    }
}
...
fn validate_name(name: &str) -> Result<()> {
    if name.is_empty() || name.len() >= 16 || name.contains(['\0', '/', ':']) || name.chars().any(char::is_whitespace) {
...
fn link_info(message: LinkMessage) -> Result<Link> {
    ...
    for attr in message.attributes {
        match attr { LinkAttribute::IfName(value) => name = Some(value), LinkAttribute::Address(value) => mac = value, _ => {} }
    }
    Ok(Link { index: message.header.index, name: name.ok_or("link response has no name")?, mac })
}
...
pub fn in_namespace<T: Send + 'static>(namespace: File, work: impl FnOnce() -> Result<T> + Send + 'static) -> Result<T> {
    std::thread::spawn(move || {
        ...
        result
    }).join().map_err(|_| "namespace worker panicked")?
}
