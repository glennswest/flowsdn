...
impl Default for Cancellation {
    fn default() -> Self { Self(watch::channel(false).0) }
}
impl Cancellation {
...
impl Default for RunOptions {
    fn default() -> Self { Self::with_timeout(Duration::from_secs(60)).expect("default timeout fits an Instant") }
}
impl RunOptions {
    pub fn with_timeout(timeout: Duration) -> Result<Self, CommandError> {
        let deadline = Instant::now().checked_add(timeout).ok_or_else(|| failure("timeout exceeds the clock range"))?;
        Ok(Self { cancellation: Cancellation::default(), deadline, grace: Duration::from_millis(100) })
    }
    pub(crate) fn check(&self) -> Result<(), CommandError> {
        if self.cancellation.is_cancelled() { Err(CommandError::Cancelled) }
        else if Instant::now() >= self.deadline { Err(CommandError::Deadline) }
        else { Ok(()) }
    }
}

fn failure(error: impl std::fmt::Display) -> CommandError { CommandError::Failure(error.to_string()) }

pub(crate) async fn execute(state: &mut State, args: &[String], options: &RunOptions) -> Result<Control, CommandError> {
    options.check()?;
    #[cfg(target_os = "linux")]
    {
...
mod linux {
    ...
    use tokio::{io::{AsyncRead, AsyncReadExt}, process::{Child, Command}, sync::oneshot};

    struct Process { child: Child, group: Option<Pid> }
    impl Drop for Process {
        fn drop(&mut self) {
            if let Some(group) = self.group.take() {
                if !matches!(waitid(WaitId::Pid(group), WaitIdOptions::EXITED | WaitIdOptions::NOHANG | WaitIdOptions::NOWAIT), Err(rustix::io::Errno::CHILD)) {
    ...
    struct Output { stdout: Vec<u8>, stderr: Vec<u8>, result: Result<(), CommandError> }

    pub(super) async fn execute(state: &mut State, args: &[String], options: &RunOptions) -> Result<Control, CommandError> {
        let (program, arguments) = args.split_first().ok_or_else(|| failure("exec requires a program"))?;
        ...
        let program = if program.contains('/') {
            ...
            let path = state.environment.get("PATH").ok_or_else(|| failure("exec requires an explicit script PATH for bare program names"))?;
            path.split(':').map(|part| {
                let directory = PathBuf::from(part);
                if directory.is_absolute() { directory.join(program) } else { cwd.join(directory).join(program) }
            }).find(|path| {
        ...
        command.args(arguments).current_dir(&cwd).env_clear().envs(
            state.environment.iter().filter(|(name, _)| name.as_str() != "/" && name.as_str() != ":")
        ).stdin(Stdio::null()).stdout(Stdio::piped()).stderr(Stdio::piped()).process_group(0);
        let child = command.spawn().map_err(failure)?;
        let pid = child.id().and_then(|id| i32::try_from(id).ok()).and_then(Pid::from_raw)
            .ok_or_else(|| failure("child has no process identity"))?;
        let process = Process { child, group: Some(pid) };
        ...
        let output = receiver.await.map_err(|_| failure("child supervisor terminated"))?;
    ...
    async fn read_output(mut stream: impl AsyncRead + Unpin, bytes: &mut Vec<u8>) -> Result<(), CommandError> {
        let mut chunk = [0u8; 8192];
        loop {
            let count = stream.read(&mut chunk).await.map_err(failure)?;
            if count == 0 { return Ok(()); }
            if bytes.len().saturating_add(count) > MAX_OUTPUT {
                return Err(CommandError::LimitExceeded("exec output exceeds 8 MiB per stream"));
            }
    ...
    async fn interrupted(options: &RunOptions, sender: &mut oneshot::Sender<Output>) -> CommandError {
        tokio::select! {
            biased;
    ...
    async fn supervise(mut process: Process, pid: Pid, options: RunOptions, mut sender: oneshot::Sender<Output>) {
        let Some(stdout_pipe) = process.child.stdout.take() else { return };
        let Some(stderr_pipe) = process.child.stderr.take() else { return };
        let mut stdout = Vec::new();
        let mut stderr = Vec::new();
        let mut result;
        {
            let capture = async { tokio::try_join!(read_output(stdout_pipe, &mut stdout), read_output(stderr_pipe, &mut stderr)).map(|_| ()) };
            tokio::pin!(capture);
            ...
            if matches!(waitid(WaitId::Pid(pid), WaitIdOptions::EXITED | WaitIdOptions::NOHANG | WaitIdOptions::NOWAIT), Err(rustix::io::Errno::CHILD)) {
                process.group = None;
            ...
            if result.is_ok() {
                result = status.and_then(|status| if status.success() { Ok(()) } else { Err(failure(format!("process exited with {status}"))) });
            }
            if !captured {
                match tokio::time::timeout(Duration::from_millis(100), &mut capture).await {
                    Ok(Err(error)) if result.is_ok() => result = Err(error),
                    Err(_) if result.is_ok() => result = Err(failure("child output pipes remained open after cleanup")),
                    _ => {},
                }
            }
        }
        let _ = sender.send(Output { stdout, stderr, result });
    }
}
