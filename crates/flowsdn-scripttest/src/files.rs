...
fn mutable_entry(context: &Context, name: &str) -> Result<PathBuf> {
    ...
    if path.file_name().is_none() {
        return Err(fail("operation requires an entry below WORK"));
    }
    Ok(path)
}
...
pub(crate) fn exists(state: &mut State, args: &[String]) -> Result<Control> {
    ...
    if paths.is_empty() {
        return Err(fail("exists requires paths"));
    }
        ...
        if flags.iter().any(|flag| flag == "--exec") {
            #[cfg(unix)]
            {
                ...
                if metadata.permissions().mode() & 0o111 == 0 {
                    return Err(fail("path has no executable permission bits"));
                }
            }
            #[cfg(not(unix))]
            {
...
pub(crate) fn mv(state: &mut State, args: &[String]) -> Result<Control> {
    ...
    let [old, new] = paths.as_slice() else {
        return Err(fail("mv requires old and new paths"));
    };
    let context = state.context()?;
    context
        .workspace
        .dir
        .rename(
            mutable_entry(context, old)?,
            &context.workspace.dir,
            mutable_entry(context, new)?,
        )
        .map_err(io_error)?;
    Ok(Control::Continue)
}
...
pub(crate) fn chmod(state: &mut State, args: &[String]) -> Result<Control> {
    ...
    let (mode, paths) = words
        .split_first()
        .ok_or_else(|| fail("chmod requires an octal mode and paths"))?;
    ...
    for path in paths {
        ...
        let file = context
            .workspace
            .dir
            .open_with(mutable_entry(context, path)?, &options)
            .map_err(io_error)?;
        ...
        #[cfg(unix)]
        {
            ...
            if metadata.dev() == root.dev() && metadata.ino() == root.ino() {
                return Err(fail(
                    "chmod cannot change the owned WORK root through an alias",
                ));
            }
        }
        set_mode(&file, mode)?;
...
fn set_mode(file: &cap_std::fs::File, mode: u32) -> Result<()> {
    ...
    {
        let _ = (file, mode);
        Err(fail("numeric chmod requires Unix permission semantics"))
    }
}
...
pub(crate) fn symlink(state: &mut State, args: &[String]) -> Result<Control> {
    ...
    let [link, arrow, target] = words else {
        return Err(fail("symlink requires path -> target"));
    };
    if arrow != "->" || target.is_empty() {
        return Err(fail("symlink requires path -> target"));
    }
    if Path::new(target).is_absolute() {
        return Err(fail(
            "absolute symlink targets are not supported by the capability workspace",
        ));
    }
    let context = state.context()?;
    #[cfg(unix)]
    {
        context
            .workspace
            .dir
            .symlink(target, mutable_entry(context, link)?)
            .map_err(io_error)?;
    }
    #[cfg(not(unix))]
    {
        let _ = context;
        return Err(fail("symlink command currently requires Unix"));
    }
    Ok(Control::Continue)
}
...
pub(crate) fn rm(state: &mut State, args: &[String]) -> Result<Control> {
    let (_, paths) = options(args, &[])?;
    if paths.is_empty() {
        return Err(fail("rm requires paths"));
    }
    let context = state.context()?;
...
fn open_removable_dir(parent: &Dir, path: &Path, root: &Dir) -> Result<Dir> {
    ...
    {
        use cap_std::fs::OpenOptionsExt;
        options.custom_flags(libc::O_NONBLOCK | libc::O_DIRECTORY | libc::O_NOFOLLOW);
    }
    #[cfg(target_os = "linux")]
    {
        use cap_std::fs::OpenOptionsExt;
    ...
    let metadata = file.metadata().map_err(io_error)?;
    if !metadata.is_dir() {
        return Err(fail("directory changed during rm"));
    }
    #[cfg(unix)]
    {
        use cap_std::fs::MetadataExt;
...
impl State {
    ...
    pub(crate) fn process_directory(&self) -> Result<(Dir, Context)> {
        ...
        let path = if context.cwd.as_os_str().is_empty() {
            Path::new(".")
        } else {
            &context.cwd
        };
