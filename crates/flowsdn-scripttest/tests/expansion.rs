...
fn quoted_fragments_and_doubled_quotes_remain_literal() {
    ...
    let expanded: Vec<_> = tokens
        ...
        .map(|token| {
            token
                .expand(&environment(), ExpansionMode::Plain, 7)
                .unwrap()
        })
        .collect();
...
fn regex_escaping_applies_only_to_substituted_values() {
    ...
    let pattern = token
        .expand(&environment(), ExpansionMode::Regex, 4)
        .unwrap();
    ...
    assert_eq!(
        token
            .expand(&environment(), ExpansionMode::Plain, 4)
            .unwrap(),
        "^/tmp/a.b+[c](d)/($x|other)$"
    );
}
...
fn malformed_unquoted_braces_report_source_line() {
    ...
    assert_eq!(
        token
            .expand(&environment(), ExpansionMode::Plain, 23)
            .unwrap(),
        "${}"
    );
}
...
fn bounded_expansion_counts_values_quotes_and_regex_escaping_before_append() {
    ...
    assert_eq!(
        expand_text_bounded("$x$x", &env, ExpansionMode::Plain, 7, 8).unwrap(),
        "abcdabcd"
    );
    ...
    assert!(
        token
            .expand_bounded(&env, ExpansionMode::Plain, 7, 9)
            .is_err()
    );
    assert_eq!(
        token
            .expand_bounded(&env, ExpansionMode::Plain, 7, 10)
            .unwrap(),
        "prefixabcd"
    );
}
