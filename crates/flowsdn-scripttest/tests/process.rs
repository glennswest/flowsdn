...
fn state() -> State {
    ...
    state.environment.insert(
        "CHILD".into(),
        env!("CARGO_BIN_EXE_scripttest-fixture").into(),
    );
    state
}
...
fn sync_exec_is_a_harness_error_even_for_negative_assertions() {
    for prefix in ["", "! ", "? "] {
        let error = Engine::new()
            .run(&format!("{prefix}exec ignored"), &mut state())
            .unwrap_err();
...
async fn child_uses_script_cwd_environment_arguments_and_both_buffers() {
    ...
    state
        .environment
        .insert("FIXTURE_VALUE".into(), "hello world".into());
    ...
    assert!(state.stdout.contains(&format!(
        "cwd={}\n",
...
async fn exit_expectations_and_conditions_share_the_existing_engine_contract() {
    ...
    assert!(
        engine
            .run_async("! exec $CHILD exit 0", &mut state, &options)
            .await
            .unwrap_err()
            .message
    ...
    assert!(
        engine
            .run_async("exec $CHILD exit 2", &mut state, &options)
            .await
            .unwrap_err()
            .message
    ...
    assert!(
        engine
            .run_async("exec bare-program", &mut state, &options)
            .await
            .unwrap_err()
            .message
            .contains("explicit script PATH")
    );
}
...
async fn deadline_is_not_an_expected_failure_and_reaps_the_child() {
    ...
    let error = Engine::new()
        .run_async("! exec $CHILD sleep pid", &mut state, &options)
        .await
        .unwrap_err();
...
async fn cancellation_sends_sigint_before_forced_cleanup() {
    ...
    let error = Engine::new()
        .run_async(
            "? exec $CHILD interrupt pid interrupted",
            &mut state,
            &options,
        )
        .await
    ...
    assert_eq!(
        fs::read_to_string(root.join("interrupted")).unwrap(),
        "interrupted"
    );
...
async fn dropping_run_future_still_terminates_and_reaps_child() {
    ...
    let task = tokio::spawn(async move {
        Engine::new()
            .run_async("exec $CHILD sleep pid", &mut state, &RunOptions::default())
            .await
    });
    let pid = ready(&pidfile).await;
    task.abort();
...
async fn output_overflow_is_fatal_and_bounded_even_under_negative_status() {
    let mut state = state();
    let error = Engine::new()
        .run_async("! exec $CHILD spam", &mut state, &RunOptions::default())
        .await
        .unwrap_err();
...
async fn leader_exit_cleans_descendants_holding_output_pipes() {
    ...
    Engine::new()
        .run_async(
            "exec $CHILD descendant descendant-pid",
            &mut state,
            &options,
        )
        .await
...
async fn invalid_utf8_cannot_hide_a_deadline_behind_negative_status() {
    ...
    let options = RunOptions::with_timeout(Duration::from_millis(300)).unwrap();
    let error = Engine::new()
        .run_async("! exec $CHILD invalid-sleep", &mut state, &options)
        .await
        .unwrap_err();
