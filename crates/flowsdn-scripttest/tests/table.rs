...
#[derive(Clone)]
struct Item { id: u8, name: String }
impl Keyed for Item { fn primary_key(&self) -> Key { vec![self.id] } }
impl TableRender for Item {
    fn headers() -> &'static [&'static str] { &["ID", "Name"] }
    fn cells(&self) -> Vec<String> { vec![self.id.to_string(), self.name.clone()] }
}
fn item(id: u8, name: &str) -> Item { Item { id, name: name.into() } }
fn table() -> Arc<Table<Item>> { Arc::new(Table::new(vec![]).unwrap()) }
fn state() -> State { State::with_workspace(&std::env::temp_dir(), &std::env::temp_dir()).unwrap() }
...
async fn show_uses_primary_key_order_and_exact_aligned_headers() {
    ...
    engine.run("db/show --columns=Name,ID items --format table", &mut state).unwrap();
...
async fn bindings_are_independent_and_duplicate_registration_never_replaces() {
    ...
    assert!(first.register_command("db/show", false, |_, _| Ok(Control::Continue)).is_err());
    assert!(first.register_command("db/empty", false, |_, _| Ok(Control::Continue)).is_err());
...
async fn output_file_is_confined_and_invalid_flags_do_not_change_it() {
    ...
    for script in ["db/show items -o actual --columns=name", "db/show items -o actual --format=json", "db/show items -o actual --format=yaml", "db/show items -o actual --columns=ID,ID", "db/show items -o actual --out=other", "db/show items -o", "db/show items --unknown", "db/show items extra"] {
        assert!(engine.run(script, &mut state).is_err(), "{script}");
        assert_eq!(fs::read_to_string(root.join("actual")).unwrap(), "Name\none \n");
    }
    assert!(engine.run("db/show items -o ../outside", &mut state).is_err());
    assert!(engine.run("db/show items -o $DATADIR/outside", &mut state).is_err());
}
...
fn empty_and_unknown_table_arguments_are_explicit_errors() {
    ...
    engine.run("db/empty one two", &mut State::default()).unwrap();
    for script in ["db/empty", "db/empty one missing", "db/show missing", "db/show", "db/show one --columns=", "db/empty --unknown"] {
        assert!(engine.run(script, &mut State::default()).is_err(), "{script}");
    }
    let mut state = State::default();
...
#[derive(Clone)]
struct Malformed { kind: u8 }
impl Keyed for Malformed { fn primary_key(&self) -> Key { vec![self.kind] } }
impl TableRender for Malformed {
    fn headers() -> &'static [&'static str] { &["A", "B"] }
    fn cells(&self) -> Vec<String> {
        match self.kind { 0 => vec!["missing".into()], 1 => vec!["tab\tcell".into(), "x".into()], 2 => vec!["line\ncell".into(), "x".into()], _ => vec!["line\rcell".into(), "x".into()] }
    }
}
#[tokio::test(flavor = "current_thread")]
async fn renderer_validates_cell_count_and_unselected_cell_text() {
    for kind in 0..4 {
        ...
        assert!(engine.run("db/show bad --columns=B", &mut State::default()).is_err());
    }
}

#[derive(Clone)]
struct Padded { id: u8 }
impl Keyed for Padded { fn primary_key(&self) -> Key { vec![self.id] } }
impl TableRender for Padded {
    fn headers() -> &'static [&'static str] { &["Value"] }
    fn cells(&self) -> Vec<String> { vec![if self.id == 0 { "x".repeat(131_072) } else { String::new() }] }
}
#[tokio::test(flavor = "current_thread")]
async fn padding_amplification_is_bounded_before_publication_or_file_write() {
    let table = Arc::new(Table::new(vec![]).unwrap());
    for id in 0..65 { table.insert(Padded { id }).await.unwrap(); }
    let mut engine = Engine::new();
    engine.register_table("wide", table).unwrap();
    let mut state = state();
    let target = state.work_dir().unwrap().join("actual");
    fs::write(&target, "retained").unwrap();
    let error = engine.run("! db/show wide -o actual", &mut state).unwrap_err();
...
async fn whole_section_retry_refreshes_live_table_snapshots() {
    ...
    engine.run_async("# current table\ndb/show items\n* db/empty items", &mut state, &options).await.unwrap();
    assert_eq!(state.stdout, "ID   Name\n");
...
async fn output_writer_rejects_fifo_and_escaping_symlinks_without_blocking() {
    ...
    for name in ["escape", "fifo"] {
        assert!(engine.run(&format!("db/show items -o {name}"), &mut state).is_err());
    }
...
impl Keyed for InvalidHeaders { fn primary_key(&self) -> Key { Vec::new() } }
impl TableRender for InvalidHeaders {
    fn headers() -> &'static [&'static str] { &["Name", "Name"] }
    fn cells(&self) -> Vec<String> { vec![] }
}
#[test]
...
#[derive(Clone)]
struct WideEmpty { id: u16 }
impl Keyed for WideEmpty { fn primary_key(&self) -> Key { self.id.to_be_bytes().to_vec() } }
impl TableRender for WideEmpty {
    fn headers() -> &'static [&'static str] { &["C00", "C01", "C02", "C03", "C04", "C05", "C06", "C07", "C08", "C09", "C10", "C11", "C12", "C13", "C14", "C15", "C16", "C17", "C18", "C19", "C20", "C21", "C22", "C23", "C24", "C25", "C26", "C27", "C28", "C29", "C30", "C31", "C32", "C33", "C34", "C35", "C36", "C37", "C38", "C39", "C40", "C41", "C42", "C43", "C44", "C45", "C46", "C47", "C48", "C49", "C50", "C51", "C52", "C53", "C54", "C55", "C56", "C57", "C58", "C59", "C60", "C61", "C62", "C63"] }
    fn cells(&self) -> Vec<String> { panic!("cell-count limit must reject before allocating row cells") }
}
#[tokio::test(flavor = "current_thread")]
async fn empty_wide_rows_are_bounded_before_allocating_cell_metadata() {
    let table = Arc::new(Table::new(vec![]).unwrap());
    for id in 0..1024 { table.insert(WideEmpty { id }).await.unwrap(); }
    let mut engine = Engine::new();
    engine.register_table("wide", table).unwrap();
