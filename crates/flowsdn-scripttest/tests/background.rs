...
async fn launch_clears_buffers_wait_aggregates_in_launch_order() {
    ...
    Engine::new()
        .run_async("wait", &mut state, &RunOptions::default())
        .await
        .unwrap();
...
async fn wait_flags_and_non_async_background_commands_are_harness_errors() {
    for script in ["! wait --all", "? wait extra", "echo no &", "wait &"] {
        let error = Engine::new()
            ...
            .unwrap_err();
        assert!(
            error.message.contains("wait takes no arguments")
                || error.message.contains("only exec"),
            "{error}"
        );
    }
...
async fn normal_end_stop_and_script_error_cancel_and_reap_pending_children() {
    for ending in ["", "\nstop", "\nunknown-command"] {
        ...
        let error = Engine::new()
            .run_async(
                &format!("exec $CHILD sleep pid &\nexec $CHILD await-file pid{ending}"),
                &mut state,
                &RunOptions::default(),
            )
            .await
...
async fn cancellation_during_wait_cannot_be_hidden_by_job_or_wait_status() {
    ...
    let cancel = tokio::spawn(async move {
        ready(&root.join("pid")).await;
        cancellation.cancel();
    });
    let error = Engine::new()
        .run_async("! exec $CHILD sleep pid &\n! wait", &mut state, &options)
        .await
        .unwrap_err();
    assert!(error.message.contains("cancelled"), "{error}");
    cancel.await.unwrap();
...
async fn spawn_errors_are_deferred_to_the_jobs_own_expected_status() {
    ...
    Engine::new()
        .run_async(
            "! exec /nonexistent/fixture-program &\nwait",
            &mut state,
            &RunOptions::default(),
        )
        .await
    ...
    assert!(
        state
            .log
            .iter()
            .any(|line| line.contains("expected failure"))
    );
}
...
#[tokio::test(flavor = "current_thread")]
async fn retry_reaps_failed_attempt_jobs_and_preserves_prior_section_jobs() {
    let mut state = state();
    let root = state.work_dir().unwrap().to_owned();
