...
fn synthetic_txtar_executes_environment_output_and_stop() {
    ...
    assert!(
        state
            .log
            .iter()
            .any(|message| message == "finished successfully")
    );
}
...
fn conditions_select_commands_and_validate_prefix_arity() {
    ...
    engine
        .register_condition("enabled", false, |state, _| {
            Ok(state.environment.contains_key("enabled"))
        })
        .unwrap();
    engine
        .register_condition("feature", true, |_, suffix| Ok(suffix == "present"))
        .unwrap();
    let mut state = State::default();
...
fn cancellation_and_deadlines_never_satisfy_negative_assertions() {
    let mut engine = Engine::new();
    engine
        .register_command("cancel", false, |_, _| Err(CommandError::Cancelled))
        .unwrap();
    engine
        .register_command("deadline", false, |_, _| Err(CommandError::Deadline))
        .unwrap();
    for command in ["cancel", "deadline"] {
        for prefix in ["", "! ", "? "] {
            let mut state = State::default();
            let error = engine
                .run(&format!("{prefix}{command}\necho must-not-run"), &mut state)
                .unwrap_err();
            assert_eq!(error.line, 1);
...
fn mismatched_status_stops_before_the_next_command() {
    ...
    let mut state = State::default();
    let execution = Engine::new()
        .run("? echo success\n! stdout missing\necho final", &mut state)
        .unwrap();
...
fn unsupported_constructs_and_unknown_commands_cannot_hide_behind_status() {
    ...
    let error = Engine::new()
        .run("# first\necho 'unterminated", &mut State::default())
        .unwrap_err();
    assert_eq!(error.line, 2);
    let error = Engine::new()
        .run("! echo ${broken", &mut State::default())
        .unwrap_err();
...
fn output_publication_and_expected_failures_preserve_buffers() {
    let mut engine = Engine::new();
    engine
        .register_command("produce", false, |state, _| {
            state.publish("out\n", "err\n");
...
fn pattern_substitutions_are_literal_and_flags_select_pattern_argument() {
    ...
    engine
        .run("! stdout --count=2 -- $pattern", &mut state)
        .unwrap();
}
...
fn environment_dump_queries_and_mutations_have_deterministic_output() {
    ...
    let engine = Engine::new();
    engine
        .run("env b=second a=first\nenv a b missing", &mut state)
        .unwrap();
    ...
    assert!(state.environment.contains_key(":"));
    engine
        .run("echo '  shared value  '\nenv --from-stdout a b", &mut state)
        .unwrap();
...
fn registry_collisions_and_execution_budget_are_explicit() {
    ...
    assert!(
        engine
            .register_command("echo", false, |_, _| Ok(Control::Continue))
            .is_err()
    );
    assert!(
        engine
            .register_command("", false, |_, _| Ok(Control::Continue))
            .is_err()
    );
    assert!(
        engine
            .register_condition("linux", false, |_, _| Ok(true))
            .is_err()
    );
    assert!(
        engine
            .register_condition("bad:suffix", true, |_, _| Ok(true))
            .is_err()
    );
    engine.set_max_commands(1);
    let mut state = State::default();
    let error = engine
        .run("echo first\necho second", &mut state)
        .unwrap_err();
    assert_eq!(error.line, 2);
