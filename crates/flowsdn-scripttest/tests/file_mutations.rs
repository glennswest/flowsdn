...
use std::{fs, os::unix::fs::{PermissionsExt, FileTypeExt, symlink}};

fn workspace() -> State { State::with_workspace(&std::env::temp_dir(), &std::env::temp_dir()).unwrap() }
...
fn numeric_modes_reject_symbolic_negative_and_out_of_range_values() {
    ...
    for script in ["chmod u+x item", "chmod 888 item", "chmod -1 item", "chmod 10000 item", "chmod 755", "chmod '' item", "exists --unknown item"] {
        assert!(engine.run(script, &mut state).is_err(), "{script}");
        assert_eq!(fs::metadata(&path).unwrap().permissions().mode() & 0o7777, 0o640);
    }
    engine.run("chmod 000 item\nexists --readonly item\nchmod 0600 item", &mut state).unwrap();
...
fn removal_repairs_unreadable_directories_and_never_follows_symlinks() {
    ...
    let engine = Engine::new();
    engine.run("chmod 000 nested/deeper\nchmod 000 nested\nrm nested\nrm missing", &mut state).unwrap();
    ...
    drop(state);
    assert!(!root.exists(), "owned workspace cleanup repairs directory permissions");
    assert_eq!(fs::read_to_string(sentinel).unwrap(), "keep");
}
...
fn mutations_cannot_touch_datadir_or_the_owned_root() {
    ...
    for script in [
        "rm $DATADIR/sentinel", "chmod 000 $DATADIR/sentinel", "mv $DATADIR/sentinel moved",
        "mv source $DATADIR/sentinel", "symlink $DATADIR/link -> sentinel", "rm $WORK",
        "mv $WORK moved", "chmod 000 $WORK", "chmod 000 root-alias", "rm ..", "rm ../outside",
    ] { assert!(engine.run(script, &mut state).is_err(), "{script}"); }
    assert_eq!(fs::read_to_string(&sentinel).unwrap(), "keep");
    assert_eq!(fs::metadata(&sentinel).unwrap().permissions().mode(), mode);
    assert_eq!(fs::read_to_string(state.work_dir().unwrap().join("source")).unwrap(), "source");
}
...
fn relative_symlinks_are_literal_dangling_links_can_be_removed() {
    ...
    let root = state.work_dir().unwrap().to_owned();
    let engine = Engine::new();
    engine.run("symlink dangling -> not-yet-created\n! exists dangling", &mut state).unwrap();
    assert_eq!(fs::read_link(root.join("dangling")).unwrap().to_str().unwrap(), "not-yet-created");
    engine.run("mv dangling renamed-link\nrm renamed-link", &mut state).unwrap();
    assert!(fs::symlink_metadata(root.join("renamed-link")).is_err());
    engine.run("symlink escape -> ../../outside\n! exists escape\nrm escape", &mut state).unwrap();
    for script in ["symlink bad wrong target", "symlink bad -> /outside", "symlink bad -> ''"] {
...
fn fifo_metadata_rename_chmod_and_removal_do_not_open_io_channels() {
    ...
    Engine::new().run("exists fifo\nchmod 0400 fifo\nexists --readonly fifo\nmv fifo renamed", &mut state).unwrap();
    assert!(fs::metadata(root.join("renamed")).unwrap().file_type().is_fifo());
    Engine::new().run("rm renamed\n! exists renamed", &mut state).unwrap();
}
...
fn double_dash_paths_and_rename_replacement_follow_filesystem_semantics() {
    ...
    engine.run("symlink -- -link -> destination\nexists -- -link\nrm -- -link destination", &mut state).unwrap();
