...
fn only_exact_marker_lines_start_files() {
    let archive = Archive::parse(" -- wrong --\n-- wrong -- \n-- real --\n--not--\n-- next --\n").unwrap();
...
fn quotes_comments_empty_arguments_and_unicode() {
    ...
    assert_eq!(literal, ["echo", "a'b", "ab", "", "# literal", "$x${x}", "café"]);
    let mixed = tokens.iter().find(|token| token.literal() == "$x${x}").unwrap();
    assert_eq!(mixed.0.len(), 2);
    assert!(mixed.0.first().unwrap().quoted);
    assert!(!mixed.0.last().unwrap().quoted);
    assert!(tokens.iter().find(|token| token.literal() == "").unwrap().unquoted().is_none());
...
fn statuses_conditions_background_and_sections() {
    let parsed = parse_script("# header\n  # ignored\n\n!* [linux] [!short] [exec:ip] exec ip &\n'!' echo '&'").unwrap();
    assert_eq!(parsed.len(), 3);
    assert!(matches!(parsed.first(), Some(Line::Section { line: 1, .. })));
    let commands: Vec<_> = parsed.iter().filter_map(|line| match line { Line::Command(command) => Some(command), _ => None }).collect();
    ...
    assert!(first.conditions.iter().any(|condition| condition.name == "short" && condition.negated));
    ...
    for (prefix, expected) in [("", Status::Success), ("! ", Status::Failure), ("? ", Status::SuccessOrFailure), ("* ", Status::SuccessRetry), ("!* ", Status::FailureRetry)] {
        let parsed = parse_script(&format!("{prefix}echo hi")).unwrap();
        let Some(Line::Command(command)) = parsed.first() else { panic!("missing command") };
...
fn malformed_prefixes_report_the_source_line() {
    for text in ["! ? echo", "!", "[linux]", "[] echo", "[!] echo", "[linux echo", "&", "echo 'bad"] {
