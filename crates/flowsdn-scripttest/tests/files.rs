...
fn materialization_preserves_bytes_expands_names_and_cleans_owned_root() {
    let root;
    {
        ...
        state.environment.insert("name".into(), "nested/value".into());
        let archive = Archive::parse("\n-- ${name} --\n$WORK stays literal\n-- duplicate --\nfirst\n-- duplicate --\nlast").unwrap();
        state.materialize(&archive).unwrap();
        assert_eq!(fs::read_to_string(root.join("nested/value")).unwrap(), "$WORK stays literal\n");
        assert_eq!(fs::read_to_string(root.join("duplicate")).unwrap(), "last");
        assert!(root.join("tmp").is_dir());
        assert_eq!(state.environment.get("PWD").unwrap(), root.to_str().unwrap());
...
fn synthetic_archive_executes_file_assertions_copy_and_per_script_cd() {
    ...
    assert_eq!(state.environment.get("PWD").unwrap(), state.work_dir().unwrap().to_str().unwrap());
}

#[test]
fn cmpenv_expands_both_inputs_and_diff_preserves_missing_newline() {
    let mut state = workspace();
    let archive = Archive::parse("env x=value\ncmpenv a b\n! cmp a b\n-- a --\n$x\n-- b --\nvalue\n").unwrap();
    Engine::new().run_archive(&archive, &mut state).unwrap();
    assert!(state.log.iter().any(|entry| entry.contains("--- a\n+++ b\n@@")));
    ...
    assert!(state.log.iter().skip(before).any(|entry| entry.contains("No newline at end of file")));
    let before = state.log.len();
...
fn replace_is_single_pass_and_decodes_byte_and_unicode_escapes() {
    ...
    for command in ["replace '' value original", "replace '\\q' value original", "replace one original"] {
        assert!(Engine::new().run(command, &mut state).is_err());
        assert_eq!(fs::read(state.work_dir().unwrap().join("original")).unwrap(), bytes);
    }
}
...
fn binary_compare_and_copy_are_exact_but_text_commands_reject_invalid_utf8() {
    ...
    Engine::new().run("cp binary duplicate\ncmp binary duplicate", &mut state).unwrap();
    assert_eq!(fs::read(state.work_dir().unwrap().join("duplicate")).unwrap(), [0, 255, 1]);
    assert!(Engine::new().run("cat binary", &mut state).unwrap_err().message.contains("UTF-8"));
    assert!(Engine::new().run("grep pattern binary", &mut state).is_err());
}
...
fn paths_cannot_escape_workspace_or_modify_read_only_data_directory() {
    ...
    assert!(state.materialize(&Archive::parse(&format!("-- {sibling_prefix} --\nnot allowed")).unwrap()).is_err());
    Engine::new().run("cat $DATADIR/sentinel\nstdout '^unchanged$'", &mut state).unwrap();
    for script in ["cp stdout $DATADIR/sentinel", "mkdir ../escaped", "cd .."] {
        assert!(Engine::new().run(script, &mut state).is_err());
    }
    assert_eq!(fs::read_to_string(outside_root.join("sentinel")).unwrap(), "unchanged");
}
...
fn symlink_escapes_are_denied_and_cleanup_does_not_follow_them() {
    ...
    assert!(Engine::new().run("cp stdout escape/sentinel", &mut state).is_err());
    assert!(state.materialize(&Archive::parse("-- escape/sentinel --\nchanged").unwrap()).is_err());
    drop(state);
    assert!(!root.exists());
...
fn file_size_limit_and_unimplemented_flags_fail_explicitly() {
    let mut state = workspace();
    fs::File::create(state.work_dir().unwrap().join("large")).unwrap().set_len(8_388_609).unwrap();
    assert!(Engine::new().run("cat large", &mut state).unwrap_err().message.contains("8 MiB"));
    assert!(Engine::new().run_archive(&Archive::parse("#! --agent-option=true\necho ignored").unwrap(), &mut state).is_err());
    assert!(Engine::new().run("cmp --update large large", &mut state).is_err());
    assert!(Engine::new().run("cat missing", &mut State::default()).unwrap_err().message.contains("working directory"));
}
...
fn cmpenv_caps_repeated_substitutions_while_building_output() {
    ...
    fs::write(state.work_dir().unwrap().join("expanding"), "$x".repeat(4097)).unwrap();
    fs::write(state.work_dir().unwrap().join("expected"), "").unwrap();
    let error = Engine::new().run("cmpenv expanding expected", &mut state).unwrap_err();
...
fn individual_diffs_and_cumulative_grep_logs_have_hard_limits() {
    ...
    let error = Engine::new().run("! cmp left right", &mut state).unwrap_err();
    assert!(error.message.contains("64 KiB"));
    assert!(state.log.is_empty());
    let line = format!("{}\n", "a".repeat(512));
    fs::write(state.work_dir().unwrap().join("matches"), line.repeat(2200)).unwrap();
    let error = Engine::new().run("? grep a matches", &mut state).unwrap_err();
...
fn fifo_reads_writes_and_materialization_fail_without_opening_a_peer() {
    ...
    assert!(state.materialize(&Archive::parse("-- fifo --\ncontent").unwrap()).is_err());
    assert!(state.materialize(&Archive::parse("-- link --\ncontent").unwrap()).is_err());
}
