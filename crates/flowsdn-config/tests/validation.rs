...
use flowsdn_config::{Class, Entry, KeySpec, Kind, Registry, Resolved, Source};

fn config(keys: &[(&str, Kind, &str)]) -> Resolved {
    Registry::new(
        keys.iter()
            .map(|(name, kind, value)| KeySpec::new(*name, kind.clone(), *value)),
    )
    .unwrap()
    .resolve([])
    .unwrap()
...
fn ipcache_size_bounds_and_reserved_fixed_names_are_foundation_errors() {
    ...
    for mapping in [
        ...
        assert_eq!(
            validation::foundation(&configuration).unwrap_err().key,
    ...
    for mapping in ["", "128=tenant-a,255=tenant-b"] {
        assert!(
            validation::foundation(&config(&[("fixed-identity-mapping", Kind::Map, mapping)]))
                .is_ok()
        );
    }
    ...
    assert_eq!(
        resolved.get("bpf-ipcache-map-max").unwrap().value,
        flowsdn_config::Value::UInt(512000)
    );
    assert_eq!(
        resolved.get("bpf-ipcache-map-max").unwrap().class,
        Class::Immutable
    );
    assert_eq!(
        resolved.get("force-config-change").unwrap().value,
        flowsdn_config::Value::Bool(false)
    );
}
...
fn endpoint_id_max_accepts_u16_nonzero_range_and_rejects_invalid_limits() {
    ...
    for value in ["0", "65536", "18446744073709551615"] {
        let error = validation::foundation(&config(&[(
            "endpoint-id-max",
            Kind::UInt { bits: 64 },
            value,
        )]))
    ...
    assert_eq!(
        catalogue
            ...
            .unwrap()
            .value,
        flowsdn_config::Value::UInt(4095)
    );
    assert!(
        catalogue
            .resolve([Entry::new(Source::Flag, "endpoint-id-max", "-1")])
            .is_err()
    );
}
...
fn address_families_and_ndp_dependencies_are_checked() {
    for (v4, v6, ndp, device, expected) in [
        ...
        let config = config(&[
            ("enable-ipv4", Kind::Bool, v4),
            ("enable-ipv6", Kind::Bool, v6),
            ("enable-ipv6-ndp", Kind::Bool, ndp),
            ("ipv6-mcast-device", Kind::String, device),
        ]);
        assert_eq!(
            validation::foundation(&config).err().map(|error| error.key),
            expected.map(str::to_owned)
        );
    }
    let missing = config(&[("enable-ipv4", Kind::Bool, "true")]);
    assert_eq!(
        validation::foundation(&missing).unwrap_err().key,
        "enable-ipv6"
    );
...
fn enums_route_metric_and_ipv6_prefixes_have_named_errors() {
    ...
    assert!(
        validation::foundation(&config(&[("route-metric", Kind::Int { bits: 64 }, "0")])).is_ok()
    );
    assert_eq!(
        validation::foundation(&config(&[("route-metric", Kind::Int { bits: 64 }, "-1")]))
            .unwrap_err()
            .key,
        "route-metric"
    );
    for prefix in ["fd00::/64", "2001:db8::1/64"] {
        ...
        assert!(
            validation::foundation(&config(&[("ipv6-cluster-alloc-cidr", Kind::Cidr, prefix)]))
                .is_ok()
        );
    }
...
fn cluster_names_and_cluster_id_boundaries_match_both_mesh_widths() {
    ...
    for name in [
        ...
        assert_eq!(
            validation::foundation(&config(&[("cluster-name", Kind::String, name)]))
                .unwrap_err()
                .key,
            "cluster-name"
        );
    ...
    for (max, id, valid) in [
        ...
        let config = config(&[
            ("max-connected-clusters", Kind::UInt { bits: 32 }, max),
            ("cluster-id", Kind::UInt { bits: 32 }, id),
        ]);
        assert_eq!(
            validation::foundation(&config).is_ok(),
            valid,
            "max={max} id={id}"
        );
    }
...
fn delegated_ipam_requires_routes_and_disables_health_and_masquerade() {
    for (masquerade, health, routes, valid) in [
        ...
        let config = config(&[
            ("ipam", Kind::String, "delegated-plugin"),
            ("enable-ipv4-masquerade", Kind::Bool, masquerade),
            ("enable-endpoint-health-checking", Kind::Bool, health),
            ("enable-endpoint-routes", Kind::Bool, routes),
        ]);
        assert_eq!(validation::foundation(&config).is_ok(), valid);
    }
    assert!(
        validation::foundation(&config(&[("ipam", Kind::String, "delegated-plugin")])).is_err()
    );
}

#[test]
fn vtep_and_identity_backends_check_active_dependencies() {
    for (enabled, mask, valid) in [
        ("true", "255.255.255.0", true),
        ("true", "garbage", false),
        ("true", "::1", false),
        ("false", "", true),
    ] {
        assert_eq!(
            validation::foundation(&config(&[
                ("enable-vtep", Kind::Bool, enabled),
                ("vtep-mask", Kind::String, mask)
            ]))
            .is_ok(),
            valid
        );
    }
    for (mode, backend, valid) in [
        ("crd", "", true),
        ("kvstore", "etcd", true),
        ("kvstore", "", false),
        ("doublewrite-readcrd", "etcd", false),
    ] {
        assert_eq!(
            validation::foundation(&config(&[
                ("identity-allocation-mode", Kind::String, mode),
                ("kvstore", Kind::String, backend)
            ]))
            .is_ok(),
            valid
        );
...
fn misdeclared_schema_types_cannot_bypass_validation() {
    for (key, kind, value) in [
        ("enable-ipv4", Kind::String, "true"),
        ("route-metric", Kind::String, "-1"),
        ("cluster-name", Kind::Bool, "true"),
        ("bpf-map-dynamic-size-ratio", Kind::Int { bits: 64 }, "1"),
    ] {
        assert_eq!(
            validation::foundation(&config(&[(key, kind, value)]))
                .unwrap_err()
                .key,
            key
        );
...
fn ignored_and_script_values_never_activate_foundation_rules() {
    for class in [Class::Ignored, Class::Script] {
        ...
        assert!(
            registry
                .resolve([Entry::new(Source::Flag, "enable-vtep", "invalid-bool")])
                .is_err(),
            "ignored keys still require valid scalar types"
        );
    }
    ...
    assert_eq!(
        validation::foundation(&resolved).unwrap_err().key,
        "vtep-mask",
        "immutable settings remain active; ignored values cannot satisfy their dependencies"
    );
}
