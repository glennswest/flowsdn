...
fn every_normative_declaration_has_exact_metadata_and_unique_canonical_name() {
    ...
    for line in table.lines().filter(|line| line.starts_with("| `")) {
        ...
        let expected = if class.contains("immutable") {
            Class::Immutable
        } else if class.contains("ignored") {
            Class::Ignored
        } else if class.contains("script") {
            Class::Script
        } else if class.contains("runtime") {
            Class::Runtime
        } else if class.contains("dynamic") {
            Class::Dynamic
        } else {
            Class::Active
        };
        assert_eq!(entry.class, expected, "{name}");
        let inventory_name = class
            .split_once("inventory row `")
...
fn semantic_classification_census_and_aliases_match_foundation_contract() {
    ...
    assert_eq!(
        catalogue::get("install-iptables-rules").unwrap().class,
        Class::Ignored
    );
    assert_eq!(
        catalogue::get("install-no-conntrack-iptables-rules")
            .unwrap()
            .class,
        Class::Active,
        "this flag selects nftables notrack and must not be ignored"
    );
    assert_eq!(
        catalogue::get("hive-start-timeout").unwrap().class,
        Class::Active
    );
    assert_eq!(
        catalogue::get("hive-log-threshold").unwrap().class,
        Class::Ignored
    );
    assert_eq!(
        catalogue::get("dynamic-lifecycle-config")
            .unwrap()
            .inventory_name,
        Some("ConfigKey")
    );
    assert!(
        catalogue::get("ConfigKey").is_none(),
        "inventory constant is not a public alias"
    );
    assert_eq!(catalogue::get("ENABLE_IPV4").unwrap().name, "enable-ipv4");
    assert_eq!(
        catalogue::get("CT_GLOBAL_MAX_ENTRIES_TCP").unwrap().name,
    ...
    assert_eq!(
        catalogue::get("monitor-aggregation")
            .unwrap()
            .aliases()
            .collect::<Vec<_>>(),
...
fn literal_defaults_are_typed_and_evaluated_without_inventing_symbolic_values() {
    ...
    assert_eq!(
        resolved.get("http-request-timeout").unwrap().value,
        Value::UInt(3_600)
    );
    assert_eq!(
        resolved.get("allocator-list-timeout").unwrap().value,
        Value::Duration(180_000_000_000)
    );
    assert_eq!(
        resolved.get("monitor-aggregation-flags").unwrap().value,
        Value::List(vec!["syn".into(), "fin".into(), "rst".into()])
    );
    assert_eq!(
        resolved.get("kvstore-opt").unwrap().value,
        Value::Map(Default::default())
    );
    assert_eq!(resolved.get("debug").unwrap().class, Class::Runtime);
    assert_eq!(
        resolved.get("subnet-topology").unwrap().class,
        Class::Dynamic
    );
}
...
fn complete_schema_fails_closed_then_requires_explicit_typed_resolutions() {
    ...
    let gops = overrides
        .iter_mut()
        .find(|value| value.key == "enable-gops")
        .unwrap();
    gops.value = "invalid";
    assert!(
        matches!(catalogue::complete_registry(&overrides), Err(Error::InvalidValue(error)) if error.key == "enable-gops")
    );
}
...
fn override_validation_rejects_unknown_duplicate_and_unattributed_definitions() {
    for (overrides, error_kind) in [
        ...
        assert!(matches!(
            (error_kind, error),
            ("unknown", Error::UnknownOverride(_))
                | ("provenance", Error::MissingProvenance(_))
                | ("duplicate", Error::DuplicateOverride(_))
        ));
    }
...
fn runtime_snapshot_retains_runtime_and_dynamic_classification() {
    ...
    let resolved = built.registry.resolve([]).unwrap();
    let metadata = flowsdn_config::runtime::Metadata {
        version: "schema-test".into(),
        written_at: "2026-09-09T01:32:26Z".into(),
    };
    ...
    assert_eq!(
        decoded.resolved.get("subnet-topology").unwrap().class,
        Class::Dynamic
    );
    assert_eq!(
        decoded
            .resolved
            .get("install-iptables-rules")
            .unwrap()
            .class,
        Class::Ignored
    );
