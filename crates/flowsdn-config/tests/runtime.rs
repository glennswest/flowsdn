...
use flowsdn_config::{Class, Entry, Kind, KeySpec, Registry, Source, Value};
...
impl Fixture {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!("flowsdn-runtime-test-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
...
fn metadata(version: &str) -> Metadata {
    Metadata { version: version.into(), written_at: "2026-09-08T21:16:40Z".into() }
}

fn schema() -> Registry {
    let mut immutable = KeySpec::new("routing-mode", Kind::Enum(vec!["native".into(), "tunnel".into()]), "tunnel");
...
fn snapshot_roundtrip_preserves_typed_values_sources_and_unknown_raw_values() {
    ...
    assert_eq!(resolved.unknown_values().get("unknown-key").unwrap().source, Source::Env);
    ...
    for (key, effective) in resolved.values() {
        if effective.class == Class::Script { assert!(decoded.resolved.get(key).is_none()); }
        else { assert_eq!(decoded.resolved.get(key), Some(effective), "{key}"); }
    }
    ...
    assert_eq!(json.get("config").unwrap().get("delay"), Some(&json!(-1_500_000_000_i64)));
...
fn decoder_validates_schema_metadata_sources_and_immutable_membership() {
    ...
    for (field, replacement) in [
        ("written-at", json!("not-a-timestamp")), ("flowsdn-version", json!("")),
        ("reference-compat", json!("unknown")), ("immutable-keys", json!(["missing"])),
        ...
        malformed.as_object_mut().unwrap().insert(field.into(), replacement);
        assert!(runtime::decode(&malformed.to_string(), &registry).is_err(), "{field}");
    }
    let mut malformed: serde_json::Value = serde_json::from_str(&encoded).unwrap();
    malformed.get_mut("config").unwrap().as_object_mut().unwrap().insert("unsigned".into(), json!("123"));
...
fn timestamps_require_real_calendar_dates_and_explicit_timezones() {
    ...
    for timestamp in ["2024-02-29T23:59:59.123+05:30", "2026-09-08t21:16:40z", "2026-09-08T21:16:40-00:00"] {
        assert!(runtime::encode(&resolved, &Metadata { version: "test".into(), written_at: timestamp.into() }).is_ok());
    }
    for timestamp in ["2026-02-29T00:00:00Z", "2026-04-31T00:00:00Z", "2026-09-08T24:00:00Z", "2026-09-08T21:16:40", "2026-09-08T21:16:40+99:00"] {
        assert!(runtime::encode(&resolved, &Metadata { version: "test".into(), written_at: timestamp.into() }).is_err());
    }
}
...
fn decoding_never_fills_new_defaults_or_drops_removed_immutable_keys() {
    ...
    assert_eq!(removed.resolved.get("routing-mode").unwrap().class, Class::Immutable);
    assert!(immutable::check(immutable::Previous::Parsed(&removed.resolved), &empty_schema.resolve([]).unwrap(), true, true).is_err());
    ...
    assert!(immutable::check(immutable::Previous::Parsed(&decoded.resolved), &resolved, true, true).is_err());
}
...
fn publication_rotates_three_snapshots_and_cleans_staging_files() {
    ...
    for (name, version) in [(runtime::CURRENT, "four"), (runtime::PREVIOUS, "three"), (runtime::OLDEST, "two")] {
        let snapshot = runtime::decode(&fs::read_to_string(fixture.path().join(name)).unwrap(), &registry).unwrap();
...
fn store_failures_are_nonfatal_and_leave_no_partial_snapshot() {
    ...
    let registry = schema();
    let resolved = registry.resolve([]).unwrap();
    let missing = runtime::store(&fixture.path().join("missing"), &resolved, &metadata("test"));
...
fn rotation_failure_preserves_current_publication_and_reports_history_problem() {
    ...
    fs::write(fixture.path().join(runtime::PREVIOUS).join("keep"), "other content").unwrap();
    let report = runtime::store(fixture.path(), &resolved, &metadata("two"));
    assert!(report.published);
    assert!(!report.warnings.is_empty());
    assert_eq!(fs::read_to_string(fixture.path().join(runtime::PREVIOUS).join("keep")).unwrap(), "other content");
    assert_eq!(runtime::decode(&fs::read_to_string(fixture.path().join(runtime::CURRENT)).unwrap(), &registry).unwrap().metadata.version, "two");
...
fn snapshot_symlinks_never_overwrite_their_targets() {
    ...
    let registry = schema();
    let report = runtime::store(fixture.path(), &registry.resolve([]).unwrap(), &metadata("new"));
    assert!(report.published);
    assert_eq!(fs::read_to_string(&target).unwrap(), "keep");
    assert!(fs::symlink_metadata(fixture.path().join(runtime::CURRENT)).unwrap().is_file());
    assert_eq!(fs::metadata(fixture.path().join(runtime::CURRENT)).unwrap().permissions().mode() & 0o777, 0o600);
}
...
fn concurrent_readers_always_see_a_complete_current_snapshot() {
    ...
    ready.recv().unwrap();
    for version in 0..20 { assert!(runtime::store(fixture.path(), &resolved, &metadata(&version.to_string())).published); }
...
fn previous_file_validation_feeds_restart_immutability_without_failing_bad_history() {
    ...
    let changed = registry.resolve([Entry::new(Source::Flag, "routing-mode", "native")]).unwrap();
    let path = fixture.path().join(runtime::CURRENT);
    assert!(runtime::check_previous(&path, &registry, &changed, true, true).unwrap().warnings.is_empty());
    fs::write(&path, "invalid JSON").unwrap();
    assert_eq!(runtime::check_previous(&path, &registry, &changed, true, true).unwrap().warnings, [immutable::Warning::PreviousUnparseable]);
    ...
    assert_eq!(runtime::check_previous(&path, &registry, &changed, false, true).unwrap().changes.first().unwrap().current, Some(Value::Enum("native".into())));
}
...
fn previous_special_files_and_symlinks_are_nonfatal_without_blocking() {
    ...
    fs::write(&ordinary, runtime::encode(&current, &metadata("old")).unwrap()).unwrap();
...
fn oversized_previous_snapshots_are_bounded_and_empty_map_keys_are_invalid() {
    ...
    fs::File::create(&path).unwrap().set_len((runtime::MAX_SNAPSHOT_BYTES as u64).saturating_add(1)).unwrap();
    assert_eq!(runtime::check_previous(&path, &registry, &current, true, true).unwrap().warnings, [immutable::Warning::PreviousUnparseable]);
    let mut document: serde_json::Value = serde_json::from_str(&runtime::encode(&current, &metadata("old")).unwrap()).unwrap();
    document.get_mut("config").unwrap().get_mut("labels").unwrap().as_object_mut().unwrap().insert(String::new(), json!("invalid"));
