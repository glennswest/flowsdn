...
use flowsdn_config::{Class, Entry, KeySpec, Kind, Registry, Resolved, Source, Value};
...
fn config(routing: &str, debug: &str) -> Resolved {
    ...
    Registry::new([routing, KeySpec::new("debug", Kind::Bool, debug)])
        .unwrap()
        .resolve([])
        .unwrap()
}
...
fn immutable_changes_refuse_only_when_restoring_existing_endpoints() {
    ...
    for (restore, endpoints) in [(false, false), (false, true), (true, false)] {
        let report =
            immutable::check(Previous::Parsed(&previous), &current, restore, endpoints).unwrap();
    ...
    assert_eq!(
        error.changes.first().unwrap().previous,
        Some(Value::String("tunnel".into()))
    );
    assert_eq!(
        error.changes.first().unwrap().current,
        Some(Value::String("native".into()))
    );
}
...
fn runtime_values_and_source_changes_do_not_trigger_immutable_diff() {
    ...
    assert!(
        immutable::check(Previous::Parsed(&previous), &current, true, true)
            .unwrap()
            .changes
            .is_empty()
    );
    ...
    let sourced = registry
        .resolve([Entry::new(Source::Flag, "routing-mode", "tunnel")])
        .unwrap();
    assert!(sourced.warnings().is_empty());
    assert!(
        immutable::check(Previous::Parsed(&previous), &sourced, true, true)
            .unwrap()
            .changes
            .is_empty()
    );
...
fn removed_new_and_reclassified_immutable_keys_are_compared() {
    ...
    let reclassified = Registry::new([KeySpec::new("routing-mode", Kind::String, "native")])
        .unwrap()
        .resolve([])
        .unwrap();
...
fn force_is_current_explicit_and_keeps_the_immutable_diff_visible() {
    ...
    let registry = Registry::new([
        routing,
        KeySpec::new("force-config-change", Kind::Bool, "false"),
    ])
    .unwrap();
    let previous = registry
        .resolve([Entry::new(Source::Flag, "force-config-change", "true")])
        .unwrap();
    ...
    assert!(
        immutable::check(Previous::Parsed(&previous), &previous, true, true)
            .unwrap()
            .warnings
            .is_empty()
    );
    ...
    assert_eq!(
        no_restore.warnings,
        [Warning::ChangedWithoutRestoredEndpoints]
    );
}
