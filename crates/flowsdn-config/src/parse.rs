...
pub(super) fn parse(kind: &Kind, raw: &str) -> Result<(Value, bool), String> {
    ...
    let value = match kind {
        ...
        Kind::Int { bits } => {
            ...
            if !valid { return Err("integer out of range".into()); }
            Value::Int(number)
        }
        Kind::UInt { bits } => {
            if !raw.bytes().all(|b| b.is_ascii_digit()) { return Err(invalid()); }
        ...
        Kind::Float { bits } => {
            ...
            if !number.is_finite() { return Err("float must be finite and in range".into()); }
            Value::Float(number)
        }
        Kind::Duration => {
        ...
        Kind::Map => {
            ...
            if !raw.is_empty() {
                for item in raw.split(',') {
                    ...
                    if key.is_empty() { return Err("map key is empty".into()); }
        ...
        Kind::Enum(values) => {
            if !values.iter().any(|v| v == raw) { return Err(format!("expected one of {}", values.join(", "))); }
            Value::Enum(raw.into())
        }
        ...
        Kind::Cidr => {
            ...
            if !prefix.bytes().all(|b| b.is_ascii_digit()) { return Err(invalid()); }
            let prefix: u8 = prefix.parse().map_err(|_| invalid())?;
            if prefix > if address.is_ipv4() { 32 } else { 128 } { return Err("CIDR prefix out of range".into()); }
        ...
        Kind::HostPort => {
            let (host, port) = if let Some(rest) = raw.strip_prefix('[') {
                ...
                if host.contains([':', '[', ']']) { return Err(invalid()); }
                (host, port)
            };
            if host.is_empty() || host.chars().any(char::is_whitespace)
                || !port.bytes().all(|b| b.is_ascii_digit())
            { return Err(invalid()); }
            Value::HostPort { host: host.into(), port: port.parse().map_err(|_| invalid())? }
        }
    };
    Ok((value, false))
}
...
fn duration(raw: &str) -> Result<(i64, bool), String> {
    ...
    if body.is_empty() { return Err(invalid()); }
    ...
    while chars.peek().is_some() {
        ...
        while let Some(digit) = chars.peek().copied().filter(char::is_ascii_digit) {
            chars.next();
            has_digit = true;
            integer = integer.checked_mul(10).and_then(|n| n.checked_add(u128::from(digit.to_digit(10)?))).ok_or_else(invalid)?;
        }
        ...
        if !has_digit { return Err(invalid()); }
        let mut unit = String::new();
        while let Some(ch) = chars.peek().copied().filter(|ch| !ch.is_ascii_digit() && *ch != '.') {
            unit.push(ch);
            chars.next();
        }
        ...
        for digit in fraction.chars().rev() {
            fractional_nanos = u128::from(digit.to_digit(10).ok_or_else(invalid)?)
                .checked_mul(multiplier).and_then(|n| n.checked_add(fractional_nanos))
                .and_then(|n| n.checked_div(10)).ok_or_else(invalid)?;
        }
        total = integer.checked_mul(multiplier).and_then(|n| n.checked_add(fractional_nanos))
            .and_then(|n| total.checked_add(n)).ok_or_else(invalid)?;
    }
    let magnitude = i128::try_from(total).map_err(|_| invalid())?;
    let signed = if negative { magnitude.checked_neg().ok_or_else(invalid)? } else { magnitude };
