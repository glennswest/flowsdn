...
use crate::{
    Class, Effective, Kind, Registry, Resolved, Source, UnknownValue, Value, immutable, normalize,
};
...
fn timestamp(text: &str) -> bool {
    ...
    let Some((date, clock)) = text.split_once(['T', 't']) else {
        return false;
    };
    let mut date = date.split('-');
    let Some(year) = date.next().and_then(|v| number(v, 4)) else {
        return false;
    };
    let Some(month) = date.next().and_then(|v| number(v, 2)) else {
        return false;
    };
    let Some(day) = date.next().and_then(|v| number(v, 2)) else {
        return false;
    };
    ...
    let clock = if let Some(clock) = clock.strip_suffix(['Z', 'z']) {
        clock
    } else if let Some(offset) = clock.rfind(['+', '-']) {
        let Some(zone) = clock.get(offset..).and_then(|s| s.get(1..)) else {
            return false;
        };
        let Some((hours, minutes)) = zone.split_once(':') else {
            return false;
        };
        if number(hours, 2).is_none_or(|v| v > 23) || number(minutes, 2).is_none_or(|v| v > 59) {
            return false;
        }
        let Some(clock) = clock.get(..offset) else {
            return false;
        };
        clock
    } else {
...
fn encoded(value: &Value) -> Result<Json, Error> {
    Ok(match value {
        ...
        Value::Float(value) => Json::Number(
            serde_json::Number::from_f64(*value)
                .ok_or_else(|| invalid("non-finite configuration float"))?,
        ),
        ...
        Value::HostPort { host, port } => Json::String(if host.contains(':') {
            format!("[{host}]:{port}")
        } else {
            format!("{host}:{port}")
        }),
...
pub fn encode(resolved: &Resolved, metadata: &Metadata) -> Result<String, Error> {
    if metadata.version.is_empty() || !timestamp(&metadata.written_at) {
        return Err(invalid(
            "snapshot metadata requires a version and RFC3339 timestamp",
        ));
    }
    ...
    for (key, effective) in resolved.values() {
        ...
        if effective.class == Class::Immutable {
            immutable.push(Json::String(key.clone()));
        }
    }
    ...
    let mut output = serde_json::to_string_pretty(&document)
        .map_err(|_| invalid("cannot serialize runtime configuration"))?;
    output.push('\n');
    if output.len() > MAX_SNAPSHOT_BYTES {
        return Err(invalid("runtime snapshot exceeds the size limit"));
    }
    Ok(output)
}
...
fn object<'a>(document: &'a Map<String, Json>, key: &str) -> Result<&'a Map<String, Json>, Error> {
    document
        ...
        .ok_or_else(|| Error(format!("snapshot field {key} must be an object")))
}
...
fn string<'a>(document: &'a Map<String, Json>, key: &str) -> Result<&'a str, Error> {
    document
        ...
        .filter(|s| !s.is_empty())
        .ok_or_else(|| Error(format!("snapshot field {key} must be a non-empty string")))
}

fn source(value: &Json) -> Result<Source, Error> {
    match value.as_str() {
        Some("default") => Ok(Source::Default),
        Some("file") => Ok(Source::File),
        Some("dir") => Ok(Source::Dir),
        Some("env") => Ok(Source::Env),
        Some("flag") => Ok(Source::Flag),
...
fn typed(value: &Json, kind: &Kind) -> Result<Value, Error> {
    let raw = match kind {
        Kind::List => {
            return value
                .as_array()
                .ok_or_else(|| invalid("expected a JSON array for list"))?
                .iter()
                .map(|value| {
                    value
                        .as_str()
                        .map(str::to_owned)
                        .ok_or_else(|| invalid("list members must be strings"))
                })
        ...
        Kind::Map => {
            return value
                .as_object()
                .ok_or_else(|| invalid("expected a JSON object for map"))?
                .iter()
                .map(|(key, value)| {
                    if key.is_empty() {
                        return Err(invalid("map key is empty"));
                    }
                    value
                        .as_str()
                        .map(|value| (key.clone(), value.to_owned()))
                        .ok_or_else(|| invalid("map values must be strings"))
                })
        ...
        Kind::Bool => value
            ...
            .to_string(),
        Kind::Int { .. } | Kind::Duration => value
            .as_i64()
            .ok_or_else(|| invalid("expected a signed JSON integer"))?
            .to_string(),
        Kind::UInt { .. } => value
            .as_u64()
            .ok_or_else(|| invalid("expected an unsigned JSON integer"))?
            .to_string(),
        Kind::Float { .. } => value
            .as_f64()
            .ok_or_else(|| invalid("expected a JSON number"))?
            .to_string(),
        _ => value
            .as_str()
    ...
    crate::parse::parse(kind, &raw)
        .map(|(value, _)| value)
        .map_err(|error| Error(format!("invalid typed snapshot value: {error}")))
}
        ...
        Json::Number(value) => {
            if let Some(value) = value.as_i64() {
                ...
                value
                    .as_f64()
                    .map(Value::Float)
                    .ok_or_else(|| invalid("invalid numeric snapshot value"))
            }
        }
...
pub fn decode(text: &str, registry: &Registry) -> Result<Snapshot, Error> {
    ...
    let json: Json =
        serde_json::from_str(text).map_err(|_| invalid("invalid runtime configuration JSON"))?;
    let document = json
        .as_object()
        .ok_or_else(|| invalid("runtime configuration must be an object"))?;
    let metadata = Metadata {
        version: string(document, "flowsdn-version")?.into(),
        written_at: string(document, "written-at")?.into(),
    };
    if !timestamp(&metadata.written_at) {
        return Err(invalid("snapshot written-at must be RFC3339"));
    }
    if string(document, "reference-compat")? != REFERENCE_COMPAT {
        return Err(invalid("unsupported reference compatibility version"));
    }
    let config = object(document, "config")?;
    let sources = object(document, "sources")?;
    let unknown = object(document, "unknown-keys")?;
    let immutable = document
        .get("immutable-keys")
        .and_then(Json::as_array)
        .ok_or_else(|| invalid("immutable-keys must be an array"))?;
    let mut immutable_keys = BTreeSet::new();
    for key in immutable {
        let key = key
            .as_str()
            .ok_or_else(|| invalid("immutable keys must be strings"))?;
        if !config.contains_key(key) || !immutable_keys.insert(key) {
            return Err(invalid("immutable key is missing or duplicated"));
        }
    }
    let expected_sources: BTreeSet<_> = config.keys().chain(unknown.keys()).collect();
    if sources.keys().collect::<BTreeSet<_>>() != expected_sources {
        return Err(invalid("snapshot sources do not match configuration keys"));
    }
    ...
    for (key, value) in config {
        if key.is_empty() || normalize(key) != *key || unknown.contains_key(key) {
            return Err(invalid("snapshot keys must be canonical and disjoint"));
        }
        let spec = registry.specs.get(key);
        if spec.is_some_and(|spec| spec.class == Class::Script) {
            return Err(invalid(
                "script keys cannot appear in runtime configuration",
            ));
        }
        let value = match spec {
            Some(spec) => typed(value, &spec.kind)?,
            None => untyped(value)?,
        };
        ...
        let source = source(
            sources
                .get(key)
                .ok_or_else(|| invalid("missing configuration source"))?,
        )?;
    ...
    for (key, value) in unknown {
        if key.is_empty() || normalize(key) != *key {
            return Err(invalid("unknown snapshot keys must be canonical"));
        }
        let raw = value
            .as_str()
            .ok_or_else(|| invalid("unknown snapshot values must be strings"))?;
        let source = source(
            sources
                .get(key)
                .ok_or_else(|| invalid("missing unknown configuration source"))?,
        )?;
...
) -> Result<immutable::Report, immutable::Incompatible> {
    match read_previous(path) {
        Ok(text) => match decode(&text, registry) {
            Ok(snapshot) => immutable::check(
                immutable::Previous::Parsed(&snapshot.resolved),
                current,
                restore,
                has_endpoint_state,
            ),
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => immutable::check(
            immutable::Previous::Absent,
            current,
            restore,
            has_endpoint_state,
        ),
...
fn read_previous(path: &Path) -> std::io::Result<String> {
    ...
    #[cfg(not(unix))]
    {
        ...
        Err(std::io::Error::new(
            ...
            "safe snapshot reads require Unix",
        ))
    }
    #[cfg(unix)]
    {
        ...
        if !metadata.is_file() || metadata.len() > MAX_SNAPSHOT_BYTES as u64 {
            return Err(std::io::Error::new(
                ...
                "snapshot must be a bounded regular file",
            ));
        }
...
impl StoreReport {
    fn warn(&mut self, operation: &'static str, error: impl fmt::Display) {
        self.warnings.push(StoreWarning {
            operation,
...
pub fn store(dir: &Path, resolved: &Resolved, metadata: &Metadata) -> StoreReport {
    ...
    let text = match encode(resolved, metadata) {
        Ok(text) => text,
        Err(error) => {
            report.warn("encode", error);
            return report;
        }
    };
    ...
    let history = fs::symlink_metadata(&current).and_then(|metadata| {
        if metadata.is_file() {
            fs::hard_link(&current, &backup_path)
        } else {
            Err(std::io::Error::other(
                "current snapshot is not a regular file",
            ))
        }
    });
    match history {
        Ok(()) => {
            ...
            match fs::symlink_metadata(&previous) {
                Ok(metadata) if metadata.is_file() => {
                    if let Err(error) = fs::rename(&previous, &oldest) {
                        report.warn("rotate-previous", error);
                    }
                }
            ...
            if let Err(error) = fs::rename(&backup.0, &previous) {
                report.warn("rotate-current", error);
            }
        }
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {}
        Err(error) => report.warn("stage-history", error),
    }
    if let Err(error) = fs::rename(&staging.0, &current) {
        report.warn("publish", error);
        return report;
    }
    report.published = true;
    if let Err(error) = File::open(dir).and_then(|file| file.sync_all()) {
        report.warn("sync-directory", error);
    }
    report
}
