...
prefix the datapath cannot serve black-holes it; the ClusterIP address type in
particular MUST NOT be offered to users until the datapath setting it depends
on is confirmed (open decision O-8).
...
**O-6. Preserve reference policy naming.** `peer-<name>-export` and
`<Type>-<resource>-ipv4` are ugly but leak into CLI output and into the twenty
ported scenarios. *Recommendation:* preserve them exactly. The cost of
divergence is rewriting twenty pinned expectation files and breaking runbooks;
the benefit is cosmetic.

**O-7. `CiliumBGPNodeConfigOverride.peers[].localPort`.** Dead in the
reference. *Recommendation:* keep the field, ignore it, document it (§2). It
costs nothing and manifests written for the reference keep validating.
Implementing it (binding the local source port) would be a new behavior with no
consumer.

**O-8. ClusterIP advertisement and `bpf.lbExternalClusterIP`.** Advertising a
ClusterIP is only useful if the datapath answers external traffic for it.
*Recommendation:* keep `ClusterIP` in the CRD enum (compatibility) and emit a
`warn` at reconcile time when a `ClusterIP` advertisement is configured while
the datapath setting is off, naming the setting. Do not refuse the
configuration — an operator may have external reachability arranged another
way.
...
**O-10. Per-peer `backend` selection.** §3.17 makes `bgp-backend` a node-level
setting. A node could in principle drive some peers through the speaker and
others through a router. *Recommendation:* no — keep it node-level. Per-peer
backends would mean two next-hop resolution models and two status sources in
one instance for no demonstrated use case. Revisit only with a concrete
