...
| `/statedb/query` on the API socket | resolved #46: the reference client uses `GET` with a JSON body; flowsdn MUST accept both `GET` and `POST` with body `{"table":"health","index":"id","key":"<base64>","lowerbound":bool}` and stream `{"rev":n,"obj":<HealthStatus>}` objects then EOF, or `{"err":"…"}`. Only `table == "health"`; other tables `404`. This is what upstream `cilium-dbg status` calls to print "Modules Health" |
