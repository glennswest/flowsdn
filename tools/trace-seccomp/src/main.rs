...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(arg) = args.next() {
        ...
        match arg.as_str() {
            ...
            "--caps" => caps = Some(value.split(',').filter(|v| !v.is_empty()).map(str::to_owned).collect::<BTreeSet<_>>()),
            "--exe" => { executables.insert(value); },
    ...
    if output == report || output == baseline_path || report == baseline_path || traces.contains(&output) || traces.contains(&report) { return Err("input and output paths must differ".into()); }
    if executables.is_empty() || traces.is_empty() { return Err("at least one --exe and --trace required".into()); }
    ...
    if caps.iter().any(|c| !c.starts_with("CAP_") || !c.bytes().all(|b| b.is_ascii_uppercase() || b == b'_')) { return Err("invalid capability name".into()); }
    ...
    for path in traces {
        if !unique_files.insert(std::fs::canonicalize(&path)?) { return Err("duplicate trace file".into()); }
        ...
        if parsed.selected_execs == 0 { unattributed_files = unattributed_files.saturating_add(1); }
        for (name, count) in parsed.counts { let total = counts.entry(name).or_default(); *total = total.saturating_add(count); }
    }
    if selected_execs == 0 { return Err("no successful selected executable execve found".into()); }
    let calls: BTreeMap<_, _> = counts.into_iter().map(|(name, count)| {
    ...
    use std::io::Write;
    let mut profile_file = std::fs::OpenOptions::new().write(true).create_new(true).open(output)?;
    let mut report_file = std::fs::OpenOptions::new().write(true).create_new(true).open(report)?;
    writeln!(profile_file, "{}", serde_json::to_string_pretty(&generated)?)?;
    writeln!(report_file, "{}", serde_json::to_string_pretty(&summary)?)?;
    Ok(())
}

fn main() {
    if let Err(error) = run() { eprintln!("trace-seccomp: {error}"); std::process::exit(1); }
}
