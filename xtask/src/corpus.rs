...
fn files(root: &Path, output: &mut Vec<std::path::PathBuf>) -> Result<(), Box<dyn Error>> {
    for entry in fs::read_dir(root)? {
        ...
        if kind.is_dir() { files(&entry.path(), output)?; }
        else if kind.is_file() && entry.path().extension().is_some_and(|s| s=="txtar") { output.push(entry.path()); }
    }
    Ok(())
}
        ...
        if let Some(line)=line.strip_prefix("#!") {
            for token in tokenize(line,index.saturating_add(1))? {
                ...
                if let Some(flag)=value.strip_prefix("--") {
                    ...
                    if name.is_empty() { return Err("empty startup flag".into()); }
...
pub fn report() -> Result<Value, Box<dyn Error>> {
    let mut paths=Vec::new(); files(Path::new("tests/scripttest/corpus"),&mut paths)?; paths.sort();
    ...
    for path in &paths {
        ...
        for key in startup_flags(archive.script())? { flags.entry(key).or_default().insert(path.to_string_lossy().into_owned()); }
        for line in parse_script(archive.script())? {
            if let Line::Command(command)=line { if let Some(name)=command.words.first() { commands.insert(name.literal()); } }
        }
        names.extend(symbols.find_iter(&source).map(|m|m.as_str().to_owned()));
    }
    let unknown:Vec<_>=flags.keys().filter(|key|flowsdn_config::catalogue::get(key).is_none()).cloned().collect();
    Ok(json!({"reference":"7d68cfb394", "files":paths.len(), "startup_flags":flags,
...
pub fn run(check: bool) -> Result<(), Box<dyn Error>> {
    let report=report()?;
    if check {
        let expected:Value=serde_json::from_str(&fs::read_to_string("tests/scripttest/naming-audit.json")?)?;
        if expected != report { return Err("corpus naming inventory changed; regenerate and review naming-audit.json".into()); }
        println!("corpus naming inventory matches reviewed snapshot");
    } else { println!("{}",serde_json::to_string_pretty(&report)?); }
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    fn startup_names_ignore_flag_values_and_embedded_data() {
        ...
        assert_eq!(startup_flags(archive.script()).expect("flags"),BTreeSet::from(["cluster-name".into(),"metrics".into()]));
    }
}
